...
import {
  ...
  nonSecretHeaders,
  recordPolicyDenial,
} from '../src/llm/policy-flight-recorder.js';
...
function capture(dataDirectory: string, i = 0) {
  ...
  return recordPolicyDenial(
    ...
    },
    {
      ...
      body: Buffer.from(
        '{"error":{"message":"prompt flagged for usage policy"}}',
      ),
    },
...
test('policy flight recorder detects provider denial text', () => {
  assert.equal(
    isPolicyDenial(
      new Error('prompt was flagged as potentially violating our usage policy'),
    ),
    true,
  );
...
test('policy flight recorder strips secret transport headers', () => {
  ...
  assert.deepEqual(nonSecretHeaders(headers), {
    'content-type': 'application/json',
    session_id: 'keep-me',
  });
});
...
test('policy flight recorder writes replayable exact-byte bundle', () => {
  const dataDirectory = fs.mkdtempSync(
    path.join(os.tmpdir(), 'elpis-policy-flight-'),
  );
  ...
  assert.equal(
    fs.readFileSync(
      path.join(record.directory, manifest.request.bodyFile),
      'utf8',
    ),
    '{"i":0}',
  );
  assert.match(
    fs.readFileSync(
      path.join(record.directory, manifest.response.bodyFile),
      'utf8',
    ),
    /flagged/,
  );
  ...
  for (const name of ['manifest.json', 'request-body.bin', 'response-body.bin'])
    assert.equal(
      fs.statSync(path.join(record.directory, name)).mode & 0o777,
      0o600,
    );
});
...
test('policy flight recorder retains every bundle from the last seven days and prunes older bundles', () => {
  const dataDirectory = fs.mkdtempSync(
    path.join(os.tmpdir(), 'elpis-policy-retain-'),
  );
  ...
  assert.equal(
    recent.length,
    12,
    'no count ceiling within the retention window',
  );
  ...
  assert.equal(
    ...
    12,
    'one expired bundle pruned and one new bundle added',
  );
});
