...
test('parseCustomEmotes: ignores mentions, timestamps, urls, malformed markup', () => {
  ...
  assert.deepEqual(
    parseCustomEmotes('<t:1753343555:R> and <t:1753343555>'),
    [],
  );
  assert.deepEqual(parseCustomEmotes('<https://example.com/a:b:123456>'), []);
  assert.deepEqual(
    parseCustomEmotes('<ab:cd:123456> <::123456> <:name:abc>'),
    [],
  );
});
...
test('emoteCdnUrl: emote png/gif, sticker hosts per format', () => {
  const e = (animated: boolean): EmoteOrStickerRef => ({
    kind: 'emote',
    id: '99',
    name: 'x_y',
    animated,
  ...
  assert.match(
    emoteCdnUrl(e(false)),
    /^https:\/\/cdn\.discordapp\.com\/emojis\/99\.png\?/,
  );
  assert.match(
    emoteCdnUrl(e(true)),
    /^https:\/\/cdn\.discordapp\.com\/emojis\/99\.gif\?/,
  );
  assert.equal(
    emoteCdnUrl({ kind: 'sticker', id: '7', name: 's', format: 'gif' }),
    'https://media.discordapp.net/stickers/7.gif',
  );
  assert.equal(
    emoteCdnUrl({ kind: 'sticker', id: '7', name: 's', format: 'lottie' }),
    'https://cdn.discordapp.com/stickers/7.json',
  );
  assert.equal(
    emoteCdnUrl({ kind: 'sticker', id: '7', name: 's', format: 'apng' }),
    'https://cdn.discordapp.com/stickers/7.png',
  );
  assert.equal(
    emoteCdnUrl({ kind: 'sticker', id: '7', name: 's', format: 'png' }),
    'https://cdn.discordapp.com/stickers/7.png',
  );
});
...
test('isAnimated / refKey', () => {
  assert.equal(
    isAnimated({ kind: 'emote', id: '1', name: 'a_b', animated: true }),
    true,
  );
  assert.equal(
    isAnimated({ kind: 'emote', id: '1', name: 'a_b', animated: false }),
    false,
  );
  assert.equal(
    isAnimated({ kind: 'sticker', id: '1', name: 's', format: 'apng' }),
    true,
  );
  assert.equal(
    isAnimated({ kind: 'sticker', id: '1', name: 's', format: 'gif' }),
    true,
  );
  assert.equal(
    isAnimated({ kind: 'sticker', id: '1', name: 's', format: 'png' }),
    false,
  );
  assert.equal(
    isAnimated({ kind: 'sticker', id: '1', name: 's', format: 'lottie' }),
    false,
  );
  assert.equal(
    refKey({ kind: 'emote', id: '5', name: 'n_m', animated: false }),
    'e:5',
  );
  assert.equal(
    refKey({ kind: 'sticker', id: '5', name: 's', format: 'png' }),
    's:5',
  );
});
...
test('attachmentName carries kind, name and id; frames only when of > 1', () => {
  ...
  assert.equal(
    attachmentName(e, { frame: 2, of: 4 }),
    'emote-blobwave-123456789-frame2of4.png',
  );
  assert.equal(
    attachmentName(e, { frame: 1, of: 1 }),
    'emote-blobwave-123456789.png',
  );
  const s: EmoteOrStickerRef = {
    kind: 'sticker',
    id: '77',
    name: 'Wave Hello',
    format: 'lottie',
  ...
  assert.equal(
    attachmentName(s, { ext: 'json' }),
    'sticker-wave-hello-77.json',
  );
});
...
test('registry: first use attaches, second use is silent, reset re-arms, cache skips re-download', async () => {
  ...
  const reg = createEmoteRegistry({
    ...
    keyframes: 4,
    cacheDir: tmpCache(),
    fetchFn: stubFetch(TINY_PNG, calls),
  });
  ...
  assert.deepEqual(
    await reg.collect({ content: '<:blobheart:111111> once more' }),
    [],
  );
...
test('registry: failed download is not marked seen and retries on next use', async () => {
  ...
  const reg2 = createEmoteRegistry({
    log: noopLogger,
    keyframes: 4,
    cacheDir: dir,
    fetchFn: stubFetch(TINY_PNG, calls),
...
test('registry: lottie sticker degrades to a name-only metadata entry', async () => {
  ...
  const out = await reg.collect({
    content: '',
    stickers: [{ id: '333', name: 'Wave Hello', format: 3 }],
  });
...
test('registry: unknown sticker format is skipped entirely', async () => {
  const reg = createEmoteRegistry({
    ...
    keyframes: 4,
    cacheDir: tmpCache(),
    fetchFn: stubFetch(TINY_PNG, []),
  });
  assert.deepEqual(
    await reg.collect({
      content: '',
      stickers: [{ id: '9', name: 'future', format: 42 }],
    }),
    [],
  );
  assert.equal(reg.seenCountForTest(), 0);
...
test('registry: per-message cap defers the excess to the next use', async () => {
  ...
  const content =
    '<:a1:100001> <:a2:100002> <:a3:100003> <:a4:100004> <:a5:100005>';
...
test('registry: soft deadline — a slow first-use download does not block collect, attaches on next use', async () => {
  ...
  assert.ok(
    Date.now() - t0 < 2000,
    'collect must return around the deadline, not wait for the download',
  );
  assert.equal(
    reg.seenCountForTest(),
    0,
  ...
  assert.equal(
    calls.length,
    1,
    'the timed-out download was reused, not re-fetched',
  );
});

test(
  'registry: keyframes=1 attaches ONE extracted static PNG frame for an animated emote',
  { skip: !HAS_FFMPEG },
  async () => {
    const dir = tmpCache();
    ...
    const gen = spawnSync('ffmpeg', [
      ...
      'lavfi',
      '-i',
      'testsrc=duration=1:rate=10:size=96x96',
      gifPath,
    ]);
    ...
    const reg = createEmoteRegistry({
      ...
      keyframes: 1,
      cacheDir: dir,
      fetchFn: stubFetch(fs.readFileSync(gifPath), []),
    });
    ...
    assert.equal(
      out[0].contentType,
      'image/png',
      'a single downscaled PNG frame, not the raw gif',
    );
  },
);
...
test('registry: static PNG sticker attaches as a single image', async () => {
  ...
  const out = await reg.collect({
    content: '',
    stickers: [{ id: '444', name: 'OK!', format: 1 }],
  });
  assert.equal(out.length, 1);
...
test(
  'registry: animated emote extracts evenly spaced keyframes via ffmpeg',
  { skip: !HAS_FFMPEG },
  async () => {
...
test('agent: transcript persist strips base64 contentParts (persistable), in-memory keeps them', async () => {
  ...
  const userMsg = agent.messagesForTest.find(
    (m) => m.role === 'user' && m.content.includes('look at this'),
  );
  assert.ok(
    userMsg?.contentParts && userMsg.contentParts.length === 2,
    'in-memory history keeps the image parts',
  );

  const sessionsDir = path.join(tmpDir, 'sessions');
  const files = fs
    .readdirSync(sessionsDir, { recursive: true, encoding: 'utf8' })
  ...
  assert.ok(
    !raw.includes('contentParts'),
    'no contentParts field reaches the transcript',
  );
  assert.ok(
    !raw.includes('base64,'),
    'no base64 payload reaches the transcript',
  );
});
...
test('agent: compaction-apply re-arms the emote seen-set', async () => {
  ...
  assert.equal(
    resets,
    1,
    'compaction apply should reset the seen-set exactly once',
  );
  agent.stop();
});

test(
  'registry: animated APNG sticker extracts keyframes via ffmpeg',
  { skip: !HAS_FFMPEG },
  async () => {
    const dir = tmpCache();
    ...
    const gen = spawnSync('ffmpeg', [
      ...
      'lavfi',
      '-i',
      'testsrc=duration=1:rate=10:size=100x100',
      '-f',
      'apng',
      apngPath,
    ]);
    ...
    const reg = createEmoteRegistry({
      ...
      keyframes: 3,
      cacheDir: dir,
      fetchFn: stubFetch(apng, []),
    });
    const out = await reg.collect({
      content: '',
      stickers: [{ id: '666', name: 'Bouncy', format: 2 }],
    });
