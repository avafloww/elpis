...
test('sandbox: ponder slugifies thread names + close() is collision-safe', async () => {
  ...
  assert.ok(
    fs.existsSync(path.join(tmp, 'ponder', `${slug}.md`)),
  ...
  assert.ok(
    !fs.existsSync(path.join(tmp, 'SOUL.md')),
  ...
  assert.ok(
    fs.existsSync(path.join(resolvedDir, 'dup-topic.md')),
    'first archive kept',
  );
  assert.ok(
    fs.existsSync(path.join(resolvedDir, 'dup-topic-2.md')),
...
test('e2e: require("node:fs") reads a file', async () => {
  ...
  const r = await sandbox.run(
    `require("node:fs").readFileSync(${JSON.stringify(probe)}, "utf8").trim()`,
  );
  assert.equal(r.ok, true);
...
test('e2e: require("node:fs") writes a file', async () => {
  ...
  const r = await sandbox.run(
    `require("node:fs").writeFileSync(${JSON.stringify(out)}, "written"); "ok"`,
  );
  assert.equal(r.ok, true);
...
test('e2e: fs global reads a file', async () => {
  const r = await sandbox.run(
    `fs.readFileSync("${memoryPath.replace(/\\/g, '\\\\')}", "utf8")`,
  );
  assert.equal(r.ok, true, String(r.error));
...
test('e2e: elpis.edit() replaces a unique substring and returns a diff', async () => {
  ...
  const r = await sandbox.run(
    `elpis.edit(${JSON.stringify(f)}, "two\\nthree", "TWO\\nTHREE")`,
  );
  assert.equal(r.ok, true, String(r.error));
  ...
  assert.match(
    r.preview || '',
    /@@ -2,2 \+2,2 @@/,
    'result must carry the diff header',
  );
...
test('e2e: elpis.edit() on a non-unique needle throws with a line-numbered count', async () => {
  ...
  const dup = await sandbox.run(
    `elpis.edit(${JSON.stringify(f)}, "= 1;", "= 2;")`,
  );
...
test('e2e: elpis.edit(..., { replaceAll: true }) replaces every occurrence', async () => {
  ...
  const r = await sandbox.run(
    `elpis.edit(${JSON.stringify(f)}, "foo", "bar", { replaceAll: true })`,
  );
  assert.equal(r.ok, true, String(r.error));
...
test('e2e: elpis.edit() not-found throws with a near-miss window', async () => {
  ...
  const r = await sandbox.run(
    `elpis.edit(${JSON.stringify(f)}, "const timeuot = 5000;", "x")`,
  );
  assert.equal(r.ok, false);
...
test('e2e: elpis.memory.search finds matches across MEMORY.md, people/ and ponder/', async () => {
  ...
  try {
    ...
    const r2 = await sandbox.run(
      'elpis.memory.search("FALCON metaphors (missing)")',
    );
...
test('no-op run: empty/comment-only code returns an explicit empty-program note', async () => {
  for (const code of ['', '   ', '// just a comment', '/* block */\n// line']) {
    ...
    assert.match(
      r.preview || '',
      /empty program — nothing executed/,
...
test('elpis.channel(name): resolves a room name to its id; unknown name throws with known rooms', async () => {
  ...
  const named = createSandbox({
    ...
    resolveChannel: (ref) => {
      ...
      return clean === 'unnamed-agent' || clean === '111222333'
        ? '111222333'
        : null;
    },
  });
  const ok = await named.run(
    'await elpis.channel("unnamed-agent").send("hi via name")',
  );
  assert.equal(ok.ok, true);
  assert.equal(sent.length, 1);
  assert.equal(sent[0].channelId, '111222333', 'name resolved to the real id');
  const hash = await named.run(
    'await elpis.channel("#unnamed-agent").send("with hash")',
  );
  ...
  assert.match(
    bad.error || '',
    /unnamed-agent/,
    'the throw lists known rooms by their qualified name',
  );
  // an all-digits id skips resolution entirely
  const byId = await named.run(
    'await elpis.channel("111222333").send("by id")',
  );
...
test('elpis.channel().send: literal escape sequences throw unless allowEscapes', async () => {
  ...
  const bad = await sb.run(
    'await elpis.channel("111").send("line one\\\\nline two")',
  );
  ...
  const good = await sb.run(
    'await elpis.channel("111").send("line one\\nline two")',
  );
  assert.equal(good.ok, true, String(good.error));
  assert.equal(sent.length, 1);
  // Override sends the backslashes verbatim.
  const forced = await sb.run(
    'await elpis.channel("111").send("regex: \\\\d+\\\\n", { allowEscapes: true })',
  );
...
test('B4: elpis.read() honors from/to slice', async () => {
  ...
  const r = await sandbox.run(
    `elpis.read(${JSON.stringify(f)}, { from: 2, to: 4 })`,
  );
  assert.equal(r.ok, true);
...
test('B4: elpis.read() numbers:false drops line numbers', async () => {
  ...
  const r = await sandbox.run(
    `elpis.read(${JSON.stringify(f)}, { numbers: false })`,
  );
  assert.equal(r.ok, true);
...
test("B4/2a: elpis.read() self-paginates a large file — the continuation marker survives elpis.preview()'s downstream head/tail split", async () => {
  ...
  assert.match(
    r.preview || '',
    /continue: read\(/,
    "the continuation marker must survive elpis.preview()'s own truncation",
  );
...
test('E3: elpis.memory.person() creates people/ file with frontmatter stub', async () => {
  ...
  const sb = createSandbox({
    ...
    memory: {
      read: () => fs.readFileSync(memoryPath, 'utf8'),
      append: (t) => fs.appendFileSync(memoryPath, t),
      overwrite: (t) => fs.writeFileSync(memoryPath, t),
    },
    logbuf: [],
    inbound: {
      id: 'x',
  ...
  const r = await sb.run(
    'elpis.memory.person("Bramble", "wants my identity based in myself")',
  );
  assert.equal(r.ok, true, String(r.error));
...
test('elpis.memory.person() for a NON-author slug creates the file WITHOUT the author id', async () => {
  ...
  const sb = createSandbox({
    ...
    memory: {
      read: () => fs.readFileSync(memPath, 'utf8'),
      append: (t) => fs.appendFileSync(memPath, t),
      overwrite: (t) => fs.writeFileSync(memPath, t),
    },
    logbuf: [],
    inbound: {
      id: 'x',
  ...
  assert.doesNotMatch(
    body,
    /discord:111/,
    'must not stamp the inbound author id onto a non-author file',
  );
});
...
test('elpis.memory.person() for the inbound author DOES pre-fill their id', async () => {
  ...
  const r = await sb.run(
    'elpis.memory.person("Bramble", "wants identity in herself")',
  );
  assert.equal(r.ok, true, String(r.error));
  ...
  assert.match(
    body,
    /ids: \[discord:111\]/,
    'the inbound author own file must pre-fill their id',
  );
});
...
test('git: commitAndPush stages untracked files by default', async () => {
  ...
  execSync(
    `git init && git config user.name "Test" && git config user.email "test@example.com" && git remote add origin ${remote} && git commit --allow-empty -m init && git push -u origin HEAD`,
    { cwd: dir },
  );
  ...
  assert.match(
    tracked,
    /new\.txt/,
    'the untracked file was staged + committed',
  );
});
...
test('deploy: refuses to build/restart a dirty tree unless allowDirty', async () => {
  ...
  execSync(
    'git init && git config user.name "Test" && git config user.email "test@example.com" && git commit --allow-empty -m init',
    { cwd: dir },
  );
...
test('deploy: a successful build still refuses restart when the fresh dist rejects live config', async () => {
  ...
  execSync(
    'git init && git config user.name "Test" && git config user.email "test@example.com" && git add . && git commit -m init',
    { cwd: dir },
  );
...
test('grep: finds matches in a path, returns raw file:line hits', async () => {
  ...
  const hit = await sb.run(
    `await elpis.grep("NEEDLE", { path: ${JSON.stringify(dir)} })`,
  );
...
test('concurrent runs keep their logs separate (reentrant, no shared buffer)', async () => {
  const [r1, r2] = await Promise.all([
    sandbox.run(
      'console.log("A1"); await new Promise(res => setTimeout(res, 40)); console.log("A2"); 1',
    ),
    sandbox.run(
      'console.log("B1"); await new Promise(res => setTimeout(res, 15)); console.log("B2"); 2',
    ),
  ]);
...
test('post-detach logs do NOT bleed into the next run; DO arrive with the settle notice', async () => {
  ...
  const detachSandbox = createSandbox({
    config: {
      ...
      kagi: { apiKey: null },
      paths: { harnessRoot: tmp2, dataDirectory: tmp2 },
    },
    memory: { read: () => '', append: () => {}, overwrite: () => {} },
    logbuf: [],
    bg: createBgRegistry(tmp2),
    onFutureSettled: (id, value, rejected, logs) => {
      settled.push({ id, value, rejected, logs });
    },
  });
  ...
  assert.equal(
    r.logs,
    'pre-detach',
    'the detached RunResult carries only the pre-detach logs',
  );
  assert.match(
    r.preview ?? '',
    /\[bg <[^>]+>\]/,
    'detached preview uses [bg <id>] format',
  );
  ...
  assert.equal(
    settled[0].logs,
    'post-detach',
    'post-detach logs arrive with the settle notice',
  );
});
...
test('heredoc: runs end-to-end through the sandbox with zero escaping', async () => {
  ...
  const r2 = await sandbox.run(
    '[hd.includes("`tpl ${x}`"), hd.includes(String.fromCharCode(92) + String.fromCharCode(34) + "q"), hd.endsWith(String.fromCharCode(10))]',
  );
...
test('heredoc: missing terminator is a teachable pre-parse error', async () => {
  ...
  assert.match(
    r.error ?? '',
    /heredoc <<<EOF opened at line 1 has no terminator/,
  );
});
...
test('heredoc: a marker inside a string literal is NOT expanded', async () => {
  ...
  assert.equal(
    r.code,
    src,
    'source with marker only inside a string must be untouched',
  );
});
...
test('heredoc: two blocks in one run; content mentioning another marker stays literal', async () => {
  ...
  assert.equal(r.error, undefined);
  assert.ok(
    r.code.includes(JSON.stringify('mentions <<<TWO here\n')),
    'first block verbatim',
  );
  assert.ok(
    r.code.includes(JSON.stringify('second\n')),
...
test('elpis.sh.q: quoted value round-trips through a real shell byte-for-byte', async () => {
  const r = await sandbox.run(
    'const tricky = `weird \'val\' "q" $HOME \\u005c backtick`; (await elpis.sh("printf %s " + elpis.sh.q(tricky))).stdout === tricky',
  );
  assert.equal(r.ok, true, String(r.error));
