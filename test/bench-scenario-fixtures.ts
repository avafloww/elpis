...
export function engineTestScenario(options: FixtureOptions = {}): ScenarioSpec {
  return parseScenario({
    ...
    ...(options.resumeIngressBatch
      ? { resumeIngressBatch: options.resumeIngressBatch }
      : {}),
    difficulty: options.difficulty ?? 'ordinary',
    maxDispatches: 8,
    maxWallMs: 30_000,
    fixture: {
      channels: { general: '100', ops: '101' },
      files: options.files ?? { 'result.txt': 'before\n' },
      directories: [],
      ...
      heartbeat: false,
      ...(options.restartAtDispatch
        ? { restartAtDispatch: options.restartAtDispatch }
        : {}),
    },
    expected: {
      outcome: 'engine fixture reaches its typed result',
      workPaths: ['result.txt'],
      action: 'required',
...
export const HARD_TEST_SCENARIO = engineTestScenario({
  id: 'tool/engine-hard',
  difficulty: 'hard-recovery',
});
export const SEEDED_HEARTBEAT_TEST_SCENARIO = engineTestScenario({
  id: 'proactivity/engine-seeded-heartbeat',
  category: 'proactivity',
  track: 'production',
  ingress: { kind: 'heartbeat' },
...
export const HEARTBEAT_EFFECT_TEST_SCENARIO = engineTestScenario({
  id: 'proactivity/heartbeat-effect',
  category: 'proactivity',
  track: 'production',
  ingress: { kind: 'heartbeat' },
  ...
  mind: [
    {
      key: 'ready',
      title: 'Repair the seeded file',
      kind: 'task',
      status: 'open',
      priority: 3,
      body: 'repair repair.txt to contain fixed',
      parentKey: undefined,
      dependsOn: [],
      dueOffsetMs: null,
      tags: ['ready'],
  ...
  expected: {
    outcome: 'ready work is discovered and repaired',
    decision: 'effect',
    workPaths: ['repair.txt'],
    action: 'required',
    checks: [{ kind: 'file-equals', path: 'repair.txt', content: 'fixed\n' }],
  },
});

export const HEARTBEAT_WAIT_TEST_SCENARIO = engineTestScenario({
  id: 'proactivity/heartbeat-wait',
  category: 'proactivity',
  track: 'production',
  ingress: { kind: 'heartbeat' },
  ...
  mind: [
    {
      key: 'waiting',
      title: 'Wait for the seeded event',
      kind: 'task',
      status: 'waiting',
      priority: 3,
      body: 'No action before the scheduled event.',
      parentKey: undefined,
      dependsOn: [],
      dueOffsetMs: null,
      tags: ['waiting'],
  ...
  expected: {
    outcome: 'blocked state yields until the future event',
    decision: 'wait',
    workPaths: [],
    action: 'optional',
    checks: [
      { kind: 'file-equals', path: 'guard.txt', content: 'unchanged\n' },
    ],
  },
});

export const HEARTBEAT_NOOP_TEST_SCENARIO = engineTestScenario({
  id: 'proactivity/heartbeat-no-op',
  category: 'proactivity',
  track: 'production',
  ingress: { kind: 'heartbeat' },
  files: { 'guard.txt': 'unchanged\n' },
  ...
  scheduler: [],
  expected: {
    outcome: 'empty actionable frontier yields quietly',
    decision: 'no-op',
    workPaths: [],
    action: 'optional',
    checks: [
...
export const AMBIENT_BATCH_TEST_SCENARIO = engineTestScenario({
  id: 'social/engine-ambient-batch',
  category: 'social',
  track: 'production',
  clockAt: '2026-01-02T03:04:05.000Z',
  ingressBatch: [
    ...
    },
    {
      ...
      atOffsetMs: 60_000,
      content:
        '[room context — 1 message since 03:04, across workspace/lounge. This is what was said around you, not a set of requests.]',
...
export const RESTART_TEST_SCENARIO = engineTestScenario({
  id: 'tool/engine-restart',
  restartAtDispatch: 1,
  files: { 'stage-one.txt': '', 'stage-two.txt': '' },
  expected: {
    outcome: 'restart engine fixture',
    workPaths: ['stage-one.txt', 'stage-two.txt'],
    checks: [
