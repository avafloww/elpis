...
test('preview: nested strings keep 200 chars before truncation (richer defaults)', () => {
  ...
  assert.ok(
    p.includes('a'.repeat(197) + '…'),
    'nested string should keep ~200 chars',
  );
...
test('preview: opts.strCap and opts.maxDepth are honored', () => {
  ...
  assert.ok(
    shallow.includes('b'.repeat(17) + '…'),
    'strCap should bound nested strings',
  );
...
test('capLines: truncates at the last newline boundary, never mid-line', () => {
  const lines = Array.from(
    { length: 50 },
    (_, i) => `log line ${i + 1} with some padding text`,
  );
  const s = lines.join('\n');
  ...
  assert.ok(
    markerIdx > -1,
    'marker must be present (input exceeds the budget)',
  );
  const keptLines = out.slice(0, markerIdx).split('\n');
  for (const l of keptLines) {
    assert.ok(
      lines.includes(l),
      `every kept line must be a COMPLETE original line, got: ${JSON.stringify(l)}`,
    );
  }
  assert.match(
    out,
    /\[showing first \d+ of 50 logged lines; \+\d+ more bytes — the values live in your variables, print a narrower slice\]/,
  );
});
...
test('capLines: N/M/K counts in the marker are accurate', () => {
  ...
  const m =
    /\[showing first (\d+) of (\d+) logged lines; \+(\d+) more bytes/.exec(out);
  ...
  assert.equal(
    shown.split('\n').length,
    Number(nStr),
    'N must match the actually-kept line count',
  );
  assert.equal(
    totalBytes - Buffer.byteLength(shown, 'utf8'),
    Number(kStr),
    'K must be the exact byte remainder',
  );
});
...
test('capLines: a single logged line longer than the budget falls back to the byte cap', () => {
  ...
  assert.doesNotMatch(
    out,
    /logged lines/,
    'no line-boundary marker possible with zero complete lines',
  );
  assert.match(out, /more bytes\]/, "falls back to cap()'s own marker");
  assert.equal(
    out,
    cap(s, 100),
    "must be exactly cap()'s output, no double marker",
  );
});
...
test('B2: long string preview scales with budget', () => {
  ...
  assert.doesNotMatch(
    p1,
    /slice it for more/,
    'the old generic elision suffix must be gone',
  );
...
test('preview: string elision names the exact slice offsets that reproduce the elided middle', () => {
  ...
  assert.ok(
    p.includes(JSON.stringify(s.slice(0, headEnd))),
    'headEnd must match the head string actually shown',
  );
  assert.ok(
    p.includes(JSON.stringify(s.slice(tailStart))),
    'tailStart must match the tail string actually shown',
  );
  assert.ok(
    headEnd < tailStart,
...
test('B3: sh result stdout tail-biased when over budget', () => {
  ...
  const p = preview(
    { stdout: longStdout, stderr: '', code: 1, signal: null },
    1024,
  );
...
test('formatRead: a render that fits under budget is byte-identical to the un-paginated form', () => {
  ...
  assert.ok(
    !out.includes('continue: read('),
    'no continuation marker when the full render already fits',
  );
});
...
test('formatRead: line prefixes are plain NN: line numbers (no content hash)', () => {
  ...
  assert.match(
    out,
    /^\s*1: alpha$/m,
    `line 1 must be a plain "NN: " prefix, got: ${out}`,
  );
  assert.doesNotMatch(
    out,
    /^\s*1:[0-9a-f]{2}:/m,
    'no 2-hex hash after the line number',
  );
});
...
test('formatRead: clamps at the last complete line and appends a continuation marker', () => {
  ...
  const content = Array.from(
    { length: nLines },
    (_, i) => `line ${i + 1} of ${nLines} with some padding text here`,
  ).join('\n');
  const out = formatRead('big.txt', content, {}, 500);
  const m =
    /showing lines 1–(\d+) of 100 — continue: read\('big\.txt', \{from: (\d+)\}\)/.exec(
      out,
    );
  assert.ok(
    m,
    `expected a clamped continuation marker, got tail: ${out.slice(-200)}`,
  );
  const shownTo = Number(m[1]);
  const contFrom = Number(m[2]);
  assert.ok(
    shownTo < nLines,
    'must have actually clamped (not shown the whole file)',
  );
  assert.equal(
    contFrom,
    shownTo + 1,
    'continuation from must be exactly shownTo + 1',
  );
  // The last shown line must be COMPLETE, not cut mid-line.
  const lastLineRe = new RegExp(
    `^\\s*${shownTo}: line ${shownTo} of ${nLines} with some padding text here$`,
    'm',
  );
  ...
  assert.ok(
    !out.includes(`line ${shownTo + 1} of`),
    'must not include any part of the first unshown line',
  );
});
...
test('formatRead: a {from,to} range that overflows clamps within the range — marker points at the first unshown line of the REQUESTED range', () => {
  ...
  const content = Array.from(
    { length: nLines },
    (_, i) => `row-${i + 1}-payload-text-here`,
  ).join('\n');
  const out = formatRead('rows.txt', content, { from: 101, to: 250 }, 400);
  const m =
    /showing lines 101–(\d+) of 300 — continue: read\('rows\.txt', \{from: (\d+)\}\)/.exec(
      out,
    );
  assert.ok(
    m,
    `expected a clamped marker within the requested range, got tail: ${out.slice(-200)}`,
  );
  const shownTo = Number(m[1]);
  assert.ok(
    shownTo >= 101 && shownTo < 250,
    'clamp must land inside the requested range, before its end',
  );
...
test('formatRead: raw-byte accounting — backslash-heavy content no longer pays a JSON-escape tax (raw render), so LF and escape-heavy lines clamp alike', () => {
  ...
  const plain = formatRead(
    'f.txt',
    makeContent('line payload text aa'),
    {},
    budget,
  );
  const escapey = formatRead(
    'f.txt',
    makeContent('line \\"payload\\" \\\\a'),
    {},
    budget,
  );
...
test('preview: long multiline string elides the middle with a raw cursor, head/tail unescaped', () => {
  ...
  assert.match(
    out,
    /\[middle elided — _\.slice\(\d+, \d+\) for the rest; full value in `_`\]/,
  );
  assert.ok(
    out.includes('payload line with some text\n'),
