...
import { createTranscriptStore, loadMostRecentForChannel, parseTranscriptFile } from '../src/store/sessions.js';
...
test('sessions: append writes a JSON line per message to discord/CHANNEL_ID/', () => {
 ...
 // find the file
  ...
  assert.equal(files.length, 1, 'one transcript file per channel until rotation');
  const raw = fs.readFileSync(path.join(dir, files[0]), 'utf8').trim().split('\n');
...
test('sessions: round-trips tool_calls and tool_call_id', () => {
  ...
  const assistantMsg: ChatMessage = {
    role: 'assistant',
    content: '',
    tool_calls: [{ id: 'tc1', type: 'function', function: { name: 'run', arguments: '{"code":"1+1"}' } }],
  };
  const toolMsg: ChatMessage = { role: 'tool', tool_call_id: 'tc1', content: '{"ok":true,"preview":"2"}' };
...
test('sessions: round-trips reasoning_content on assistant messages', () => {
  const root = tmpRoot();
  const store = createTranscriptStore(root);
  const reasoning = 'I should use elpis.channel().send() to deliver this message to the user, since assistant content is not visible.';
  const assistantMsg: ChatMessage = {
    role: 'assistant',
    content: '',
  ...
  const a = loaded!.messages[0];
  assert.equal(a.role, 'assistant');
  assert.equal(a.reasoning_content, reasoning, 'reasoning_content must survive the transcript round-trip');
...
test('sessions: channel stamp + sends round-trip (V1 whitelist, review N6)', () => {
  ...
  const tool: ChatMessage = {
    role: 'tool', tool_call_id: 't1', content: '[run ok]', channel: 'internal',
...
test('sessions: rotate with sentinel writes an empty newest file (clear honored)', () => {
 ...
 // The newest file is now empty → boot loads nothing (does not resurrect).
  assert.equal(loadMostRecentForChannel(root, 'main'), null, 'clear sentinel prevents resurrection');
});
...
test('sessions: adopt continues an existing file so restart-resume keeps prior context', () => {
 ...
 // The next boot must see ALL THREE messages in one file.
  ...
  assert.ok(reloaded);
  assert.equal(reloaded!.messages.length, 3, 'adopted file carries prior + new messages');
  assert.equal(reloaded!.messages[2].content, 'second-boot message');
 // exactly one transcript file exists (no stranded fresh file)
  const files = fs.readdirSync(path.join(root, 'discord', 'main')).filter((f) => f.endsWith('.jsonl'));
