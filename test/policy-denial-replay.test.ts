...
function fakeStore() {
  return {
    location: 'fake',
    read: () => ({ access: 'fresh', refresh: 'r', expires: Date.now() + 60_000, accountId: 'fresh-account' }),
...
function bundle() {
  const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'elpis-policy-replay-'));
  const config = makeConfig({ paths: { ...makeConfig().paths, dataDirectory } });
  const body = Buffer.from('{"model":"gpt-5.6-sol","input":[{"role":"user","content":"harmless"}]}');
  const record = recordPolicyDenial(config, 'codex-responses', {
...
test('policy denial replay resends exact body/session with fresh auth and appends result', async () => {
  ...
  const fetchFn = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    return new Response('{"error":{"message":"still flagged for usage policy"}}', { status: 400, headers: { 'content-type': 'application/json' } });
  }) as typeof fetch;
  const result = await replayPolicyDenial(config, fakeStore(), record.directory, fetchFn);
  ...
  assert.equal(JSON.parse(fs.readFileSync(result.resultPath, 'utf8')).preservedSessionId, 'original-session');
  assert.equal(fs.existsSync(path.join(record.directory, 'manifest.json')), true);
});

test('policy denial replay refuses a tampered request before network access', async () => {
  const { config, record } = bundle();
  fs.appendFileSync(path.join(record.directory, 'request-body.bin'), 'tampered');
  let networkCalls = 0;
  await assert.rejects(() => replayPolicyDenial(config, fakeStore(), record.directory, (async () => {
    networkCalls++;
    return new Response();
  }) as typeof fetch), /hash mismatch/);
