...
import { aliasCandidates, BUNDLED_ALIAS_WORDLISTS, loadAliasWordlists } from '../src/sandbox/wordlists.js';
import { resolveDataLayout } from '../src/store/data-layout.js';

function directory(): string { return fs.mkdtempSync(path.join(os.tmpdir(), 'sandbox-words-')); }
const logger = () => {
  const warnings: string[] = [];
  return { warnings, value: { warn: (message: string) => warnings.push(message) } };
};
...
test('missing wordlists seed atomically under elpis-data/config and hot-reload authored words', () => {
  ...
  for (const file of ['adverbs.txt', 'adjectives.txt', 'nouns.txt']) {
    assert.equal(fs.statSync(path.join(wordlistDirectory, file)).mode & 0o777, 0o600);
  }
  ...
  assert.deepEqual(reloaded, { adverbs: ['suspiciously'], adjectives: ['saucy'], nouns: ['blahaj'] });
...
test('invalid authored files remain byte-identical while bundled fallback is used', () => {
  ...
  const file = path.join(resolveDataLayout(dataDirectory).wordlists, 'adjectives.txt');
  const invalid = Buffer.from('saucy\nNOT VALID\n');
  fs.writeFileSync(file, invalid);
  const log = logger();
  const loaded = loadAliasWordlists(dataDirectory, log.value as any);
  assert.deepEqual(fs.readFileSync(file), invalid, 'invalid authored content is preserved');
...
test('candidate walk visits the complete product once from a randomized rotation', () => {
  const lists = { adverbs: ['softly', 'boldly'], adjectives: ['saucy', 'tiny'], nouns: ['blahaj', 'moth'] };
