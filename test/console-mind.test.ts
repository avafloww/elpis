...
import { ConsoleHub, type HubClient, type HubSources } from '../src/console/hub.js';
...
function setup() {
  ...
  const scheduler = {
    create(opts: any) { const row = { id: next++, ...opts }; tasks.set(row.id, row); return row; },
  ...
  const mind = new MindService({ db, scheduler, logger: makeConfig().logger, onChanged: () => hub.mindChanged() });
  const sources: HubSources = {
    usage: () => ({ tokens: 0, maxTokens: 1, triggerTokens: 1, byRoom: [], cache: null } as any),
    rooms: () => [], participants: () => 0, meta: () => ({ gitHash: 'x', treeClean: true, uptimeMs: 1, model: 'test', botTag: 'aster' }),
    archived: () => [], subUsage: () => null, mind,
  };
  hub.attach(sources);
...
test('console mind mutations broadcast snapshots and return affected records', async () => {
  ...
  hub.handleClientMessage(client, JSON.stringify({ t: 'mind', op: 'create', reqId: 1, item: { title: 'build the pane', tags: ['console'] } }));
  const result = client.frames.find((x) => x.t === 'mindResult');
  assert.equal(result.ok, true);
  assert.equal(result.result.title, 'build the pane');
  assert.ok(client.frames.some((x) => x.t === 'mindSnapshot' && x.items.length === 1));

  client.frames.length = 0;
  hub.handleClientMessage(client, JSON.stringify({ t: 'mind', op: 'get', reqId: 2, id: result.result.id }));
...
test('console supports comments, links, reminders and teachable failures', async () => {
  ...
  hub.handleClientMessage(client, JSON.stringify({ t: 'mind', op: 'link', reqId: 1, id: second.id, dependsOn: first.id }));
  assert.equal(client.frames.at(-1).ok, true);
  hub.handleClientMessage(client, JSON.stringify({ t: 'mind', op: 'comment', reqId: 2, id: second.id, body: 'waiting on the foundation' }));
  assert.equal(client.frames.at(-1).ok, true);
  hub.handleClientMessage(client, JSON.stringify({ t: 'mind', op: 'remind', reqId: 3, id: second.id, at: Date.now() + 60_000 }));
  assert.equal(client.frames.at(-1).ok, true);
  assert.equal(tasks.size, 1);

  hub.handleClientMessage(client, JSON.stringify({ t: 'mind', op: 'link', reqId: 4, id: first.id, dependsOn: second.id }));
  const failure = client.frames.at(-1);
  assert.equal(failure.ok, false);
  assert.match(failure.error, /cycle/);
  assert.equal(mind.get(second.id)!.comments[0].body, 'waiting on the foundation');
  db.close();
});

test('console mind unavailable and malformed operations do not throw', async () => {
  const hub = new ConsoleHub();
  hub.attach({ usage: () => ({} as any), rooms: () => [], participants: () => 0, meta: () => ({} as any), archived: () => [], subUsage: () => null });
  const client = new Client();
  await hub.addClient(client);
  assert.doesNotThrow(() => hub.handleClientMessage(client, JSON.stringify({ t: 'mind', op: 'snapshot', reqId: 9 })));
