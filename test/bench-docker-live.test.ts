...
import {
  AMBIENT_BATCH_TEST_SCENARIO,
  HEARTBEAT_EFFECT_TEST_SCENARIO,
  HEARTBEAT_NOOP_TEST_SCENARIO,
  HEARTBEAT_WAIT_TEST_SCENARIO,
  RESTART_TEST_SCENARIO,
  SEEDED_HEARTBEAT_TEST_SCENARIO,
...
test(
  'live Docker boundary denies network/root writes/capabilities and applies deterministic time',
  { skip: !live },
  () => {
...
test(
  'live oracle episode keeps private state at 0700/0600 and the exact nested gitignore at 0644',
  { skip: !live },
  async () => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), 'elpisbench-private-live-'),
    );
    ...
    try {
      const record = await runScenario(config, scenario, 'oracle', {
        oracle: true,
      });
      ...
      const db = new DatabaseSync(resolveDataLayout(work).database, {
        readOnly: true,
      });
      try {
        ...
        const items = db
          .prepare(
            'SELECT title, status, parent_id, due_at FROM mind_items ORDER BY id',
          )
          .all() as Array<{
          title: string;
          status: string;
        ...
        assert.deepEqual(
          items.map((item) => [item.title, item.status]),
          [
            ['Engine workspace', 'in_progress'],
            ['Seeded project', 'in_progress'],
            ['Finished prerequisite', 'done'],
            ['Ready leaf', 'open'],
            ['Blocked leaf', 'open'],
          ],
        );
        ...
        const dependencies = db
          .prepare(
            'SELECT item_id, depends_on_id FROM mind_dependencies ORDER BY item_id',
          )
          .all()
          .map((row) => ({
            item_id: Number((row as { item_id: number }).item_id),
            depends_on_id: Number(
              (row as { depends_on_id: number }).depends_on_id,
            ),
          }));
        assert.deepEqual(dependencies, [
          { item_id: 4, depends_on_id: 3 },
          { item_id: 5, depends_on_id: 4 },
        ]);
        const scheduledRow = db
          .prepare(
            'SELECT name, channel_id, payload, next_run_at FROM scheduled_tasks',
          )
          .get() as {
          name: string;
          channel_id: string;
        ...
        const scheduled = {
          name: String(scheduledRow.name),
          channel_id: String(scheduledRow.channel_id),
          payload: String(scheduledRow.payload),
          next_run_at: Number(scheduledRow.next_run_at),
        };
        assert.deepEqual(scheduled, {
          name: 'seeded-future-task',
          channel_id: '101',
          payload: '[seeded future wake]',
          next_run_at: Date.parse('2026-01-02T03:05:05.000Z'),
        });
      } finally {
        db.close();
      }
      const sessionDir = path.join(
        resolveDataLayout(work).sessions,
        'discord',
        'main',
      );
      assert.ok(
        fs.readdirSync(sessionDir).some((name) => name.endsWith('.jsonl')),
      );
      const runtimeConfig = parseYaml(
        fs.readFileSync(path.join(results, 'runtime-config.yaml'), 'utf8'),
      ) as {
      ...
      assert.equal(
        ...
        createHash('sha256')
          .update(fs.readFileSync(path.join(results, 'runtime-config.yaml')))
          .digest('hex'),
      );
      assert.match(record.provenance.dataSnapshotDigest, /^[a-f0-9]{64}$/);
      assert.ok(record.provenance.dbSchemaVersion >= 11);
      assert.equal(
        record.provenance.toolContractVersion,
        TOOL_CONTRACT_VERSION,
      );
      assert.equal(
        record.provenance.promptDigest,
        record.provenance.promptDigests[0],
      );
      ...
      assert.equal(
        record.provenance.adapterVersions.discord,
        'deterministic-discord-v1',
      );
      const leakedCandidateFiles: string[] = [];
      const hiddenMarkers = [scenario.title, scenario.expected.outcome].map(
      ...
      const scanCandidateFiles = (dir: string) => {
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
          ...
          if (entry.isDirectory()) scanCandidateFiles(file);
            ...
            if (hiddenMarkers.some((marker) => bytes.indexOf(marker) >= 0))
              leakedCandidateFiles.push(path.relative(work, file));
          }
        }
      };
      scanCandidateFiles(work);
      assert.deepEqual(leakedCandidateFiles, []);
      assert.deepEqual(fs.readdirSync(results).sort(), [
        'record.json',
        'runtime-config.yaml',
      ]);
      const containerSource = fs.readFileSync(
        path.join(process.cwd(), 'bench', 'container-main.ts'),
        'utf8',
      );
      ...
      const walk = (dir: string) => {
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
          ...
          if (entry.isDirectory()) {
            if (mode !== 0o700) wrong.push(`${mode.toString(8)} ${file}`);
            walk(file);
          } else {
            const expectedMode = file === publicGitignore ? 0o644 : 0o600;
            if (mode !== expectedMode)
              wrong.push(`${mode.toString(8)} ${file}`);
          }
        }
      };
      walk(dataDirectory);
...
test(
  'live production runtime drains ambient Discord and its harness wake into one dispatch',
  { skip: !live },
  async () => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), 'elpisbench-ingress-batch-live-'),
    );
    const dataDirectory = path.join(root, 'data');
    const provider = {
      provider_type: 'openai-compatible' as const,
    ...
    try {
      ...
      assert.match(
        transcript,
        /guild=\\"workspace\\" channel=\\"lounge\\" author=\\"person\\" bot=\\"false\\" time=\\"2026-01-02T03:04:05\.000Z\\"/,
      );
...
test(
  'live production runtime preserves state across a fresh container restart',
  { skip: !live },
  async () => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), 'elpisbench-restart-live-'),
    );
    const dataDirectory = path.join(root, 'data');
    const provider = {
      provider_type: 'openai-compatible' as const,
    ...
    try {
      ...
      assert.equal(Object.values(record.gates).every(Boolean), true);
      const restarts = record.events.filter(
      ...
      assert.equal(
        fs.readFileSync(path.join(work, 'stage-one.txt'), 'utf8'),
        'stage one\n',
      );
      assert.equal(
        fs.readFileSync(path.join(work, 'stage-two.txt'), 'utf8'),
        'stage two\n',
      );
      assert.equal(fs.existsSync(resolveDataLayout(work).database), true);
      const sessionDir = path.join(
...
test(
  'live oracle distinguishes heartbeat effect, wait, and no-op decisions',
  { skip: !live },
  async () => {
    for (const scenario of [
      ...
      const root = fs.mkdtempSync(
        path.join(os.tmpdir(), 'elpisbench-heartbeat-live-'),
      );
      const dataDirectory = path.join(root, 'data');
      const provider = {
        provider_type: 'openai-compatible' as const,
      ...
      try {
        ...
        assert.equal(
          wake?.kind,
          scenario.expected.decision === 'no-op' ? 'auto' : 'after',
          scenario.id,
        );
        const [episode] = fs.readdirSync(path.join(dataDirectory, 'episodes'));
        const work = path.join(dataDirectory, 'episodes', episode, 'work');
        if (scenario.expected.decision === 'effect')
          assert.equal(
            fs.readFileSync(path.join(work, 'repair.txt'), 'utf8'),
            'fixed\n',
          );
        else {
          assert.equal(record.metrics.sendsPerRun, 0, scenario.id);
          assert.equal(
            fs.readFileSync(path.join(work, 'guard.txt'), 'utf8'),
            'unchanged\n',
          );
        }
      } finally {
...
test(
  'live Docker timeout removes the named container',
  { skip: !live },
  async () => {
    ...
    await assert.rejects(
      withContainerTimeout(child, name, new Promise<never>(() => {}), 500),
      /episode timeout/,
    );
    assert.throws(() =>
      execFileSync('docker', ['container', 'inspect', name], {
        stdio: 'ignore',
      }),
    );
  },
