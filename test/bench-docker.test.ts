...
test('Docker episodes deny network/capabilities and mount only explicit episode paths', () => {
  ...
  assert.match(joined, /dst=\/home\/agent\/data(?:\s|$)/);
  assert.match(joined, /dst=\/run\/elpis-clock,readonly/);
  assert.doesNotMatch(joined, /\/episode\//);
  fs.rmSync(root, { recursive: true, force: true });
});

test('clock advancement is deterministic and bounded', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'bench-clock-')),
    clock = path.join(root, 'clock');
  prepareEpisodeMounts(
    path.join(root, 'w'),
    path.join(root, 'r'),
  ...
  advanceClockFile(clock, 90_000);
  assert.equal(fs.readFileSync(clock, 'utf8'), '@2026-01-01 00:01:30\n');
  assert.throws(() => advanceClockFile(clock, 15 * 86400000), /14 days/);
  fs.rmSync(root, { recursive: true, force: true });
});
