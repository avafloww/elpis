...
test('loop: a fully-leaked response is retried, then the clean reply enters history', async () => {
  ...
  const { promise: cleanGenerated, resolve: signalClean } =
  ...
  const assistants = agent.messagesForTest.filter(
    (m) => m.role === 'assistant',
  );
  assert.equal(
    assistants.length,
    1,
    'only the clean assistant message enters history',
  );
  agent.stop();
});
...
test('loop: after MAX_LEAK_RETRIES the loop surfaces a notice and ends the turn', async () => {
  ...
  const { promise: noticeSent, resolve: signalNotice } =
    Promise.withResolvers<void>();
  ...
  assert.equal(
    llm.calls,
    3,
    'should stop after the retry cap, not loop forever',
  );
  ...
  assert.equal(
    ...
    0,
    'triggering user message should be dropped',
  );
  agent.stop();
});
