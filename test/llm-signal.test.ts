...
test('llm: signal is passed as a request option, not in the API body', async () => {
 ...
 // Spy on the client's create: capture (body, options) separately.
  ...
  (llm.client.chat.completions as any).create = async (body: any, opts?: any) => {
    capturedBody = body;
    capturedOpts = opts;
 ...
 // The body (API payload) must NOT contain `signal`.
  ...
  assert.ok(!('signal' in capturedBody),
 ...
 // The signal must be in the request options (second argument).
  ...
  assert.ok('signal' in capturedOpts,
    'signal must be in the request options (SDK second arg) so it controls fetch, not the API body');
  assert.equal(typeof (capturedOpts as any).signal?.addEventListener, 'function',
...
test('llm: responses stream aborts when no meaningful progress arrives before the idle timeout', async () => {
  ...
  const config = makeConfig({ ...base, llm: { ...base.llm, streamIdleTimeoutMs: 20 } });
  const llm = createLLM(config);
  ...
  assert.equal(aborted, true, 'idle watchdog must abort the underlying request');
  assert.ok(Date.now() - started < 1000, 'idle timeout must not inherit the 20-minute SDK timeout');
  (llm.client.responses as any).create = origCreate;
});

test('llm: non-progress SSE keepalives cannot postpone the idle timeout', async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'harness-llm-idle-keepalive-'));
  ensureFile(path.join(tmpDir, 'SOUL.md'), '# Soul');
  ensureFile(path.join(tmpDir, 'MEMORY.md'), '# Memory');
  const base = stubConfig(tmpDir, 'responses');
  const config = makeConfig({ ...base, llm: { ...base.llm, streamIdleTimeoutMs: 35 } });
  const llm = createLLM(config);
  const origCreate = llm.client.responses.create;
  ...
  assert.ok(Date.now() - started < 500, 'keepalives must not extend the meaningful-progress deadline');
  (llm.client.responses as any).create = origCreate;
});

test('llm: responses stream idle timeout resets on each meaningful SSE event', async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'harness-llm-idle-reset-'));
  ensureFile(path.join(tmpDir, 'SOUL.md'), '# Soul');
  ensureFile(path.join(tmpDir, 'MEMORY.md'), '# Memory');
  const base = stubConfig(tmpDir, 'responses');
  const config = makeConfig({ ...base, llm: { ...base.llm, streamIdleTimeoutMs: 35 } });
  ...
  await llm.complete([{ role: 'system', content: 'sys' }, { role: 'user', content: 'hi' }]);
  (llm.client.responses as any).create = origCreate;
});

test('llm: responses path passes signal as a request option, not in the API body', async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'harness-llm-sig-resp-'));
  ensureFile(path.join(tmpDir, 'SOUL.md'), '# Soul');
  ensureFile(path.join(tmpDir, 'MEMORY.md'), '# Memory');
  ...
  assert.equal(capturedBody.store, false, 'stateless mode: store must be false');
  assert.ok(Array.isArray(capturedBody.include) && capturedBody.include.includes('reasoning.encrypted_content'),
    'encrypted reasoning must be requested via include');
  assert.ok(capturedOpts && 'signal' in capturedOpts,
