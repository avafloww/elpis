...
const tmp = (prefix: string) =>
  fs.mkdtempSync(path.join(os.tmpdir(), `harness-${prefix}-`));
...
test('changelog: a backdated entry (sorts before seen ones) is still delivered', () => {
  ...
  fs.writeFileSync(
    path.join(root, 'changelogs', '2026-07-10-backdated.md'),
    'y',
  );
  assert.deepEqual(readUnseenChangelogs(root, dataDir), [
    '2026-07-10-backdated.md',
  ]);
});

test('changelog: missing changelogs/ dir returns [] without throwing', () => {
  assert.deepEqual(
    readUnseenChangelogs(tmp('changelog-empty-root'), tmp('changelog-data')),
    [],
  );
});
...
test('changelog: notice is a pointer — names every entry and shows the elpis.read() invocation', () => {
  const notice = formatChangelogNotice([
    '2026-07-12-changelog-system.md',
    '2026-07-13-x.md',
  ]);
  ...
  assert.match(
    notice,
    /read\(HARNESS_ROOT \+ '\/changelogs\/2026-07-12-changelog-system\.md'\)/,
  );
});
...
test('changelog: onDelivered fires when the notice enters history, not at enqueue', async () => {
  const { promise: completed, resolve: signalDone } =
  ...
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-changelog-loop-',
  });
  try {
    let delivered = 0;
    agent.notifyHarnessChangelog('[harness updated] test notice', () => {
      delivered++;
    });
    ...
    assert.ok(
      users.some(
        (m) =>
          ...
          m.content.includes('[harness updated] test notice'),
      ),
...
test('changelog: a notice never drained is never marked delivered', () => {
  const { agent, cleanup } = buildTestAgent({
    tmpPrefix: 'harness-changelog-undrained-',
  });
  try {
    let delivered = 0;
    agent.notifyHarnessChangelog('[harness updated] test notice', () => {
