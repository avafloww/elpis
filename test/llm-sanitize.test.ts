...
test('sanitize: passes through a clean assistant message with tool_calls', () => {
  ...
  assert.equal(out.message.tool_calls?.[0].function.arguments, JSON.stringify({ code: '1 + 1' }));
});

test('sanitize: passes through a clean natural reply (no tool_calls)', () => {
  const out = sanitizeAssistantMessage({ content: 'hello there', tool_calls: null });
...
test('sanitize: clears content that is pure leaked CoT (markers only)', () => {
  const out = sanitizeAssistantMessage({
    content: '<|tool_calls_section_begin|><|tool_call_begin|><|tool_call_argument_begin|>',
  });
  assert.equal(out.stripped, true);
...
test('sanitize: drops a tool call whose arguments contain a raw newline (the 400 cause)', () => {
 ...
 // a literal \n inside the JSON string value of "code".
  const leakedArgs = '{"code": "const fs =IRV F8 constants. But what did Bramble see?\n\nActually I should look."}';
 // sanity: this string is NOT valid JSON (raw control char)
  ...
  const out = sanitizeAssistantMessage({
    content: '',
    tool_calls: [
      { id: 'functions.run:82', type: 'function', function: { name: 'run', arguments: leakedArgs } },
    ],
  });
  assert.equal(out.stripped, true);
...
test('sanitize: keeps the valid tool calls and drops only the malformed ones', () => {
  ...
  const out = sanitizeAssistantMessage({
    content: '',
    tool_calls: [
      { id: 'bad', type: 'function', function: { name: 'run', arguments: leakedArgs } },
      { id: 'good', type: 'function', function: { name: 'run', arguments: good } },
    ],
  });
  assert.equal(out.stripped, true);
  assert.equal(out.message.tool_calls?.length, 1);
...
test('sanitize: drops non-function tool calls (type guard)', () => {
  const out = sanitizeAssistantMessage({
    content: '',
    tool_calls: [
      { id: 'x', type: 'code_interpreter', function: { name: 'run', arguments: '{}' } },
    ],
  });
  assert.equal(out.stripped, true);
  assert.equal(out.message.tool_calls, undefined);
...
test('sanitize: the resulting message survives the server\'s nested-JSON parse of arguments', () => {
 ...
 // value round-trips (no raw control chars hiding in nested strings).
  ...
  for (const args of cases) {
    const out = sanitizeAssistantMessage({
      content: '',
      tool_calls: [{ id: 'c', type: 'function', function: { name: 'run', arguments: args } }],
    });
    assert.equal(out.stripped, false, `should keep valid args: ${args}`);
    assert.equal(out.message.tool_calls?.length, 1, `should keep valid args: ${args}`);
 // strict re-parse (what the server does)
    assert.doesNotThrow(() => JSON.parse(out.message.tool_calls![0].function.arguments));
  }
});
...
test('sanitize: think-tag leak with no usable reply triggers stripped + empty (retry path)', () => {
  const CLOSE = '<' + '/think>';
  const out = sanitizeAssistantMessage({ content: `just reasoning, no reply${CLOSE}` });
  assert.equal(out.stripped, true);
  assert.equal(out.message.content, '');
});
...
test('sanitize: reasoning_content is preserved through sanitization', () => {
 ...
 // cross-turn thinking continuity and compactor visibility.
  const reasoning = 'The user wants me to greet them. I should use elpis.channel().send() to deliver a message to Discord, since assistant content is not visible to the user.';
  const out = sanitizeAssistantMessage({
    content: '',
    reasoning_content: reasoning,
    tool_calls: [{ id: 'tc1', type: 'function', function: { name: 'run', arguments: JSON.stringify({ code: 'elpis.channel().send("hi")' }) } }],
  });
  assert.equal(out.stripped, false);
...
test('sanitize: empty/null reasoning_content is not stored', () => {
  const out = sanitizeAssistantMessage({ content: 'hello', reasoning_content: '' });
  assert.equal(out.message.reasoning_content, undefined);
  const out2 = sanitizeAssistantMessage({ content: 'hello', reasoning_content: null });
