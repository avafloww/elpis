...
test('device login initializes, treats 403 as pending, then exchanges server PKCE', async () => {
  ...
  const replies = [
    ...
    new Response(JSON.stringify({ authorization_code: 'auth-code', code_verifier: 'server-verifier' })),
    new Response(JSON.stringify({ access_token: access, refresh_token: 'refresh-1', expires_in: 3600 })),
  ];
  ...
  const creds = await loginOpenAICodexDevice({
    ...
    onCode: (url, code) => { shown = { url, code }; },
  });

  assert.deepEqual(shown, { url: OPENAI_CODEX_DEVICE_AUTH_URL, code: 'ABCD-EFGH' });
  ...
  assert.deepEqual(JSON.parse(String(requests[0].init?.body)), { client_id: OPENAI_CODEX_CLIENT_ID });
...
test('refresh omits unavailable identity fields so OAuthStore preserves login workspace', async () => {
  const fetchFn = (async () =>
    new Response(JSON.stringify({ access_token: 'opaque-access-token', expires_in: 7200 }))) as typeof fetch;
  const creds = await refreshOpenAICodexToken('refresh-old', fetchFn);
...
test('device login has a bounded wait', async () => {
  ...
  const fetchFn = (async () => {
    calls++;
    if (calls === 1) return new Response(JSON.stringify({ device_auth_id: 'd', user_code: 'U', interval: 5 }));
    return new Response('pending', { status: 404 });
  }) as typeof fetch;
  await assert.rejects(
    () => loginOpenAICodexDevice({ fetchFn, sleepFn: async () => {}, maxWaitMs: 1 }),
    /timed out/,
  );
