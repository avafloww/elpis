...
function codexConfig(model = 'gpt-test-codex'): Config {
 ...
 // helpers.ts (which opens node:sqlite transitively under the real test run).
  return {
    llm: {
      providerType: 'codex-oauth', apiKey: '', baseUrl: 'https://chatgpt.com/backend-api', model,
...
function fakeStore() {
  ...
  const store = {
    ...
    read: () => ({ access: token, refresh: 'r', expires: Date.now() + 60_000, accountId: 'acct-1' }),
    getAccessToken: async () => token,
    forceRefresh: async () => { refreshes++; token = 'access-2'; },
...
test('Codex fetch injects subscription headers and retries one 401 after refresh', async () => {
  ...
  const authenticated = createCodexFetch(store, () => 'session-1', fetchFn, true);
  const response = await authenticated('https://chatgpt.com/backend-api/codex/responses', {
    method: 'POST',
  ...
  assert.deepEqual(redirects, ['error', 'error'], 'bearer requests must never follow redirects');
});
...
test('Codex fetch refuses to expose the OAuth token to another host or path', async () => {
  ...
  await assert.rejects(() => authenticated('https://example.com/backend-api/codex/responses'), /refusing/);
  await assert.rejects(() => authenticated('https://chatgpt.com/evil'), /refusing/);
...
test('Codex model discovery uses authenticated primary route and reported window', async () => {
  ...
  const fetchFn = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify({ models: [{ slug: 'gpt-test-codex', context_window: 300000 }] }));
  }) as typeof fetch;
...
test('Codex discovery uses the known 5.6 fallback when upstream omits context_window', async () => {
  const { store } = fakeStore();
  const fetchFn = (async () =>
    new Response(JSON.stringify({ data: [{ id: 'gpt-5.6-sol' }] }))) as typeof fetch;
...
test('GPT-5.6 Codex uses Responses Lite request shaping', () => {
  ...
  const shaped = shapeCodexResponsesLiteRequest({
    model: 'gpt-5.6-sol',
    input: [{ role: 'system', content: 'instructions' }, { role: 'user', content: 'hello' }],
    tools: [{ type: 'function', name: 'run' }],
    reasoning: { effort: 'high' },
    tool_choice: 'required',
  });
  assert.equal('tools' in shaped, false);
  assert.equal((shaped.input as Array<Record<string, unknown>>)[0].type, 'additional_tools');
  assert.equal((shaped.input as Array<Record<string, unknown>>)[1].role, 'developer');
  ...
  const external = shapeCodexResponsesLiteRequest({
    model: 'gpt-5.6-sol',
    input: [{ role: 'system', content: 'instructions' }, { role: 'user', content: 'hello' }],
    tools: [{ type: 'function', name: 'run' }, { type: 'function', name: 'think' }],
    reasoning: { effort: 'none' },
    tool_choice: { type: 'function', name: 'think' },
  });
  const toolItem = (external.input as Array<Record<string, unknown>>)[0];
  assert.deepEqual((toolItem.tools as Array<Record<string, unknown>>).map((tool) => tool.name), ['run', 'think']);
  assert.deepEqual(external.reasoning, { effort: 'none', context: 'all_turns' });
  assert.deepEqual(external.tool_choice, { type: 'function', name: 'think' });
});

async function captureOutboundSummary(model: string, foldText: string): Promise<Record<string, unknown>> {
  const { store } = fakeStore();
  const llm = createCodexOAuthLLM(codexConfig(model), store);
  let body: Record<string, unknown> | undefined;
  (llm.client!.responses as any).create = async (request: Record<string, unknown>) => {
    body = request;
    return {
...
test('GPT-5.6 Lite summary emits developer/user input with no turn tools', async () => {
  ...
  assert.equal(
    (body.input as Array<Record<string, unknown>>).some((item) => item.type === 'additional_tools'),
    false,
  );
  assert.deepEqual(body.reasoning, { effort: 'high', context: 'all_turns' });
  assert.match((body.input as Array<{ content: string }>)[0].content, /first person as a note to your future self/);
});
...
test('Lite standalone request carries image input without inheriting turn tools', () => {
  const body = buildCodexStandaloneRequest(codexConfig('gpt-5.6-sol'), [
  ...
  ], 'motor-lane', true) as unknown as Record<string, unknown>;
  ...
  assert.deepEqual(content[1], { type: 'input_image', image_url: 'data:image/png;base64,aGVsbG8=' });
  const replay = buildCodexStandaloneRequest(codexConfig('gpt-5.6-sol'), [
    {
      role: 'assistant', content: '{"keys":["Up"]}',
      reasoning_items: [{ id: 'rs_old', type: 'reasoning', status: 'completed', summary: [], encrypted_content: 'old-opaque' }],
    },
    { role: 'user', content: 'next frame' },
  ], 'motor-lane', true) as unknown as Record<string, unknown>;
  const replayInput = replay.input as Array<Record<string, unknown>>;
  assert.deepEqual(replayInput[0], { type: 'reasoning', summary: [], encrypted_content: 'old-opaque' });
  assert.equal(replayInput[1].role, 'assistant');
  assert.equal(replayInput[2].role, 'user');
  assert.throws(
    () => buildCodexStandaloneRequest(codexConfig(), [{ role: 'tool', content: 'no', tool_call_id: 'x' }], 'lane', false),
...
test('completeStandalone uses its lane key for authenticated transport and request cache identity', async () => {
    ...
    const events = [
      { type: 'response.output_text.delta', delta: '{"keys":["Up"]}' },
      {
        ...
        response: {
          id: 'resp_motor',
          output: [
            { id: 'rs_motor', type: 'reasoning', status: 'completed', summary: [], encrypted_content: 'opaque-motor-state' },
            { type: 'message', content: [{ type: 'output_text', text: '{"keys":["Up"]}' }] },
          ],
  ...
  const llm = createCodexOAuthLLM(codexConfig('gpt-5.6-sol'), store, undefined, fetchFn);
  const controller = new AbortController();
  const result = await llm.completeStandalone!([{ role: 'user', content: 'choose one action' }], { cacheKey: 'motor-lane', model: 'gpt-5.6-luna', reasoningEffort: 'low', signal: controller.signal });
  ...
  assert.deepEqual(result.usage, { prompt_tokens: 20, completion_tokens: 5, total_tokens: 25 });
  ...
  await assert.rejects(
    llm.completeStandalone!([{ role: 'user', content: 'wrong grammar' }], { model: 'gpt-5.3-codex' }),
...
test('Codex LLM always streams, requires run, sends no output cap, and rotates cache identity on reset', async () => {
  ...
  (llm.client!.responses as any).create = async (body: Record<string, unknown>) => {
    bodies.push(body);
    return {
      async *[Symbol.asyncIterator]() {
        yield {
...
test('Codex replay strips response-only reasoning id/status without mutating history', () => {
  ...
  const messages = [{ role: 'assistant' as const, content: 'done', reasoning_items: [nativeItem] }];
  ...
  assert.equal(messages[0].reasoning_items[0].status, 'completed', 'stored history must remain native/verbatim');
  ...
  await llm.complete([
    {
      ...
      reasoning_items: [{
        id: 'rs_1', type: 'reasoning', status: 'completed', summary: [], encrypted_content: 'blob',
      }],
    },
    { role: 'user', content: 'continue' },
  ]);
  const reasoning = (body?.input as Array<Record<string, unknown>>).find((item) => item.type === 'reasoning');
  assert.deepEqual(reasoning, { type: 'reasoning', summary: [], encrypted_content: 'blob' });
});
    ...
    return {
      async *[Symbol.asyncIterator]() {
        yield {
          ...
          output_index: 0,
          item: {
            type: 'function_call', call_id: 'call_run', name: 'run',
...
test('Codex fetch seals exact wire request and flagged response without consuming it', async () => {
  const { store } = fakeStore();
  const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'elpis-codex-flight-'));
  const config = { ...codexConfig('gpt-5.6-sol'), paths: { dataDirectory, soulPath: '', memoryPath: '', harnessRoot: '' } } as Config;
  const body = '{"model":"gpt-5.6-sol","input":[{"role":"user","content":"harmless"}]}';
  const flagged = '{"error":{"message":"Invalid prompt: your prompt was flagged as potentially violating our usage policy."}}';
  const authenticated = createCodexFetch(store, () => 'captured-session', (async () =>
    new Response(flagged, { status: 400, headers: { 'content-type': 'application/json' } })) as typeof fetch, true, config);
  const response = await authenticated('https://chatgpt.com/backend-api/codex/responses', {
  ...
  assert.equal(fs.readFileSync(path.join(bundle, manifest.request.bodyFile), 'utf8'), body);
  assert.equal(fs.readFileSync(path.join(bundle, manifest.response.bodyFile), 'utf8'), flagged);
...
test('Codex fetch seals a policy denial delivered inside an HTTP 200 SSE stream', async () => {
  const { store } = fakeStore();
  const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'elpis-codex-sse-flight-'));
  const config = { ...codexConfig('gpt-5.6-sol'), paths: { dataDirectory, soulPath: '', memoryPath: '', harnessRoot: '' } } as Config;
  const body = '{"model":"gpt-5.6-sol","input":[{"role":"user","content":"harmless"}]}';
  const flagged = 'event: error\ndata: {"error":{"message":"Invalid prompt: your prompt was flagged as potentially violating our usage policy."}}\n\n';
  const authenticated = createCodexFetch(store, () => 'sse-session', (async () =>
    new Response(flagged, { status: 200, headers: { 'content-type': 'text/event-stream' } })) as typeof fetch, true, config);
  const response = await authenticated('https://chatgpt.com/backend-api/codex/responses', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body,
  });
...
test('Codex fetch does not seal an ordinary successful SSE stream', async () => {
  const { store } = fakeStore();
  const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'elpis-codex-clean-sse-'));
  const config = { ...codexConfig('gpt-5.6-sol'), paths: { dataDirectory, soulPath: '', memoryPath: '', harnessRoot: '' } } as Config;
  const clean = 'event: response.completed\ndata: {"response":{"status":"completed"}}\n\n';
  const authenticated = createCodexFetch(store, () => 'clean-session', (async () =>
    new Response(clean, { status: 200, headers: { 'content-type': 'text/event-stream' } })) as typeof fetch, true, config);
...
test('Codex fetch seals a denial event without waiting for the SSE connection to close', async () => {
  const { store } = fakeStore();
  const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'elpis-codex-open-sse-'));
  const config = { ...codexConfig('gpt-5.6-sol'), paths: { dataDirectory, soulPath: '', memoryPath: '', harnessRoot: '' } } as Config;
  const body = '{"model":"gpt-5.6-sol","input":[]}';
  const flagged = 'event: error\ndata: {"error":{"message":"Invalid prompt: your prompt was flagged as potentially violating our usage policy."}}\n\n';
  const stream = new ReadableStream<Uint8Array>({
    start(controller) { controller.enqueue(new TextEncoder().encode(flagged)); },
  });
  const authenticated = createCodexFetch(store, () => 'open-sse-session', (async () =>
    new Response(stream, { status: 200, headers: { 'content-type': 'text/event-stream' } })) as typeof fetch, true, config);
  const response = await authenticated('https://chatgpt.com/backend-api/codex/responses', { method: 'POST', body });
...
test('Codex fetch seals policy bytes before a final SSE blank-line delimiter', async () => {
  const { store } = fakeStore();
  const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'elpis-codex-undelimited-sse-'));
  const logs: string[] = [];
  ...
  const config = { ...base, logger, paths: { dataDirectory, soulPath: '', memoryPath: '', harnessRoot: '' } } as Config;
  const body = '{"model":"gpt-5.6-sol","input":[]}';
  const flagged = 'event: error\ndata: {"error":{"message":"Invalid prompt: your prompt was flagged as potentially violating our usage policy."}}';
  const stream = new ReadableStream<Uint8Array>({
    start(controller) { controller.enqueue(new TextEncoder().encode(flagged)); },
  });
  const authenticated = createCodexFetch(store, () => 'undelimited-session', (async () =>
    new Response(stream, { status: 200, headers: { 'content-type': 'text/event-stream; charset=utf-8' } })) as typeof fetch, true, config);
  const response = await authenticated('https://chatgpt.com/backend-api/codex/responses', { method: 'POST', body });
  const first = await response.body?.getReader().read();
  assert.equal(new TextDecoder().decode(first?.value), flagged);
  const root = path.join(dataDirectory, 'private', 'policy-denials');
  ...
  assert.equal(manifest.response.bodyComplete, false);
  assert.equal(fs.readFileSync(path.join(bundle, manifest.response.bodyFile), 'utf8'), flagged);
  assert.equal(logs.some((line) => /attached .*status=200/.test(line)), true);
  assert.equal(logs.some((line) => /progress .*policy_text=true .*error_event=true/.test(line)), true);
  assert.equal(logs.some((line) => /matched .*complete_events=0/.test(line)), true);
  assert.equal(logs.some((line) => line.includes(flagged)), false, 'diagnostics must not print response payload');
});

test('Codex fetch monitors a stream request when the response omits Content-Type', async () => {
  const { store } = fakeStore();
  const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'elpis-codex-no-content-type-'));
  const logs: string[] = [];
  const base = codexConfig('gpt-5.6-sol');
  const logger = {
    debug: (...args: unknown[]) => logs.push(args.join(' ')),
  ...
  const authenticated = createCodexFetch(store, () => 'no-content-type-session', (async () =>
    new Response(stream, { status: 200 })) as typeof fetch, true, config);
  const response = await authenticated('https://chatgpt.com/backend-api/codex/responses', { method: 'POST', body });
  const first = await response.body?.getReader().read();
  assert.equal(new TextDecoder().decode(first?.value), flagged);
  const root = path.join(dataDirectory, 'private', 'policy-denials');
  ...
  const manifest = JSON.parse(fs.readFileSync(path.join(bundle, 'manifest.json'), 'utf8'));
  assert.equal(manifest.response.captureTrigger, 'stream-policy-bytes');
  assert.equal(logs.some((line) => /transport .*request_stream=true .*content_type_sse=false .*content_type_present=false/.test(line)), true);
  assert.equal(logs.some((line) => /attached .*request_stream=true .*content_type_sse=false/.test(line)), true);
  assert.equal(logs.some((line) => line.includes(flagged)), false);
});

test('Codex fetch ignores policy words in ordinary output and tool-argument events', async () => {
  const phrases = [
    ['response.output_text.delta', { type: 'response.output_text.delta', delta: 'quoted prompt was flagged as violating our usage policy' }],
    ['response.function_call_arguments.delta', { type: 'response.function_call_arguments.delta', delta: '{"note":"flagged by usage policy"}' }],
  ] as const;
  for (const [event, payload] of phrases) {
    const { store } = fakeStore();
    const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'elpis-codex-policy-quote-'));
    const config = { ...codexConfig('gpt-5.6-sol'), paths: { dataDirectory, soulPath: '', memoryPath: '', harnessRoot: '' } } as Config;
    const streamText = `event: ${event}\ndata: ${JSON.stringify(payload)}\n\nevent: response.completed\ndata: {"type":"response.completed","response":{"status":"completed"}}\n\n`;
    const authenticated = createCodexFetch(store, () => 'quoted-policy-session', (async () =>
      new Response(streamText, { status: 200 })) as typeof fetch, true, config);
    const response = await authenticated('https://chatgpt.com/backend-api/codex/responses', {
...
test('Codex fetch recognizes response.failed policy envelopes', async () => {
  const { store } = fakeStore();
  const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'elpis-codex-failed-policy-'));
  const config = { ...codexConfig('gpt-5.6-sol'), paths: { dataDirectory, soulPath: '', memoryPath: '', harnessRoot: '' } } as Config;
  ...
  const authenticated = createCodexFetch(store, () => 'failed-policy-session', (async () =>
    new Response(failed, { status: 200 })) as typeof fetch, true, config);
  const response = await authenticated('https://chatgpt.com/backend-api/codex/responses', {
    method: 'POST', body: '{"model":"gpt-5.6-sol","stream":true,"input":[]}',
  });
