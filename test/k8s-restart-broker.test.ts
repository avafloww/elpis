...
test('broker lists one fixed-label harness Pod and deletes it with a UID precondition', async () => {
  ...
  const fakeFetch = async (
    ...
    return seen.length === 1
      ? new Response(JSON.stringify({ items: [harnessPod] }), {
          status: 200,
  ...
  assert.equal(
    seen[0]!.url,
    'https://kubernetes.default.svc/api/v1/namespaces/agent-a/pods?labelSelector=app.kubernetes.io%2Fname%3Delpis%2Capp.kubernetes.io%2Fcomponent%3Dharness',
  );
  ...
  assert.equal(
    seen[1]!.url,
    'https://kubernetes.default.svc/api/v1/namespaces/agent-a/pods/elpis-harness-7d9f8d7b58-abcde',
  );
...
test('broker rejects ambiguous, terminating, foreign, and non-Deployment Pods', async () => {
  ...
  await assert.rejects(
    run([
      {
        ...harnessPod,
        metadata: { ...harnessPod.metadata, deletionTimestamp: 'now' },
      },
    ]),
    /already terminating/,
  );
  await assert.rejects(
    ...
    /outside the fixed harness selector/,
  );
  await assert.rejects(
    run([
      {
        ...harnessPod,
        metadata: { ...harnessPod.metadata, ownerReferences: [] },
      },
    ]),
    /not controlled by the fixed/,
  );
});
...
test('broker HTTP surface accepts only one bounded restart shape', async () => {
  ...
  try {
    ...
    assert.equal(
      (await fetch(base + '/v1/restart', { method: 'POST', body: '{}' }))
        .status,
      400,
    );
    ...
    const accepted = await fetch(base + '/v1/restart', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ protocol: 1, at, reason: 'load extension' }),
    });
...
test('broker rejects concurrent recreations and hides backend error details', async () => {
  ...
  const server = createRestartBrokerServer(config, {
    recreateHarnessPod: async () => {
      await blocked;
      throw new Error('private Kubernetes detail');
    },
  });
  await new Promise<void>((resolve) => server.listen(0, '127.0.0.1', resolve));
  const address = server.address();
