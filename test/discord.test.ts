...
test('fetchContextWindow: resolves configured model context_window', { skip: NO_NETWORK || NO_CONFIG }, async () => {
  const config = loadConfigFile();
  const cw = await fetchContextWindow({ ...config, llm: { ...config.llm, contextSize: null } });
...
test('fetchContextWindow: throws clearly for unknown model', { skip: NO_NETWORK || NO_CONFIG }, async () => {
  const config = loadConfigFile();
  const bad = { ...config, llm: { ...config.llm, contextSize: null, model: 'nonexistent-model-xyz' } };
