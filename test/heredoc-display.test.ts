...
import {
  ...
  restoreDisplayHeredocs,
} from '../src/lib/heredoc-display.js';
...
test('display heredoc protection preserves exact bodies and same-line trailers through Prettier', async () => {
  ...
  const formatted = await prettier.format(protectedCode.code, {
    parser: 'babel',
...
test('display heredoc protection fails closed on missing terminators', () => {
  ...
  assert.deepEqual(protectDisplayHeredocs(raw), {
    code: raw,
    heredocs: [],
    error: 'heredoc <<<NOPE has no terminator',
  });
});
