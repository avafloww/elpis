...
import { detectRuntimeProfile, resolveBuiltinModules, RESTRICTED_SENTINEL } from '../src/builtin-modules.js';
import { makeConfig } from './helpers.js';

test('restricted sentinel is non-bypassable by an unset or false environment variable', () => {
  const profile = detectRuntimeProfile({ env: { ELPIS_RESTRICTED: '0' }, exists: (path) => path === RESTRICTED_SENTINEL });
...
test('restricted mode can also be explicitly enabled by environment', () => {
  assert.deepEqual(detectRuntimeProfile({ env: { ELPIS_RESTRICTED: 'true' }, exists: () => false }), { restricted: true, source: 'environment' });
  assert.deepEqual(detectRuntimeProfile({ env: {}, exists: () => false }), { restricted: false, source: 'normal' });
});
...
test('built-in modules resolve from config and credentials with dependency reasons', () => {
  ...
  assert.match(modules.reason('motor') ?? '', /requires an active computer module/);

  const none = resolveBuiltinModules(makeConfig({ modules: { enabled: [], disabled: [] } }));
  assert.equal(none.statuses.every((status) => status.state === 'disabled'), true);

  const denyOne = resolveBuiltinModules(makeConfig({ modules: { enabled: null, disabled: ['browser'] } }));
  assert.equal(denyOne.state('browser'), 'disabled');
  assert.equal(denyOne.state('computer'), 'active');

  const restricted = resolveBuiltinModules(makeConfig({
    modules: { enabled: ['browser', 'computer', 'motor', 'fleet'], disabled: [] },
  ...
  assert.equal(restricted.state('fleet'), 'unavailable');

  const configured = resolveBuiltinModules(makeConfig({
    kagi: { apiKey: 'k' },
    bluesky: { service: 'https://example.invalid', identifier: 'agent.test', appPassword: 'pw' },
  }));
