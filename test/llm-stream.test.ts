...
function countingStream(chunks: Chunk[]): {
          ...
          tool_calls: [
            {
              ...
              id,
              type: 'function',
              function: { name: 'run', arguments: argsFragment },
            },
          ],
        },
      },
...
test('streamComplete: a 5xx throw is classified RetriableError', async () => {
  const client = mockClient({
    throwValue: { status: 503, message: 'unavailable' },
  });
  await assert.rejects(
    () =>
      streamComplete(client, stubConfig(os.tmpdir()), [
...
test('streamComplete: a 400 throw is classified NonRetriableError', async () => {
  const client = mockClient({
    throwValue: { status: 400, message: 'bad request' },
  });
  await assert.rejects(
    () =>
      streamComplete(client, stubConfig(os.tmpdir()), [
...
test('streamComplete: signal goes to the options arg, not the request body params', async () => {
  ...
  await streamComplete(client, stubConfig(os.tmpdir()), [
    { role: 'user', content: 'hi' },
  ]);
  assert.ok(
    !('signal' in seenParams),
    'params body must NOT carry a signal key',
  );
  assert.ok(
    seenOptions && typeof seenOptions === 'object' && 'signal' in seenOptions,
    'the SDK options (second arg) must carry the abort signal',
  );
  assert.ok(
    typeof seenOptions.signal.aborted === 'boolean',
