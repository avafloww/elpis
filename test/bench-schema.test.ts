...
import {
  resolveCandidateIngress,
  resolveCandidateIngressBatch,
} from '../bench/ingress.js';
...
const base = {
  ...
  maxWallMs: 1000,
  fixture: {
    channels: { general: '100', parent: '101' },
    files: {},
    directories: [],
    heartbeat: false,
  ...
  expected: {
    outcome: 'host-only-outcome-marker',
    workPaths: [],
...
test('production scenarios require explicit candidate ingress', () => {
  assert.throws(
    () => parseScenario({ ...base, track: 'production' }),
...
test('heartbeat decisions default compatibly and preserve explicit wait', () => {
  ...
  assert.equal(
    parseScenario({
      ...base,
      expected: { ...base.expected, action: 'required' },
    }).expected.decision,
    'effect',
  );
  assert.equal(
    parseScenario({ ...base, expected: { ...base.expected, decision: 'wait' } })
      .expected.decision,
    'wait',
  );
});
...
test('production ingress requires a deterministic clock', () => {
  ...
  assert.throws(
    ...
    /production ingress requires deterministic clockAt/,
  );
});
...
test('full Discord ingress and its harness wake resolve as one ordered production batch', () => {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(work, 'attachments', 'frame.png'),
      Buffer.from([1, 2, 3, 4]),
    );
    const scenario = parseScenario({
      ...base,
      ...
      ingressBatch: [
        {
          ...
          attachments: [
            {
              path: 'attachments/frame.png',
              url: 'https://cdn.invalid/frame.png',
        ...
        },
        {
          ...
          id: 'tick-1',
          atOffsetMs: 60_000,
          content: '[room context — 1 message]',
      ...
      attachments: [
        {
          ...
          name: 'frame.png',
          contentType: 'image/png',
          localPath: path.join(work, 'attachments', 'frame.png'),
          size: 4,
        },
      ],
    ...
    assert.equal(
      formatInboundEnvelope(batch[0], '[00:00]'),
      [
...
test('scheduler and restart ingress preserve their production channels and phase timestamps', () => {
  const scenario = parseScenario({
    ...base,
    track: 'production',
    fixture: { ...base.fixture, restartAtDispatch: 1 },
    ingress: {
      kind: 'scheduler',
      id: 'scheduled-1',
      channel: 'general',
      content: '[scheduled task] review',
    },
    resumeIngress: {
      kind: 'harness',
      id: 'resume-1',
      atOffsetMs: 5_000,
      content: '[restart complete] continue',
    },
  });
  ...
  assert.throws(
    () =>
      parseScenario({
        ...
        track: 'production',
        ingress: { kind: 'heartbeat' },
        resumeIngress: { kind: 'harness', content: 'unused' },
      }),
...
test('sandbox fixture seeds reject unknown, closed, and duplicate Mind bindings', () => {
  ...
  assert.throws(
    () => parseScenario(scenario([{ mindKey: 'missing' }])),
    /unknown Mind seed key missing/,
  );
  assert.throws(
    () => parseScenario(scenario([{ mindKey: 'closed' }])),
    /sandbox Mind seed closed is closed/,
  );
  assert.throws(
    () =>
      parseScenario(
        scenario([{ mindKey: 'workspace' }, { mindKey: 'workspace' }]),
      ),
...
test('attachment resolution refuses traversal and symlink escape from the seeded world', () => {
  const work = fs.mkdtempSync(
    path.join(os.tmpdir(), 'elpisbench-ingress-root-'),
  );
  const outside = fs.mkdtempSync(
    path.join(os.tmpdir(), 'elpisbench-ingress-outside-'),
  );
  try {
    fs.writeFileSync(path.join(outside, 'secret.txt'), 'outside');
    fs.symlinkSync(
      path.join(outside, 'secret.txt'),
      path.join(work, 'link.txt'),
    );
    const withAttachment = (attachmentPath: string) =>
      parseScenario({
        ...base,
        track: 'production',
        ingress: {
    ...
    assert.throws(
      () =>
        resolveCandidateIngress(withAttachment('../outside.txt'), false, work),
      /escapes work directory/,
    );
    assert.throws(
      () => resolveCandidateIngress(withAttachment('link.txt'), false, work),
      /through symlink/,
    );
  } finally {
    fs.rmSync(work, { recursive: true, force: true });
