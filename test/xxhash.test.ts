...
const hex = (buf: Uint8Array, seed: bigint) =>
  xxh64(buf, seed).toString(16).padStart(16, '0');
...
test('xxh64: canonical vectors, seed 0', () => {
  ...
  assert.equal(
    hex(bytes('The quick brown fox jumps over the lazy dog!!!!'), 0n),
    'a9b4233048596906',
  );
});
...
test('xxh64: seeded vectors (the real cch seed)', () => {
  ...
  assert.equal(
    hex(bytes('harness cch attestation body placeholder cch=00000'), seed),
    '8fe0ad3ded3a9017',
  );
