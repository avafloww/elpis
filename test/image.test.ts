...
test('sniffImageMediaType: recognizes the four vision formats by magic bytes', () => {
  ...
  assert.equal(sniffImageMediaType(Buffer.concat([Buffer.from('RIFF'), Buffer.from([1, 2, 3, 4]), Buffer.from('WEBP')])), 'image/webp');
});

test('sniffImageMediaType: unrecognized or truncated bytes return null', () => {
  assert.equal(sniffImageMediaType(Buffer.from('plain text, not an image')), null);
 ...
 // RIFF container that is NOT webp (e.g. a .wav) must not claim image/webp
  assert.equal(sniffImageMediaType(Buffer.concat([Buffer.from('RIFF'), Buffer.from([1, 2, 3, 4]), Buffer.from('WAVE')])), null);
});
...
test('sniffFileMediaType: reads only the head of a real file; missing file is null', async () => {
  ...
  try {
    ...
    assert.equal(await sniffFileMediaType(path.join(dir, 'does-not-exist.png')), null);
  } finally {
...
test('resolveAttachmentContentType: corrects only within images', () => {
 // the incident shape: declared webp, PNG bytes
  assert.equal(resolveAttachmentContentType('image/webp', 'image/png'), 'image/png');
 // agreement passes through
  assert.equal(resolveAttachmentContentType('image/png', 'image/png'), 'image/png');
 ...
 // non-image declared types are never touched (inline-text policy unchanged)
  assert.equal(resolveAttachmentContentType('text/plain', 'image/png'), 'text/plain');
