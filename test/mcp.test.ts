...
test('Streamable HTTP MCP uses canonical Mind, client provenance, task-bound wake, and session cleanup', async (t) => {
  ...
  const endpoint = createMcpEndpoint({
    mind,
    logger: config.logger,
    wake: (message) => wakes.push(message),
  });
  const port = await freePort();
  const serverConfig = makeConfig({
    console: { enabled: true, mcpEnabled: true, port, host: '127.0.0.1' },
  });
  ...
  assert.match(
    ...
    /structuredContent with compact receipts/,
  );
  ...
  assert.ok(
    tools.tools.reduce(
      (sum, tool) => sum + JSON.stringify(tool.outputSchema).length,
      0,
    ) < 6_000,
    'generic output schemas stay compact',
  );
  const prioritySchema = tools.tools.find(
    (tool) => tool.name === 'mind_create',
  )!.inputSchema.properties!.priority as Record<string, unknown>;
  assert.deepEqual(
    { minimum: prioritySchema.minimum, maximum: prioritySchema.maximum },
    { minimum: 0, maximum: 4 },
  );
  ...
  const createResult = await client.callTool({
    name: 'mind_create',
    arguments: {
      title: 'Repair parser cache invalidation',
      body: 'src/parser/cache.ts loses decoder state',
      tags: ['parser', 'cache'],
    },
  });
  const createEnvelope = envelope(createResult);
  const createText =
    createResult.content.find((part) => part.type === 'text')?.text ?? '';
  assert.ok(
    createText.length < JSON.stringify(createResult.structuredContent).length,
    'text is a compact continuation summary, not duplicated structured data',
  );
  ...
  const discovered = value(
    await client.callTool({
      name: 'mind_discover',
      arguments: {
        context: 'working in src/parser/cache.ts on decoder cache invalidation',
      },
    }),
  );
  ...
  value(
    await client.callTool({
      name: 'mind_claim',
      arguments: { id: lifecycle.id, note: 'reproducing cache loss' },
    }),
  );
  const logged = envelope(
    await client.callTool({
      ...
      arguments: {
        id: lifecycle.id,
        kind: 'decision',
        body: 'invalidate only decoder-local entries',
      },
    }),
  );
  ...
  assert.ok(
    mind
      ...
      .comments.some(
        (entry) =>
          entry.body === 'Decision: invalidate only decoder-local entries',
      ),
  );
  const shortcut = await client.callTool({
    name: 'mind_update',
    arguments: { id: lifecycle.id, status: 'done' },
  });
  ...
  assert.match(
    mind.get(lifecycle.id)!.comments.at(-1)!.body,
  ...
  const question = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Question is not executable', kind: 'question' },
    }),
  ).item;
  const ready = value(
    await client.callTool({ name: 'mind_ready', arguments: {} }),
  );
  assert.ok(
    ready.items.every(
      (item: any) =>
  ...
  const race = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Atomic claim race' },
    }),
  ).item;
  const claimed = value(
    await client.callTool({
      name: 'mind_claim',
      arguments: { id: race.id, note: 'worker 7 starts' },
    }),
  );
  ...
  const deniedClaim = await otherClient.callTool({
    name: 'mind_claim',
    arguments: { id: race.id },
  });
  assert.equal(deniedClaim.isError, true);
  assert.equal(envelope(deniedClaim).error.code, 'CLAIM_CONFLICT');
  assert.match(
    deniedClaim.content.find((part) => part.type === 'text')?.text ?? '',
    /claimed by mcp:codex-worker-7/,
  );

  value(
    await client.callTool({
      name: 'mind_renew',
      arguments: { id: race.id, ttl_minutes: 60 },
    }),
  );
  value(
    await client.callTool({
      name: 'mind_release',
      arguments: { id: race.id, note: 'handoff test' },
    }),
  );
  const claimedAfterRelease = value(
  ...
  value(
    await otherClient.callTool({
      name: 'mind_release',
      arguments: { id: race.id, note: 'test complete' },
    }),
  );

  const awaitItem = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Await direct reply' },
    }),
  ).item;
  const awaitMessage = value(
    await client.callTool({
      name: 'mind_message',
      arguments: { id: awaitItem.id, body: 'One bounded continuation wait?' },
    }),
  );
  const awaitPromise = client.callTool({
    ...
    arguments: {
      id: awaitItem.id,
      comment_id: awaitMessage.comment.id,
      wait_seconds: 2,
    },
  });
  setTimeout(
  ...
  const created = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Inspect the parser', tags: ['coding'] },
    }),
  ).item;
  ...
  const comment = value(
    await client.callTool({
      name: 'mind_comment',
      arguments: { id: created.id, body: 'Found the entry point.' },
    }),
  );
  ...
  const askPromise = client.callTool({
    ...
    arguments: {
      id: created.id,
      body: 'Should this stay decoder-local?',
      wait_seconds: 2,
    },
  });
  for (let i = 0; i < 20 && wakes.length < wakeBase + 2; i++)
  ...
  const detail = value(
    await client.callTool({ name: 'mind_get', arguments: { id: created.id } }),
  );
  assert.deepEqual(
    detail.comments.map((entry: any) => [
      entry.author,
      entry.body,
      entry.replyToId,
    ]),
  ...
  const expectedNewestEvents = mind
    .get(created.id)!
    .events.slice(0, 2)
    .map((event) => event.id);
  const withEvents = value(
    await client.callTool({
      name: 'mind_get',
      arguments: { id: created.id, include: ['events'], event_limit: 2 },
    }),
  );
  assert.deepEqual(
    withEvents.events.map((event: any) => event.id),
    expectedNewestEvents,
  );

  const unchanged = envelope(
    await client.callTool({
      name: 'mind_update',
      arguments: { id: created.id, title: created.title },
    }),
  );
  ...
  value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Different tag', tags: ['other-filter'] },
    }),
  );
  const strict = value(
    await client.callTool({
      name: 'mind_discover',
      arguments: { context: 'ISO due date', filter_tags: ['strict-filter'] },
    }),
  );
  assert.deepEqual(
    strict.matches.map((match: any) => match.item.id),
    [dated.id],
  );
  ...
  const firstPage = envelope(
    await client.callTool({
      name: 'mind_list',
      arguments: { limit: 2, sort: 'created_asc' },
    }),
  );
  ...
  const resumable = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Resume blocked work' },
    }),
  ).item;
  value(
    await client.callTool({
      name: 'mind_claim',
      arguments: { id: resumable.id },
    }),
  );
  value(
    await client.callTool({
      name: 'mind_block',
      arguments: { id: resumable.id, blocker: 'Need fixture' },
    }),
  );
  const resumed = value(
    await client.callTool({
      name: 'mind_resume',
      arguments: { id: resumable.id, note: 'Fixture arrived' },
    }),
  );
  assert.equal(resumed.item.status, 'in_progress');
  assert.equal(resumed.item.claim.owner, 'mcp:codex-worker-7');
  assert.match(
    mind.get(resumable.id)!.comments.at(-1)!.body,
    /Resumed and claimed through MCP/,
  );
  value(
    await client.callTool({
      name: 'mind_release',
      arguments: { id: resumable.id, note: 'resume tested' },
    }),
  );
  ...
  const graphParent = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Graph parent', kind: 'project' },
    }),
  ).item;
  const graphRoot = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Graph root', parent_id: graphParent.id },
    }),
  ).item;
  const graphSibling = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Graph sibling', parent_id: graphParent.id },
    }),
  ).item;
  const graphDependency = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: 'Graph dependency' },
    }),
  ).item;
  value(
    await client.callTool({
      name: 'mind_link',
      arguments: { id: graphRoot.id, depends_on: graphDependency.id },
    }),
  );
  const wideGraph = value(
    await client.callTool({
      name: 'mind_graph',
      arguments: { id: graphRoot.id, depth: 2 },
    }),
  );
  assert.ok(
    wideGraph.nodes.some((node: any) => node.id === graphSibling.id),
    'default graph retains compatibility and reaches siblings',
  );
  const dependencyGraph = value(
    await client.callTool({
      name: 'mind_graph',
      arguments: { id: graphRoot.id, depth: 2, relations: ['dependencies'] },
    }),
  );
  assert.deepEqual(
    new Set(dependencyGraph.nodes.map((node: any) => node.id)),
    new Set([graphRoot.id, graphDependency.id]),
  );
  const isolatedContext = value(
    await client.callTool({
      name: 'mind_context',
      arguments: {
        id: graphRoot.id,
        include_related: false,
        graph_relations: ['dependencies'],
      },
    }),
  );
  assert.deepEqual(isolatedContext.related, []);
  assert.ok(
    !isolatedContext.graph.nodes.some(
      (node: any) => node.id === graphSibling.id,
    ),
  );
  value(
    await client.callTool({
      name: 'mind_archive_created',
      arguments: { ids: [graphSibling.id], note: 'archival projection test' },
    }),
  );
  const parentAfterArchive = value(
    await client.callTool({
      name: 'mind_get',
      arguments: { id: graphParent.id, include: [] },
    }),
  );
  assert.equal(parentAfterArchive.item.childCount, 1);
  assert.equal(parentAfterArchive.item.totalChildCount, 2);
  const archivedSibling = value(
    await client.callTool({
      name: 'mind_list',
      arguments: { query: 'Graph sibling', include_archived: true },
    }),
  );
  ...
  const cleanup = value(
    await client.callTool({
      name: 'mind_create',
      arguments: { title: '[MCP TEST] cleanup target' },
    }),
  ).item;
  value(
    await client.callTool({
      name: 'mind_claim',
      arguments: { id: cleanup.id, note: 'cleanup lease protection' },
    }),
  );
  const claimedCleanup = await client.callTool({
    name: 'mind_archive_created',
    arguments: { ids: [cleanup.id], note: 'must not revoke live work' },
  });
  ...
  value(
    await client.callTool({
      name: 'mind_release',
      arguments: { id: cleanup.id, note: 'ready for cleanup' },
    }),
  );
  const deniedCleanup = await otherClient.callTool({
    name: 'mind_archive_created',
    arguments: { ids: [cleanup.id], note: 'wrong session' },
  });
  ...
  const archived = envelope(
    await client.callTool({
      name: 'mind_archive_created',
      arguments: { ids: [cleanup.id], note: 'test cleanup' },
    }),
  );
  assert.equal(archived.receipt.changed, true);
  assert.ok(mind.get(cleanup.id)!.archivedAt);
  const archivedAgain = envelope(
    await client.callTool({
      name: 'mind_archive_created',
      arguments: { ids: [cleanup.id], note: 'idempotence' },
    }),
  );
...
test('/mcp is absent unless explicitly enabled and wired', async (t) => {
  const port = await freePort();
  const config = makeConfig({
    console: { enabled: true, mcpEnabled: false, port, host: '127.0.0.1' },
  });
