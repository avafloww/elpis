...
test('runMigrations is idempotent and sets user_version', () => {
  ...
  assert.equal(v1, 13, 'user_version bumped to 13');
...
test('reopening an existing agent.db is a no-op that preserves data', () => {
  ...
  db1.prepare("INSERT INTO channels (id, name, updated_at) VALUES ('c1','general','2026-07-13T00:00:00Z')").run();
  db1.close();
  const db2 = openDatabase(dir);
  const row = db2.prepare("SELECT name FROM channels WHERE id='c1'").get() as { name: string } | undefined;
...
test('true v3→v4 upgrade path preserves data and creates fleet tables', () => {
 ...
 // Insert test data
  ...
  v3db.prepare("INSERT INTO channels (id, name, updated_at) VALUES (?, ?, ?)").run('ch-123', 'general', now);
  v3db.prepare("INSERT INTO scheduled_tasks (name, kind, payload, next_run_at, interval_ms) VALUES (?, ?, ?, ?, ?)").run('reminder-x', 'reminder', '{"msg":"test"}', 1000000, 60000);
 ...
 // Assert schema version upgraded to the current level
  ...
  assert.equal(finalVersion, 13, 'user_version upgraded to 13');
 ...
 // Assert fleet tables exist
  const tableNames = (upgradedDb.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all() as { name: string }[])
    .map((r) => r.name);
  assert.ok(tableNames.includes('fleet_sessions'), 'fleet_sessions table created');
  assert.ok(tableNames.includes('fleet_worktrees'), 'fleet_worktrees table created');

 // Assert pre-existing data survived
  const channelRow = upgradedDb.prepare("SELECT name FROM channels WHERE id='ch-123'").get() as { name: string } | undefined;
  assert.equal(channelRow?.name, 'general', 'channels data preserved');

  const taskRow = upgradedDb.prepare("SELECT kind, payload FROM scheduled_tasks WHERE name='reminder-x'").get() as { kind: string; payload: string } | undefined;
  assert.equal(taskRow?.kind, 'reminder', 'scheduled_tasks kind preserved');
  assert.equal(taskRow?.payload, '{"msg":"test"}', 'scheduled_tasks payload preserved');
