...
test('clearContext: empties history, queued inbound, zeroes the tracker, and resets provider session state', () => {
  ...
  assert.equal(
    llmState.resetCalls,
    1,
    'provider conversation/cache identity must rotate with the whole mind',
  );
});
...
test('clearContext: resets compactor boundary and drops pending summary', async () => {
  ...
  agent.clearContext();
  assert.equal(
    compactor.hasCompletedResult(),
    false,
    'pending summary must be dropped',
  );
...
test('clearContext: in-flight LLM response is discarded, not appended to fresh history', async () => {
  ...
  const { promise: entered, resolve: signalEntered } =
    Promise.withResolvers<void>();
  ...
  assert.equal(
    ...
    1,
    'pre-clear msg should be in history',
  );
  ...
  assert.equal(
    agent.messagesForTest.length,
    1,
    'only the post-clear msg should be in history',
  );
