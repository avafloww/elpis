...
test('ELPIS_CONFIG overrides the source-tree config path', () => {
  ...
  try {
    assert.equal(defaultConfigPath(), '/data/config.yaml');
  } finally {
...
test('module allowlist and denylist parse with empty-list semantics', () => {
  assert.deepEqual(load(configWith('  enabled: []')).modules, {
    enabled: [],
    disabled: [],
  });
  assert.deepEqual(load(configWith('  disabled: []')).modules, {
    enabled: null,
    disabled: [],
  });
  assert.deepEqual(load(configWith('  enabled: [browser, computer]')).modules, {
    enabled: ['browser', 'computer'],
    disabled: [],
  });
});
...
test('module selectors reject unknown and duplicate ids', () => {
  assert.throws(
    () => load(configWith('  enabled: [browser, nope]')),
    /unknown module 'nope'/,
  );
  assert.throws(
    () => load(configWith('  disabled: [motor, motor]')),
    /duplicate module 'motor'/,
  );
});
