...
import {
  YIELD_NUDGE_ALERT_AT,
  YIELD_NUDGE_REALERT_EVERY,
} from '../src/agent.js';
...
test('the wake description prefers auto and caps explicit waits at one hour', () => {
  assert.match(
    RUN_TOOL.function.parameters.properties.wake.description,
    /prefer auto/i,
  );
  assert.match(
    RUN_TOOL.function.parameters.properties.wake.description,
      ...
      tool_calls: [
        {
          ...
          function: {
            ...
            arguments: JSON.stringify({
              code,
              detail: 'Exercise run wake handling',
              ...(wake ? { wake: { after: '1h' } } : {}),
            }),
          },
        },
      ...
      tool_calls: [
        {
          id: 'think1',
          type: 'function',
          function: { name: 'think', arguments: JSON.stringify({ thoughts }) },
        },
      ],
    },
    stripped: false,
 ...
 * one dispatch. */
function multiRunCall(
  calls: { code: string; wake: boolean }[],
): CompleteResult {
  return {
    message: {
      role: 'assistant',
      ...
      tool_calls: calls.map((c, i) => ({
        ...
        function: {
          ...
          arguments: JSON.stringify({
            code: c.code,
            detail: 'Exercise final wake selection',
            ...(c.wake ? { wake: { after: '1h' } } : {}),
          }),
        },
      })),
    },
...
test('shared test agents expose the current inbound during a real turn and clear it when idle', async () => {
  const llm = scriptedLLM([
    runCall(
      "if (elpis.inbound?.channelId !== '100') throw new Error('missing current inbound')",
      true,
    ),
  ]);
  const { agent, inboundRef, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-inbound-ref-',
  });
  void agent.loop();
  agent.enqueue(userMsg());
  await settle();
  assert.equal(
    llm.calls,
    1,
    'documented elpis.inbound access must succeed on the first dispatch',
  );
  assert.equal(
    inboundRef.current,
    null,
    'the current inbound must clear after the turn parks',
  );
  agent.stop();
  cleanup();
});
...
test('shared test agents intercept elpis.restart without spawning a real service command', async () => {
  const llm = scriptedLLM([
    runCall(
      "const result = elpis.restart('contained'); if (!result.note.includes('simulated in test harness')) throw new Error(result.note)",
      true,
    ),
  ]);
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-restart-seam-',
  });
  void agent.loop();
  agent.enqueue(userMsg());
  await settle();
  assert.equal(
    llm.calls,
    1,
    'the simulated restart should succeed and end the turn',
  );
  agent.stop();
  cleanup();
});
...
test('external thinking is forced once on each outer turn, then the separator continuation is ordinary', async () => {
  ...
  assert.equal(
    llm.options[0].forceThink,
    true,
    'first model request must force think',
  );
  assert.equal(
    llm.options[1].forceThink,
    false,
    'post-think continuation must return to normal choice',
  );
  assert.equal(sent.length, 0, 'think arguments never become chat output');
  const thinkMessage = agent.messagesForTest.find(
    (message) =>
      message.role === 'assistant' &&
      message.tool_calls?.some((call) => call.function.name === 'think'),
  );
  assert.ok(thinkMessage);
  assert.match(thinkMessage.tool_calls![0].function.arguments, /actual edge/);
  const separator = agent.messagesForTest.find(
    (message) => message.role === 'tool' && message.tool_call_id === 'think1',
  );
...
test('external thinking stays optional on synthetic stimulus turns', async () => {
  ...
  assert.equal(
    llm.options[0].forceThink,
    false,
    'synthetic turn keeps think available without requiring it',
  );
  cleanup();
});
...
test('resident context serves once per outer turn before inbound, not on tool continuations', async () => {
  ...
  const mind = {
    ...
    list: (filter: MindListFilter = {}) => {
      if (filter.statuses && !filter.statuses.includes(mindItem.status))
        return [];
  ...
  agent.enqueue({
    ...userMsg(),
    id: 'm-batch-2',
    content: 'second message in the same inbound batch',
    guildId: 'g-home',
  });
  void agent.loop();
  await settle();
  ...
  assert.match(
    ...
    /^\[person-memory/,
    'first-seen profile stays inside the current inbound batch',
  );
  assert.match(
    String(llm.requests[0].at(-2)?.content),
    /hi/,
    'first inbound stays after its profile',
  );
  assert.match(
    String(llm.requests[0].at(-1)?.content),
    /second message in the same inbound batch/,
    'current conversation has maximum recency',
  );
  assert.ok(
  ...
  assert.match(
    ...
    /new outer turn/,
    'new conversation remains after the frontier',
  );
  cleanup();
});
...
test('wake on a FAILED run continues the chain', async () => {
  ...
  const llm = scriptedLLM([
    runCall('throw new Error("boom")', true),
    EMPTY_WAKE,
  ]);
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-end-fail-',
  });
  void agent.loop();
  agent.enqueue(userMsg());
  await settle();
  ...
  assert.equal(
    llm.calls,
    2,
    `a failed run must not end the turn (calls=${llm.calls})`,
  );
  cleanup();
});
...
test('a mixed multi-tool-call response — an ending call followed by a failing call — does not end the turn', async () => {
  ...
  const llm = scriptedLLM([
    multiRunCall([
      { code: '1 + 1', wake: true },
      { code: 'throw new Error("boom")', wake: false },
    ]),
    EMPTY_WAKE,
  ]);
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-end-mixed-',
  });
  void agent.loop();
  agent.enqueue(userMsg());
  await settle();
  ...
  assert.equal(
    llm.calls,
    2,
    `a sibling call's failure must not be swallowed by an earlier end:true (calls=${llm.calls})`,
  );
  cleanup();
});
...
test('wake still yields when the successful run only sends', async () => {
  const llm = scriptedLLM([
    runCall('await elpis.channel("100").send("hi there")', true),
  ]);
  const { agent, sent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-end-send-',
  });
  void agent.loop();
  agent.enqueue(userMsg());
  await settle();
...
test('a response with NO tool calls does not end the turn — it nudges and continues', async () => {
  ...
  const bare: CompleteResult = {
    message: { role: 'assistant', content: '</invoke>' },
    stripped: false,
  ...
  assert.ok(
    users.some((m) => m.content.includes(YIELD_NUDGE_MARK)),
    'the yield nudge should be in history',
  );
  assert.ok(
    users.some((m) => m.content.includes('wake: { auto: true }')),
...
test('the nudge repeats without bound — there is deliberately no force-end', async () => {
  ...
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-end-unbounded-',
  });
  const countNudges = () =>
    agent.messagesForTest.filter(
      (m) => m.role === 'user' && m.content.includes(YIELD_NUDGE_MARK),
    ).length;
  void agent.loop();
  agent.enqueue(userMsg());
  await settle(150);
...
test('the consecutive-nudge counter resets when a turn finally ends', async () => {
  const bare: CompleteResult = {
    message: { role: 'assistant', content: 'not yet' },
    stripped: false,
    usage: { prompt_tokens: 10, completion_tokens: 2, total_tokens: 12 },
  };
  ...
  agent.stop();
  assert.equal(
    agent.yieldNudgeCountForTest,
    0,
    'counter reset at the last finishTurn',
  );
  cleanup();
});
...
test('ghost-reply nudge still fires on a wake-yielded turn with zero sends', async () => {
      ...
      tool_calls: [
        {
          ...
          function: {
            name: 'run',
            arguments:
              '{"code":"1+1","detail":"Compute the ghost fixture","wake":{"after":"1h"}}',
          },
        },
      ],
    },
  ...
  const users = agent.messagesForTest.filter((m) => m.role === 'user');
  assert.ok(
    users.some((m) => m.content.includes('you wrote a reply but sent nothing')),
...
test('ghost-reply nudge takes precedence over the end-nudge', async () => {
  ...
  const ghostBare: CompleteResult = {
    message: {
      role: 'assistant',
      content: 'Sure, happy to help you with that today.',
    },
    stripped: false,
  ...
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-end-precedence-',
  });
  void agent.loop();
  agent.enqueue(userMsg());
  await settle(200);
  agent.stop();
  const users = agent.messagesForTest.filter((m) => m.role === 'user');
  const ghostIdx = users.findIndex((m) =>
    m.content.includes('you wrote a reply but sent nothing'),
  );
  ...
  const endIdx = users.findIndex((m) =>
    m.content.includes('that did not end your turn'),
  );
...
test('mid-turn inbound arriving during a wake-bearing run drains instead of parking', async () => {
  ...
  const users = agent.messagesForTest.filter((m) => m.role === 'user');
  assert.ok(
    users.some((m) => m.content.includes('one more thing')),
...
test('a sustained nudge loop alerts the operator exactly once, at the threshold', async () => {
  ...
  agent.stop();
  await settle(50);
  const alerts = sent.filter(
    (s) => s.channelId === '999' && /no-run-call responses/.test(s.text),
  );
  assert.equal(
    alerts.length,
    1,
    `expected exactly one operator alert, got ${alerts.length}`,
  );
  cleanup();
});
...
test('agent: a spinning nudge loop reaches console.yieldNudge with the consecutive count', async () => {
  ...
  const counts: number[] = [];
  const stubConsole = {
    yieldNudge: (count: number) => {
      counts.push(count);
    },
  } as unknown as ConsoleHub;
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-end-hub-wire-',
  ...
  assert.ok(
    counts.length > 0,
    'console.yieldNudge must be called during a spin',
  );
  assert.deepEqual(
    counts,
    counts.map((_, i) => i + 1),
...
test('agent: a throwing console.yieldNudge is swallowed — the loop keeps nudging', async () => {
  ...
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-end-hub-throw-',
    agentDeps: { console: stubConsole },
  });
  void agent.loop();
  agent.enqueue(userMsg());
  ...
  assert.ok(
    first > 0,
    'the loop must have started nudging before the throwing observer could matter',
  );
  assert.ok(
    second > first,
...
test('clearContext() resets the nudge counter mid-spin — the fresh spin can re-alert', async () => {
  ...
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-end-clear-reset-',
  });
  void agent.loop();
  agent.enqueue(userMsg());
  await settle(300);
...
test('a sustained no-run-call spin ALSO re-alerts every YIELD_NUDGE_REALERT_EVERY cycles past the threshold', async () => {
  ...
  const responses = Array.from({ length: secondCrossing }, () => bare).concat([
    EMPTY_WAKE,
  ]);
  const llm = scriptedLLM(responses);
  const base = makeConfig();
...
test('a genuinely wake-yielded turn logs its outcome exactly once (no premature/duplicate "turn end" line)', async () => {
  ...
  assert.equal(
    yielded.length,
    1,
    `expected exactly one real turn-yield log line, got ${yielded.length}`,
  );
  cleanup();
});
...
test('a ghost-bounced turn does not log a turn-end that never happened', async () => {
  ...
  const { agent, cleanup } = buildTestAgent({
    llm,
    tmpPrefix: 'harness-end-logbounce-',
    config: { logger },
  });
  void agent.loop();
  agent.enqueue(userMsg());
