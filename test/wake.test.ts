...
const guilds: GuildConfig[] = [
  ...
  },
  {
    ...
    slug: 'friends-a',
    slashCommands: false,
    quietHours: { start: 23 * 60, end: 9 * 60 },
    timezone: 'UTC',
...
test('wake: direct channel wakes immediately', () => {
  assert.equal(
    classifyInbound(
      { ...human, guildId: 'g1', channelId: '1001' },
      idx,
      noMutes,
    ),
    'wake',
...
test('wake: mention or reply wakes on any tier', () => {
  assert.equal(
    classifyInbound(
      { ...human, mentionsMe: true, guildId: 'g1', channelId: '1002' },
      idx,
      noMutes,
    ),
    'wake',
  ...
  assert.equal(
    classifyInbound(
      { ...human, replyToMe: true, guildId: 'g1', channelId: '1003' },
      idx,
      noMutes,
    ),
    'wake',
...
test('wake: social otherwise ambient; quiet ambient', () => {
  assert.equal(
    classifyInbound(
      { ...human, guildId: 'g1', channelId: '1002' },
      idx,
      noMutes,
    ),
    'ambient',
  ...
  assert.equal(
    classifyInbound(
      { ...human, guildId: 'g1', channelId: '1003' },
      idx,
      noMutes,
    ),
    'ambient',
...
test('wake: killswitch deafen drops everything', () => {
  ...
  assert.equal(
    classifyInbound(
      { ...human, mentionsMe: true, guildId: 'g1', channelId: '1001' },
      idx,
      deaf,
    ),
    'drop',
...
test('mute: direct-tier message still wakes', () => {
  ...
  assert.equal(
    classifyInbound({ ...human, guildId: 'g1', channelId: '1001' }, idx, muted),
    'wake',
  );
});
...
test('mute: non-addressed social chatter stays ambient', () => {
  const muted = () => 'mute' as const;
  assert.equal(
    classifyInbound({ ...human, guildId: 'g1', channelId: '1002' }, idx, muted),
    'ambient',
  );
});
...
test('wake: unlisted channel in configured guild drops; unconfigured guild drops', () => {
  assert.equal(
    classifyInbound(
      { ...human, guildId: 'g1', channelId: '9999' },
      idx,
      noMutes,
    ),
    'drop',
  ...
  assert.equal(
    classifyInbound(
      { ...human, guildId: 'nope', channelId: '1001' },
      idx,
      noMutes,
    ),
    'drop',
...
test('wake: channel claimed under the wrong (but configured) guild drops', () => {
  ...
  assert.equal(
    classifyInbound(
      { ...human, guildId: 'g2', channelId: '1001' },
      idx,
      noMutes,
    ),
    'drop',
...
test('countsForTick: social counts; quiet-tier and muted never; quiet-hours suppress at tick time', () => {
  ...
  assert.equal(
    countsForTick('2001', idx, noMutes, new Date('2026-07-22T23:30:00Z')),
    false,
  );
...
test('wake: guild default tier hears unknown channels while the omitted default still drops', () => {
  ...
  assert.equal(
    classifyInbound(
      { ...human, guildId: 'g-listen', channelId: '9999' },
      listen,
      noMutes,
    ),
    'ambient',
...
test('wake: explicit drop overrides a listen-all guild receive default', () => {
  const g: GuildConfig = {
    ...
    defaultAllowSend: false,
    channels: { '10': 'drop' },
    channelAllowSend: { '10': true },
  };
  assert.equal(
    classifyInbound(
      { ...human, guildId: 'g', channelId: '10' },
      buildGuildIndex([g]),
      noMutes,
    ),
    'drop',
