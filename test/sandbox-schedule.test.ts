...
test('coerceNextRunAt: ISO string parses', () => {
  assert.equal(coerceNextRunAt('2026-07-23T00:00:00Z'), Date.parse('2026-07-23T00:00:00Z'));
});
...
function baseConfig() {
  return { paths: { dataDirectory: '/tmp', harnessRoot: '/tmp' }, sandbox: { syncTimeoutMs: 5000, asyncDeadlineMs: 10000, previewMaxBytes: 2048, logMaxBytes: 2048 }, kagi: { apiKey: null } };
}

function fakeScheduler() {
  const tasks = new Map<number, { id: number; name: string; payload: string; nextRunAt: number }>();
  let nextId = 1;
  const scheduler: NonNullable<SandboxDeps['scheduler']> = {
    create: (opts) => {
      const task = { id: nextId++, name: opts.name, payload: opts.payload, nextRunAt: opts.nextRunAt };
      tasks.set(task.id, task);
      return task;
    },
    delete: (id) => tasks.delete(id),
    list: () => Array.from(tasks.values()),
    getByName: (name) => Array.from(tasks.values()).find((t) => t.name === name) ?? null,
    markDone: () => null,
    markDoneByName: () => true,
    snooze: () => null,
    snoozeByName: () => true,
...
test('elpis.schedule: requires a non-empty name', () => {
  const g = buildGlobals({ config: baseConfig(), scheduler: fakeScheduler() } as unknown as SandboxDeps);
  const elpis = g.elpis as { schedule: (opts: unknown) => unknown };
  assert.throws(() => elpis.schedule({ name: '', payload: 'x', nextRunAt: Date.now() }), /name.*non-empty string/);
});

test('elpis.schedule: requires a string payload', () => {
  const g = buildGlobals({ config: baseConfig(), scheduler: fakeScheduler() } as unknown as SandboxDeps);
  const elpis = g.elpis as { schedule: (opts: unknown) => unknown };
  assert.throws(() => elpis.schedule({ name: 'x', payload: 123, nextRunAt: Date.now() }), /payload.*must be a string/);
});
...
test('elpis.schedule: coerces an ISO nextRunAt', () => {
  ...
  const elpis = g.elpis as { schedule: (opts: unknown) => { nextRunAt: number } };
  const task = elpis.schedule({ name: 'x', payload: 'y', nextRunAt: '2026-07-23T00:00:00Z' });
...
test('elpis.schedule.update: patches an existing task by id', () => {
  const scheduler = fakeScheduler();
  const g = buildGlobals({ config: baseConfig(), scheduler } as unknown as SandboxDeps);
  const elpis = g.elpis as { schedule: ((opts: unknown) => { id: number }) & { update: (id: number, patch: unknown) => { payload: string } } };
  const task = elpis.schedule({ name: 'x', payload: 'y', nextRunAt: Date.now() });
...
test('elpis.schedule.update: null clears snoozeUntil', () => {
  ...
  const task = elpis.schedule({ name: 'clear-snooze', payload: 'y', nextRunAt: Date.now() });
  const set = elpis.schedule.update(task.id, { snoozeUntil: '2026-08-10T02:00:00Z' });
