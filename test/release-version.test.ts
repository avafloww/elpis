...
const workspacePackages = ['elpis-client', 'elpis-protocol'] as const;
...
const versionState = (version: string): ReleaseVersionState => ({
  ...
  'Cargo-lock-workspace': workspacePackages.map((name) => ({ name, version })),
});
...
test('release state accepts a reachable v-tag and checks every scalar and Cargo package version', () => {
  assert.equal(
  ...
  const cargoMismatch = versionState('1.2.3');
  cargoMismatch['Cargo-lock-workspace'][1].version = '1.2.4';
  assert.equal(
    errorCode(() => validateReleaseVersionState('v1.2.3', cargoMismatch)),
    'release_state_mismatch',
  );

  for (const cargoEntries of [
    [],
    [
      { name: 'elpis-client', version: '1.2.3' },
      { name: 'elpis-client', version: '1.2.3' },
    ],
    [{ name: '../elpis', version: '1.2.3' }],
    [{ name: 'elpis', version: 'v1.2.3' }],
    [{ name: 'elpis', version: '1.2.3', extra: true }],
  ]) {
    const versions = versionState('1.2.3') as ReleaseVersionState & {
      'Cargo-lock-workspace': unknown;
    };
    versions['Cargo-lock-workspace'] = cargoEntries;
    assert.equal(
      errorCode(() => validateReleaseVersionState('v1.2.3', versions)),
      'invalid_release_state',
    );
...
test('release planner derives patch and minor plans, preserves order, and deep-copies state', () => {
  ...
  versions['Cargo-lock-workspace'][0].version = '9.9.9';
  assert.equal(patch.versionState['Cargo-lock-workspace'][0].version, '1.2.3');
  assert.ok(Object.isFrozen(RELEASE_OWNED_PATHS));
  assert.ok(Object.isFrozen(RELEASE_SCALAR_VERSION_SOURCES));
...
test('owned release rejects every scalar and Cargo transition-state mismatch', () => {
  for (const source of RELEASE_SCALAR_VERSION_SOURCES) {
    assert.equal(
      ownedError((facts) => {
  ...
  assert.equal(
    ownedError((facts) => {
      facts.currentVersions['Cargo-lock-workspace'][1].version = '1.2.5';
    }),
    'owned_release_transition_mismatch',
  );
  assert.equal(
  ...
  assert.equal(
    ownedError((facts) => {
      facts.currentVersions['Cargo-lock-workspace'] = [
        { name: 'different-package', version: '1.2.4' },
        { name: 'elpis-protocol', version: '1.2.4' },
      ];
    }),
    'owned_release_transition_mismatch',
  );
...
test('owned release changed paths reject omissions, duplicates, extras, traversal, and lookalikes', () => {
  for (const paths of [
    ...
    ['./VERSION', ...RELEASE_OWNED_PATHS.slice(1)],
    ['version', ...RELEASE_OWNED_PATHS.slice(1)],
    ['rust/Cargo.toml/../Cargo.lock', ...RELEASE_OWNED_PATHS.slice(1)],
    ['rust\\Cargo.toml', ...RELEASE_OWNED_PATHS.slice(1)],
    ['rust/Cargo.tomI', ...RELEASE_OWNED_PATHS.slice(1)],
  ]) {
    assert.equal(
      ownedError((facts) => {
