...
import { translate, createBillingHeader, patchCch } from '../src/llm/anthropic-client.js';
...
test('translate: tool_calls → tool_use, tool result → user tool_result', () => {
  const msgs: ChatMessage[] = [
    ...
    { role: 'assistant', content: 'ok', tool_calls: [{ id: 'tc1', type: 'function', function: { name: 'run', arguments: '{"code":"1+1","wake":{"auto":true}}' } }] },
...
test('translate: tool_result followed by a user envelope coalesce into one user turn', () => {
  const msgs: ChatMessage[] = [
    sys(),
    { role: 'user', content: 'q' },
    { role: 'assistant', content: '', tool_calls: [{ id: 't1', type: 'function', function: { name: 'run', arguments: '{}' } }] },
...
test('translate: empty ghost assistant message contributes no wire message', () => {
  const msgs: ChatMessage[] = [sys(), { role: 'user', content: 'hi' }, { role: 'assistant', content: '' }];
  const { wire } = translate(msgs);
...
test('translate: thinking blocks are replayed verbatim, FIRST in the assistant turn', () => {
  const msgs: ChatMessage[] = [
    ...
    { role: 'user', content: 'q' },
    {
      role: 'assistant',
      content: 'answer',
      thinking_blocks: [{ type: 'thinking', thinking: 'let me reason', signature: 'sig123' }],
      tool_calls: [{ id: 'tc1', type: 'function', function: { name: 'run', arguments: '{"end":true}' } }],
    },
 ...
 // Order must be: thinking, then text, then tool_use.
  ...
  assert.deepEqual(assistant.content[0], { type: 'thinking', thinking: 'let me reason', signature: 'sig123' });
...
test('translate: redacted_thinking blocks replay verbatim', () => {
  const msgs: ChatMessage[] = [
    sys(),
    { role: 'user', content: 'q' },
    { role: 'assistant', content: 'a', thinking_blocks: [{ type: 'redacted_thinking', data: 'ENCRYPTED' }] },
  ];
  const { wire } = translate(msgs);
  assert.deepEqual(wire[1].content[0], { type: 'redacted_thinking', data: 'ENCRYPTED' });
});
...
test('patchCch: replaces the placeholder with a stable 5-hex attestation', () => {
  ...
  const body = JSON.stringify({ system: [{ type: 'text', text: header }], messages: [] });
