...
test('toResponsesInput: replayed reasoning items are copies, not history references', () => {
  ...
  assert.equal(
    msg.reasoning_items![0].encrypted_content,
  ...
  assert.deepEqual(out.toolCalls, [
    {
      id: 'call_9',
      type: 'function',
      function: { name: 'run', arguments: '{"code":"x"}' },
    },
  ]);
});
...
function fakeResponsesClient(
  ...
  } as unknown as Parameters<typeof responsesSummarize>[0];
}
    ...
    output: [
      {
        ...
        role: 'assistant',
        status: 'incomplete',
        content: [{ type: 'output_text', text: 'a terse cut-off line' }],
      },
    ],
  });
...
test('failureToError: transient codes stay retriable', () => {
  assert.equal(
    (failureToError({ code: 'server_error', message: 'oops' }) as any).status,
    503,
  );
  assert.equal(
    (
...
test('isResponsesUnsupported: explicit 404/405/501 raw and classify-wrapped; other statuses no', () => {
  ...
  assert.equal(
    isResponsesUnsupported(Object.assign(new Error('nope'), { status: 405 })),
    true,
  );
  assert.equal(
    isResponsesUnsupported(
  ...
  assert.equal(
    isResponsesUnsupported(Object.assign(new Error('bad'), { status: 400 })),
    false,
  );
  assert.equal(
    isResponsesUnsupported(
  ...
  assert.equal(
    isResponsesUnsupported(Object.assign(new Error('boom'), { status: 500 })),
    false,
  );
...
test('responsesRunTool: flattened FunctionTool with the same schema, strict off', () => {
  ...
  assert.deepEqual(Object.keys((t.parameters as any).properties), [
    'code',
    'detail',
    'sandbox',
    'wake',
...
test('createLLM auto: 404 on /responses falls back to chat in the same call, then skips responses', async () => {
  ...
  assert.equal(
    responsesCalls,
    1,
    'after the flip, responses is never tried again',
  );
...
test('createLLM auto: explicit 501 on /responses falls back to chat and commits the flip', async () => {
  ...
  (llm.client.responses as any).create = async () => {
    responsesCalls++;
    throw Object.assign(new Error('Responses API not implemented'), {
      status: 501,
    });
  };
  (llm.client.chat.completions as any).create = async () => {
  ...
  assert.equal(
    responsesCalls,
    1,
    'explicit unsupported response commits the flip',
  );
  assert.equal(chatCalls, 2);
});
...
test('createLLM auto: first-contact Responses rate limit never probes or latches Chat', async () => {
  ...
  await assert.rejects(
    () => llm.complete([{ role: 'user', content: 'retry' }]),
    /rate_limit_exceeded/,
  );
...
test('createLLM auto: after a responses success, later errors propagate without flipping', async () => {
  ...
  assert.equal(
    chatCalls,
    0,
    'a 5xx on a proven-working route must not silently flip to chat',
  );
});
...
test('prepareForApi: reasoning_items survive the 3a strip that drops reasoning_content', () => {
  const messages: ChatMessage[] = [
    { role: 'user', content: 'q1' },
    {
      role: 'assistant',
      content: 'done',
      reasoning_content: 'kimi-style thinking',
      reasoning_items: [REASONING_ITEM],
  ...
  assert.equal(
    ...
    undefined,
    'legacy reasoning text is stripped on completed turns',
  );
  assert.deepEqual(
    out[1].reasoning_items,
    [REASONING_ITEM],
    'encrypted reasoning items are preserved',
  );
});
...
test('transcript: reasoning_items survive an append → load round-trip', () => {
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'harness-resp-transcript-'),
  );
