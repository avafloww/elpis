...
test('bsky writes facets and correct reply/engagement records', async () => {
  ...
  globalThis.fetch = (async (input: any, init: any = {}) => {
    ...
    if (url.pathname.endsWith('resolveHandle')) {
      return new Response(JSON.stringify({ did: 'did:plc:friend' }), {
        status: 200,
      });
    }
    return new Response(
      JSON.stringify({
        uri: 'at://did:plc:aster/app.bsky.feed.post/1',
        cid: 'cid',
      }),
      { status: 200 },
    );
  ...
  try {
    ...
    await bskyReply(
      cfg,
      'reply @friend.test',
      { uri: 'at://did:plc:other/app.bsky.feed.post/p', cid: 'parent' },
      { uri: 'at://did:plc:root/app.bsky.feed.post/r', cid: 'root' },
    );
    const replyRecord = calls
      .filter((c) => c.path === 'com.atproto.repo.createRecord')
      .at(-1)?.body.record;
    assert.deepEqual(replyRecord.reply, {
      root: { uri: 'at://did:plc:root/app.bsky.feed.post/r', cid: 'root' },
      parent: { uri: 'at://did:plc:other/app.bsky.feed.post/p', cid: 'parent' },
    });
