...
function deps(
  send?: SandboxDeps['send'],
  onLateProcessError?: SandboxDeps['onLateProcessError'],
): SandboxDeps {
  return {
    config: {
      sandbox: {
...
test('real stale HTTP callback routes once while a real harness fault remains global', () => {
  const fixture = path.join(
    ...
    'fixtures',
    'sandbox-process-error-child.ts',
  );
...
test('run process errors route only while the owning scope trap is active', async () => {
  ...
  const scope: RunScope = {
    ...
    childPids: new Set(),
    sends: [],
    processError: (kind, error) => {
      seen.push({ kind, error });
      return true;
    },
  };
  ...
  assert.equal(
    runScope.run(scope, () => routeRunProcessError('uncaughtException', error)),
    true,
  );
...
test('active asynchronous callback errors fail the run and suppress repeats from that failed scope', async () => {
  ...
  const sandbox = createSandbox(
    deps(
      async () => {
        ...
        assert.equal(
          routeRunProcessError('uncaughtException', new Error('callback boom')),
          true,
        );
      },
  ...
  const result = await sandbox.run(
    `await elpis.channel('console').send('trigger'); await new Promise(() => {})`,
  );
  assert.equal(result.ok, false);
  assert.equal(result.failureKind, 'runtime');
  assert.match(
    result.error ?? '',
    /asynchronous sandbox uncaughtException:.*callback boom/s,
  );
...
test('completed run without a late reporter releases its trap to the global guard', async () => {
  ...
  const result = await sandbox.run(
    `await elpis.channel('console').send('capture'); 6`,
  );
...
test('completed run attributes one late callback error and suppresses repeats', async () => {
  ...
  const result = await sandbox.run(
    `await elpis.channel('console').send('capture'); 7`,
  );
  assert.equal(result.ok, true);
