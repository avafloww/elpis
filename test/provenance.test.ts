...
const provenance = () => ({
  ...
  apiEndpoint: new URL(
    '/v1/responses?token=fixture#frag',
    `https://${'user'}:${'pass'}@private.example`,
  ).href,
...
test('generation provenance strips URL credentials/query/fragment and records contract', () => {
  ...
  assert.equal(
    message.provenance!.apiEndpoint,
...
test('endpointAt preserves configured API base paths', () => {
  assert.equal(
    endpointAt('https://api.example/coding/v1/', '/chat/completions'),
    'https://api.example/coding/v1/chat/completions',
  );
  assert.equal(
    canonicalEndpoint(new URL('/x?q=1#x', `https://${'u'}:${'p'}@a.test`).href),
    'https://a.test/x',
  );
});
...
test('provenance and run attribution are excluded from every provider request surface', () => {
  ...
  const tool: ChatMessage = {
    ...
    run: {
      ...
      ok: true,
      detail: 'private metadata detail',
      execution: {
        kind: 'persistent',
        alias: 'harness-private-alias',
        mindId: 7,
...
test('provenance survives transcript restoration', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'prov-session-'));
...
test('restart replay keeps opaque reasoning only for an exact provider/model/surface/endpoint identity', () => {
  ...
  stampGeneration(message, {
    ...
    apiEndpoint: 'https://api.example/v1/responses',
    harnessCommit: 'abc',
  });
  store.append(MAIN_TRANSCRIPT_ID, message);
  const exact = {
  ...
  const kept = loadMostRecentMain(dir, { opaqueReplayIdentity: exact })!
    .messages[0];
  assert.equal(kept.reasoning_items?.length, 1);
  const stripped = loadMostRecentMain(dir, {
    opaqueReplayIdentity: { ...exact, model: 'model-b' },
  })!.messages[0];
  ...
  assert.equal(
    loadMostRecentMain(dir, { opaqueReplayIdentity: exact })!.messages.at(-1)
      ?.reasoning_items,
    undefined,
  );
  fs.rmSync(dir, { recursive: true, force: true });
...
test('offline transcript parsing remains lossless when no replay boundary is requested', () => {
  ...
  assert.equal(
    loadMostRecentMain(dir)!.messages[0].reasoning_items?.[0].encrypted_content,
    'raw',
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test('configured replay identity is exact and Chat Completions has no opaque replay identity', () => {
  ...
  assert.equal(
    replayIdentityForConfig(makeConfig({ llm: { ...base.llm, api: 'chat' } })),
    null,
  );
  assert.equal(
    isTrustedOpaqueReplay(undefined, replayIdentityForConfig(responses)),
    false,
  );
});
...
test('clear-thinking removes native reasoning but retains provenance', () => {
  ...
  stampGeneration(message, provenance());
  built.agent.messagesForTest.push(message);
  built.agent.clearThinking();
  assert.equal(message.reasoning_items, undefined);
  assert.equal(message.thinking_blocks, undefined);
  assert.equal(message.provenance?.model, 'gpt-5.6-sol');
  assert.equal(
    loadMostRecentMain(path.join(built.tmpDir, 'sessions'))?.messages.at(-1)
...
test('a later provider/model stamp replaces attribution without mixing epochs', () => {
  ...
  assert.equal(message.provenance?.providerType, 'anthropic-oauth');
  assert.equal(message.provenance?.model, 'claude-opus-5');
...
test('automatic Responses fallback attributes the successful Chat endpoint', async () => {
  ...
  try {
    ...
    assert.equal(
      result.message.provenance?.apiEndpoint,
...
test('transport retry retains the endpoint and request id of the successful generation', async () => {
  ...
  const server = createServer((req, res) => {
    ...
    if (calls === 1) {
      res
        ...
        .end('{"error":{"message":"retry"}}');
      return;
    }
    res.writeHead(200, {
      'content-type': 'text/event-stream',
      'x-request-id': 'req-after-retry',
    });
    res.end(
      'data: ' +
        JSON.stringify({
  ...
  try {
    ...
    assert.equal(result.message.provenance?.apiSurface, 'chat-completions');
    assert.equal(result.message.provenance?.requestId, 'req-after-retry');
  } finally {
    await new Promise<void>((resolve) => server.close(() => resolve()));
  }
});
...
test('compaction preserves provenance on the verbatim tail', async () => {
  const tracker = createContextTracker(10_000, 100);
  const llm = makeStubLLM({
    summarize: async () =>
      'A sufficiently detailed summary of the earlier context for this test.',
  });
  const compactor = createCompactor(llm, tracker, { keepTokens: 20 });
  const tail: ChatMessage = {
    role: 'assistant',
    content: 'latest generation '.repeat(10),
  };
  stampGeneration(tail, provenance());
  const messages: ChatMessage[] = [
    ...Array.from({ length: 8 }, (_, i) => ({
      role: 'user' as const,
      content: `old ${i} `.repeat(30),
    })),
    tail,
  ];
  compactor.start(messages);
  await compactor.done();
  const next = compactor.applyCompaction(messages);
  assert.equal(
    next.find((m) => m.content === tail.content)?.provenance?.requestId,
    'req-1',
  );
});
