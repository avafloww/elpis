...
function temp(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'bench-outcome-'));
}

test('target channels require delivery and enforce exclusivity only when declared', () => {
  const sends = [
    { channelId: '100', text: 'working' },
    { channelId: '101', text: 'result' },
  ];
  ...
  assert.equal(
    targetChannelSatisfied('101', false, 'optional', [
      { channelId: '100', text: 'oops' },
    ]),
    false,
  );
  assert.equal(
    targetChannelSatisfied(undefined, false, 'required', sends),
    true,
  );
});
...
test('recipient targeting distinguishes direct replies from third-party delivery', () => {
  assert.equal(
    recipientSatisfied('Jo', 'Jo', 'required', [
      { channelId: '100', text: '4817' },
    ]),
    true,
  );
  assert.equal(
    recipientSatisfied('Mira', undefined, 'required', [
      { channelId: '100', text: 'The answer is 42.' },
    ]),
    false,
  );
  assert.equal(
    recipientSatisfied('Mira', undefined, 'required', [
      { channelId: '100', text: 'Mira, the answer is 42.' },
    ]),
    true,
  );
});
...
test('required JSON outcome stays false after a read and becomes true only after valid equivalent repair', () => {
  ...
  const spec = scenario({
    outcome: 'valid JSON',
    workPaths: ['settings.json'],
    action: 'required',
    checks: [
      { kind: 'json-equals', path: 'settings.json', value: { enabled: true } },
    ],
  });
...
test('required targeted outcome needs the right channel, recipient, and content predicate', () => {
  const root = temp();
  const spec = scenario({
    outcome: 'count delivered',
    targetChannel: 'ops',
    targetRecipient: 'Mira',
    workPaths: [],
    action: 'required',
    checks: [{ kind: 'send-includes', values: ['12'] }],
  });
  assert.equal(
    evaluateOutcome(spec, root, [{ channelId: '100', text: '@Mira 12' }], true)
      .ok,
    false,
  );
  assert.equal(
    evaluateOutcome(
      spec,
      root,
      [{ channelId: '101', text: '@Mira counted it' }],
      true,
    ).ok,
    false,
  );
  assert.equal(
    evaluateOutcome(spec, root, [{ channelId: '101', text: '12' }], true).ok,
    false,
  );
  assert.equal(
    evaluateOutcome(
      spec,
      root,
      [{ channelId: '101', text: '@Mira: 12 lines' }],
      true,
    ).ok,
    true,
  );
  ...
  assert.equal(
    evaluateOutcome(direct, root, [{ channelId: '101', text: '4817' }], true)
      .ok,
    true,
  );
  fs.rmSync(root, { recursive: true, force: true });
  ...
  const spec = scenario({
    ...
    checks: [
      ...
      { kind: 'path-absent', path: 'build/remove.tmp' },
      {
        kind: 'dir-files',
        path: 'build',
        files: ['keep.txt', 'nested/result.txt'],
      },
    ],
  });
  assert.equal(evaluateOutcome(spec, root, [], true).ok, true);
...
test('required work never passes without an action or without checks when there is no target', () => {
  ...
  const checked = scenario({
    ...
    workPaths: ['x'],
    action: 'required',
    checks: [{ kind: 'path-absent', path: 'x' }],
  });
...
test('outcome predicates reject paths outside the episode root', () => {
  ...
  const spec = scenario({
    ...
    workPaths: [],
    action: 'required',
    checks: [{ kind: 'path-absent', path: '../outside' }],
  });
  assert.throws(
    () => evaluateOutcome(spec, root, [], true),
