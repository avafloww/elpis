...
import {
  ...
  pointerDragScript,
  shellQuote,
  type ComputerProcessResult,
} from '../src/sandbox/computer.js';

test('parseWindows reads wmctrl geometry, class, and spaced titles', () => {
  const rows = parseWindows(
    '0x04600007  0 1234 10 20 800 600 Navigator.Firefox host Example Domain - Mozilla Firefox\n',
  );
...
test('display shell wrapper keeps credentials across a compound command', () => {
  assert.equal(
    displayShellCommand(
      'xdotool keydown Up; sleep 1; xdotool keyup Up',
      ':77',
      '/tmp/x auth',
    ),
...
test('held key script traps cleanup and releases chords in reverse order', () => {
  ...
  assert.throws(
    () => heldKeysScript(['Up', 'bad key'], 100),
    /xdotool key name/,
  );
  assert.throws(
    () => heldKeysScript('Up', 30_001),
    /durationMs must be <= 30000/,
  );
});
...
test('coordinate grid filter uses magenta and labels the top and left edges', () => {
  ...
  assert.throws(
    () => coordinateGridFilter(320, 240, 20),
    /grid spacing must be >= 25/,
  );
});
...
test('computer maps input and app launch onto deterministic X11 commands', async () => {
  ...
  const run = async (command: string): Promise<ComputerProcessResult> => {
    calls.push(command);
    const stdout = command.startsWith('xprop -root')
      ? '_NET_CLIENT_LIST(WINDOW): window id # 0x1, 0x2\n'
      : command.startsWith('wmctrl')
        ? '0x1 0 1 0 0 100 100 app.Class host Title\n'
        : 'ok\n';
  ...
  const launched = await computer.launch('firefox-esr https://example.com', {
    name: 'web',
    cwd: '/tmp',
  });
  ...
  await assert.rejects(
    computer.click(1, 2, { button: 'side' }),
    /button must be/,
  );
...
test('computer exposes safe holds, release, bounded sequences, and chord alias', async () => {
  ...
  assert.deepEqual(await computer.hold(['Up', 'f'], 25), {
    ok: true,
    keys: ['Up', 'f'],
    durationMs: 25,
  });
  ...
  await assert.rejects(
    computer.hold('Up', 100, { timeout: 500 }),
    /cleanup margin/,
  );
  await assert.rejects(
    computer.sequence([{ keys: 'Up', durationMs: 30_000, waitMs: 30_001 }]),
    /total duration/,
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test('computer treats a tint2-only desktop as an empty app window list', async () => {
  ...
  const computer: any = createComputerTools({
    ...
    run: async (command) => {
      calls.push(command);
      if (command.startsWith('xprop -root'))
        return {
          stdout: '_NET_CLIENT_LIST(WINDOW): window id # 0x200008\\n',
          stderr: '',
          code: 0,
          signal: null,
        };
      if (command.includes('WM_CLASS'))
        return {
          stdout: 'WM_CLASS(STRING) = "tint2", "Tint2"\\n',
          stderr: '',
          code: 0,
          signal: null,
        };
  ...
  assert.equal(
    calls.some((command) => command.startsWith('wmctrl')),
    false,
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test('computer lifecycle coordinates root Xorg before the user desktop', async () => {
  ...
  assert.deepEqual(calls, [
    "sudo -n systemctl start 'elpis-xorg'",
    "systemctl --user start 'elpis-desktop'",
  ]);
  calls.length = 0;
  await computer.stop();
  assert.deepEqual(calls, [
    "systemctl --user stop 'elpis-desktop'",
    "sudo -n systemctl stop 'elpis-xorg'",
  ]);
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test('computer look captures a screenshot and queues multimodal delivery', async () => {
  ...
  assert.equal(
    result.note,
    'desktop check [100px magenta coordinate grid; origin top-left]',
  );
  ...
  const stepped = await computer.step(['Up', 'f'], 10, 'game step', {
    grid: false,
    settleMs: 0,
  });
  assert.deepEqual(stepped.action, {
    ok: true,
    keys: ['Up', 'f'],
    durationMs: 10,
  });
