...
test('elpis.mind exposes a complete normalized work-graph surface', () => {
  ...
  const second = api.add({ title: 'second', parentId: `#${first.id}`, dependsOn: [`m-${first.id}`], dueAt: '2030-01-02T03:04:05Z' });
  ...
  assert.deepEqual(api.list({ blocked: true }).map((x: any) => x.id), [second.id]);

  api.comment(second.id, 'the body matters', 'bramble');
  api.done(first.id, 'foundation complete');
  assert.deepEqual(api.ready().map((x: any) => x.id), [second.id]);
  api.update(`m-${second.id}`, { priority: 4, tags: ['dashboard'], parentId: null });
...
test('elpis.mind reminders inherit the active inbound channel', () => {
  ...
  assert.equal(tasks.get(item.reminders[0].scheduledTaskId).channelId, 'home-channel');
  api.snoozeReminder(item.reminders[0].id, at + 60_000);
  assert.equal(tasks.get(item.reminders[0].scheduledTaskId).nextRunAt, at + 60_000);
