...
import { createBrowserTools, parseBrowserJson, type BrowserProcessResult } from '../src/sandbox/browser.js';
...
test('browser namespace maps default and named sessions to argument arrays', async () => {
  ...
  const run = async (args: string[], opts?: { timeout?: number }): Promise<BrowserProcessResult> => {
  ...
  const browser: any = createBrowserTools({ browserDir: dir, maximizedChromiumConfig: config, run });

  const opened = await browser.open('https://example.com', { persistent: true, timeout: 1234 });
  assert.equal(opened.ok, true);
  assert.deepEqual(calls[0], {
    args: ['-s=elpis', 'open', 'https://example.com', '--headed', `--config=${config}`, '--persistent', '--json'],
    timeout: 1234,
  });
...
test('browser commands fail loudly on a nonzero CLI result', async () => {
  ...
  const browser: any = createBrowserTools({
    browserDir: dir,
    run: async () => ({ stdout: '', stderr: 'session not found', code: 1, signal: null }),
  });
...
test('browser look captures a deterministic file and queues multimodal delivery', async () => {
  ...
  const browser: any = createBrowserTools({
    ...
    run: async (args) => {
      ...
      if (filename) { fs.mkdirSync(path.dirname(filename), { recursive: true }); fs.writeFileSync(filename, 'png'); }
