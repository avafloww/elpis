...
test('localizeByContent: exact single-chunk send matches', () => {
  const files: LoadedFile[] = [
    { file: '/a.jsonl', messages: [toolWithSend('100', 'hello world')] },
  ];
...
test('localizeByContent: matches a chunk of a long (multi-chunk) send across the trimmed boundary', () => {
  ...
  const files: LoadedFile[] = [
    { file: '/a.jsonl', messages: [toolWithSend('100', full)] },
  ];
...
test('localizeByContent: no match returns null; wrong channel does not match', () => {
  const files: LoadedFile[] = [
    { file: '/a.jsonl', messages: [toolWithSend('100', 'hello')] },
  ];
...
test('localizeByContent: identical repeated sends → newest file/message wins', () => {
  const older: LoadedFile = {
    file: '/old.jsonl',
    messages: [toolWithSend('100', 'done')],
  };
  const newer: LoadedFile = {
    file: '/new.jsonl',
    messages: [toolWithSend('100', 'earlier'), toolWithSend('100', 'done')],
  };
