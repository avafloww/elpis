...
function fakeLLM(rewrite: (text: string, system: string) => string | Promise<string>): LLM {
  return {
    model: 'fake', runTool: {} as LLM['runTool'],
    async complete() { throw new Error('not used'); },
    async summarize(text, system = '') { return rewrite(text, system); },
  };
}

function manager(paths: ReturnType<typeof fixture>, llm: LLM, threshold = 60, target = 30) {
  return new MemoryConsolidator({ ...paths, thresholdTokens: threshold, targetTokens: target, maxContextTokens: 1000,
    estimateTokens: chars => Math.ceil(chars / 4), llm, logger: noopLogger, debounceMs: 5 });
}

test('memory limits clamp to half the usable model window', () => {
  assert.deepEqual(effectiveMemoryLimits(32_000, 24_000, 32_000, 8_000), { threshold: 12_000, target: 9_000 });
  assert.deepEqual(effectiveMemoryLimits(32_000, 24_000, 1_000_000, 8_000), { threshold: 32_000, target: 24_000 });
  assert.deepEqual(effectiveMemoryLimits(0, 24_000, 16_000, 8_000), { threshold: 0, target: 24_000 });
});

test('private consolidation prompt owns the memory, permits grug, and forbids new dates/polish', () => {
  assert.match(MEMORY_CONSOLIDATION_PROMPT, /your memory, not anyone else's profile/i);
  assert.match(MEMORY_CONSOLIDATION_PROMPT, /private by default/i);
  assert.match(MEMORY_CONSOLIDATION_PROMPT, /Grug\/fragment language is welcome/);
...
test('oversized memory is atomically consolidated and backed up', async () => {
  ...
  let seenSystem = '';
  const m = manager(p, fakeLLM((_text, system) => { seenSystem = system; return '# memory\nthing keep. fix known.\n'; }));
...
test('failure preserves original and safeMemoryView bounds boot injection', async () => {
  ...
  const original = '# Memory\n' + 'do not lose me\n'.repeat(200);
  fs.writeFileSync(p.memoryPath, original);
  const m = manager(p, fakeLLM(() => { throw new Error('provider down'); }));
  const result = await m.checkNow(p.memoryPath, true);
...
test('person frontmatter is preserved byte-for-byte while only its body is rewritten', async () => {
  ...
  const m = manager(p, fakeLLM(() => 'I know friend. kind. boundary stays.'));
  const result = await m.checkNow(person, true);
  assert.equal(result.status, 'consolidated');
  assert.equal(fs.readFileSync(person, 'utf8'), frontmatter + 'I know friend. kind. boundary stays.\n');
});
...
test('watch requests are debounced and self-write does not loop', async () => {
  ...
  fs.writeFileSync(p.memoryPath, '# small\n');
  let calls = 0;
  const m = manager(p, fakeLLM(() => { calls++; return '# small now\n'; }));
  m.startWatching();
...
test('boot scan includes MEMORY.md and people files', async () => {
  ...
  fs.writeFileSync(path.join(p.dataDirectory, 'people', 'friend.md'), 'friend\n'.repeat(100));
  const m = manager(p, fakeLLM(() => 'I keep this.'));
  const results = await m.ensureBootSafe();
  assert.deepEqual(results.map(r => r.status), ['consolidated', 'consolidated']);
});
