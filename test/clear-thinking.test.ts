...
import { clearThinkingConfirmCustomId, clearThinkingCancelCustomId, SLASH_COMMAND_NAMES } from '../src/discord/discord.js';
...
test('clear-thinking custom-ids are namespaced by user and distinct from /clear', () => {
  ...
  assert.notEqual(clearThinkingConfirmCustomId('u1'), clearThinkingConfirmCustomId('u2'));
  assert.ok((SLASH_COMMAND_NAMES as readonly string[]).includes('clear-thinking'));
});
...
test('clearThinking: strips thinking_blocks + reasoning_items in memory and on disk, keeps everything else', () => {
  ...
  try {
    ...
    const assistant: ChatMessage = {
      ...
      reasoning_items: [{
        id: 'rs-old', type: 'reasoning', summary: [], encrypted_content: 'gAAA-old-model',
      }],
