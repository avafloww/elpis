...
test('ghost reply triggers exactly one bounce, then a send clears the flag', async () => {
  ...
  const ghost =
    'I would love to help you with that. Let me think through the best approach here. ' +
        ...
        tool_calls: [
          {
            ...
            function: {
              name: 'run',
              arguments:
                '{"code":"elpis.channel(\\"100\\").send(\\"all set\\")","detail":"Send the completion reply"}',
            },
          },
        ],
      },
  ...
  const { promise: thirdCall, resolve: signalThird } =
...
test('short content with zero sends DOES trigger a bounce (no length gate)', async () => {
  ...
  const llm = scriptedLLM([
    {
      message: { role: 'assistant', content: "hoiii~ I'm well. How are you?" },
      stripped: false,
...
test('a tool-chain ghost bounces (user → tool_call → result → content-only reply)', async () => {
  ...
  const llm = scriptedLLM([
        ...
        tool_calls: [
          {
            ...
            function: {
              name: 'run',
              arguments:
                '{"code":"1 + 1","detail":"Compute the requested sum"}',
            },
          },
        ],
      },
    ...
    // Turn 1b (after the tool result): the ghost — a content-only reply, no sends.
    {
      message: {
        role: 'assistant',
        content: 'The answer is 2. Let me know if you need anything else!',
      },
      stripped: false,
        ...
        tool_calls: [
          {
            ...
            function: {
              name: 'run',
              arguments:
                '{"code":"elpis.channel(\\"100\\").send(\\"it is 2\\")","detail":"Send the computed answer"}',
            },
          },
        ],
      },
...
test('send in a tool-chain iteration + brief closing content → NO bounce (turn-scoped sends)', async () => {
  ...
  const llm = scriptedLLM([
        ...
        tool_calls: [
          {
            ...
            function: {
              name: 'run',
              arguments:
                '{"code":"elpis.channel(\\"100\\").send(\\"here you go\\")","detail":"Send the requested result"}',
            },
          },
        ],
      },
    ...
    {
      message: { role: 'assistant', content: 'sent; watching for her reply.' },
      stripped: false,
  ...
  assert.equal(
    users.length,
    2,
    'the real message + the yield nudge — a send earlier in the turn must suppress the bounce for closing content',
  );
  assert.ok(
    !users.some((m) =>
      m.content.includes('you wrote a reply but sent nothing'),
...
test('D1: a real wake into a self-muted channel is annotated so the model knows a reply will not send', async () => {
  ...
  const { agent } = buildTestAgent({
    ...
    agentDeps: {
      mutes: {
        get: (id: string) =>
          id === '100'
            ? { type: 'mute' as const, actor: 'self', reason: null }
            : null,
      } as any,
    },
  });
  ...
  assert.ok(
    users.some((m) =>
      m.content.includes('is muted — a reply here will not send'),
    ),
...
test('a ghost reply in a config-denied channel does NOT bounce', async () => {
  ...
  const guilds = [
    {
      ...
      quietHours: null,
      timezone: null,
      defaultTier: 'drop' as const,
      allowSend: true,
      defaultAllowSend: false,
      channels: { '100': 'direct' as const },
      channelAllowSend: { '100': false },
    },
  ];
  const { agent } = buildTestAgent({
    llm,
  ...
  assert.ok(
    !users.some((m) => m.content.includes('REMINDER: use elpis.channel')),
  ...
  const { agent } = buildTestAgent({
    ...
    agentDeps: {
      mutes: {
        get: (id: string) =>
          id === '100'
            ? { type: 'mute' as const, actor: 'operator', reason: null }
            : null,
      } as any,
      channels: {
        guildOf: () => null,
...
test('a send during the turn prevents the bounce', async () => {
        ...
        tool_calls: [
          {
            ...
            function: {
              name: 'run',
              arguments:
                '{"code":"elpis.channel(\\"100\\").send(\\"sent\\")","detail":"Send the turn reply"}',
            },
          },
        ],
      },
  ...
  const { agent, sent } = buildAgentWith(llm);
  const { promise: secondCall, resolve: signalSecond } =
    Promise.withResolvers<void>();
  llm.onCall = (n) => {
    if (n === 2) signalSecond();
  };
