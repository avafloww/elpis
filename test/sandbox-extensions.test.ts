...
function registry(): ExtensionRegistry {
  const api = Object.freeze(Object.assign(Object.create(null), { add: (a: number, b: number) => a + b }));
  const apis = Object.freeze(Object.assign(Object.create(null), { sample: api }));
  return Object.freeze({
    apis,
    summaries: Object.freeze([Object.freeze({ namespace: 'sample', file: 'sample.ext.ts', description: 'fixture', members: Object.freeze(['add']) })]),
...
test('elpis.ext exposes frozen namespaced APIs and deterministic help', () => {
  ...
  const elpis = globals.elpis as { ext: { sample: { add(a: number, b: number): number }; $help(namespace?: string): unknown; $failures(): unknown[] } };
  assert.equal(elpis.ext.sample.add(20, 22), 42);
  assert.deepEqual(elpis.ext.$help('sample'), { namespace: 'sample', file: 'sample.ext.ts', description: 'fixture', members: ['add'] });
  assert.ok(Object.isFrozen(elpis.ext));
  assert.ok(Object.isFrozen(elpis.ext.sample));
  assert.deepEqual(elpis.ext.$failures(), []);
  assert.throws(() => elpis.ext.$help('missing'), /unknown extension namespace/);
});

test('elpis.ext exists with empty help when no extensions are loaded', () => {
  const globals = buildGlobals({ config } as never);
  const elpis = globals.elpis as { ext: { $help(): unknown[]; $failures(): unknown[] } };
