...
const history = (): ChatMessage[] => [
  ...
  },
  {
    role: 'user',
    content:
      '<incoming-message channel="agora" author="ari" time="2026-07-04T05:09:15.235Z" local-time="12:34">\nhey\n</incoming-message>',
    channel: 'c1',
  },
  // the OPEN chain (run call without end:true) — its reasoning must survive
  {
    role: 'assistant',
    content: '',
    reasoning_content: 'live thinking',
    channel: 'c1',
    tool_calls: [
      {
        id: 'call_1',
...
test('contextSnapshot: system message first, wire shape only, request-assembly diet applied', () => {
  ...
  const { agent, config, cleanup } = buildTestAgent({
    agentDeps: { initialMessages: primed },
  });
  try {
    const snap = agent.contextSnapshot();
    assert.equal(snap.model, config.llm.model);
    assert.equal(
      (snap.tools[0] as { function: { name: string } }).function.name,
      'run',
    );
    ...
    assert.equal(
      msgs.length,
      primed.length + 1,
      'system message + the one history',
    );
    assert.equal(msgs[0].role, 'system');
    assert.ok(
      typeof msgs[0].content === 'string' &&
        (msgs[0].content as string).length > 0,
      'system prompt built',
    );
    assert.match(
      msgs[0].content as string,
      /## Mind practice[\s\S]*Recorded is not promised/,
    );
    assert.doesNotMatch(
      msgs[0].content as string,
      /heartbeat digest|digest shows how long/,
    );
    ...
    for (const m of msgs) {
      assert.ok(
        !('channel' in m),
        'channel stamp must not reach the wire shape',
      );
    ...
    assert.ok(
      !('reasoning_content' in msgs[1]),
      'completed-turn reasoning is stripped',
    );
    assert.equal(
      msgs[3].reasoning_content,
      'live thinking',
      'open-chain reasoning survives',
    );
    ...
    list: (filter: MindListFilter = {}) =>
      items
        .filter((item) => {
          ...
          if (
            filter.ready &&
            (!(item.status === 'open' || item.status === 'inbox') ||
              item.effectiveStatus === 'blocked')
          )
            return false;
          if (filter.blocked && item.effectiveStatus !== 'blocked')
            return false;
          return true;
        })
...
test('formatMindFrontier: commitments and held thoughts stay distinct; bodies stay behind get()', () => {
  const items = [
    mindItem({
      id: 7,
      title: 'Ship it',
      body: 'private implementation body',
      status: 'in_progress',
      effectiveStatus: 'in_progress',
      createdBy: 'mcp:codex-worker-7',
    }),
    mindItem({
      id: 8,
      title: 'Wait honestly',
    ...
    mindItem({
      id: 9,
      title: 'A half thought',
      kind: 'idea',
      status: 'open',
      effectiveStatus: 'open',
    }),
  ];
  const card = formatMindFrontier(
    fakeMind(items, {
      active: 3,
      ready: 1,
      blocked: 0,
...
test('allowsMindFrontier: internal and home pass; social and unknown rooms fail closed', () => {
  const guilds = [
    { id: 'g-home', slug: 'home' },
    { id: 'g-social', slug: 'social' },
  ];
  const channels = {
    guildOf: (id: string) =>
      id === 'c-home' ? 'g-home' : id === 'c-social' ? 'g-social' : null,
  };
  ...
  let permission = retainMindFrontierPermission(
    true,
    'c-home',
    false,
    channels,
  ...
  permission = retainMindFrontierPermission(
    permission,
    'c-social',
    false,
    channels,
  ...
  assert.equal(
    permission,
    false,
    'a later home message cannot reopen a mixed social turn',
  );
});
...
test('contextSnapshot: wired Mind frontier is a final ephemeral request message', () => {
  ...
  const { agent, cleanup } = buildTestAgent({
    agentDeps: { mind: mind as unknown as MindService },
  });
  try {
...
test('contextSnapshot: huge inline image payloads are elided with a marker; history is untouched', () => {
  ...
  try {
    ...
    assert.ok(
      parts[1].image_url!.url.includes(
        '[elided by contextSnapshot: 20022 chars',
      ),
      'huge data URI is elided with an honest marker',
    );
...
test('elideLargeImageUrls: small parts and string content pass through unchanged (same object)', () => {
  ...
  assert.equal(
    elideLargeImageUrls(small),
    small,
    'no copy when nothing crosses the cap',
  );
