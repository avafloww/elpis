...
test('episode bootstrap validates scenario and run metadata without filesystem controls', () => {
  ...
  const resume = {
    events: [], sends: [], promptDigests: ['a'.repeat(64)], ingressDigests: ['b'.repeat(64)], dataSnapshotDigest: 'c'.repeat(64),
  };
  assert.deepEqual(parseEpisodeBootstrap({ type: 'bootstrap', scenario, run: parsed.meta, resume }).resume, resume);
  assert.throws(() => parseEpisodeBootstrap({ type: 'bootstrap', scenario, run: parsed.meta, resume: { ...resume, dataSnapshotDigest: 'not-a-digest' } }), /invalid_format/);
