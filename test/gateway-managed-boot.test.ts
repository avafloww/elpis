...
test('managed adapter refusal closes an authentically materialized runtime', async () => {
  const dataDirectory = fs.mkdtempSync(
    path.join(os.tmpdir(), 'gateway-adapter-boot-failure-'),
  );
  const config = managedConfig(dataDirectory, false, true);
  const events: string[] = [];
  let database: Database | undefined;
  let activeToken = '';
  let runtimeStore: GatewayLlmResidentStore | undefined;
  let resolvedRegistry: object | undefined;
  const wire = serializeLlmProxyCatalog({
    format: LLM_PROXY_FORMATS.catalog,
    revision: 1,
    models: [
      managedCatalogModel('team/classifier'),
      managedCatalogModel('team/compaction'),
      managedCatalogModel('team/main'),
    ],
  });
  try {
    await assert.rejects(
      createElpisRuntime({
        ...
        openDatabase: (root) => {
          events.push('database opened');
          database = trackedDatabase(events, root);
          const resident = createGatewayResidentStore(database);
          const bytes = (size: number) => Buffer.alloc(size, 7);
          resident.beginEnrollment({
            endpoint: 'https://gateway.example.com',
            displayName: 'Aster',
            grantToken: createEnrollmentCredential(bytes).token,
          });
          const pending = resident.read();
          resident.activateEnrollment({
            format: RESIDENT_CONTROL_FORMATS.enrollmentResult,
            instanceId: pending.instanceId,
            credentialId: pending.pendingCredentialId!,
            replayed: false,
          });
          activeToken = resident.activeNodeToken();
          events.push('resident active');
          return database;
        },
        createGatewayResidentStore: (opened) => {
          runtimeStore = createGatewayResidentStore(opened);
          return runtimeStore;
        },
        materializeGatewayConfig: async (candidate, options) => {
          assert.equal(options.store, runtimeStore);
          const resolved = await materializeGatewayConfig(candidate, options);
          assert.equal(isResolvedGatewayConfig(resolved), true);
          assert.equal(Object.isFrozen(resolved.llm.registry), true);
          assert.equal(Object.isFrozen(resolved.llm.registry.models), true);
          assert.equal(Object.isFrozen(resolved.llm.registry.targets), true);
          resolvedRegistry = resolved.llm.registry;
          return resolved;
        },
        gatewayLlmFetch: async (url, init) => {
          events.push('catalog fetched');
          assert.equal(
            url,
            `https://gateway.example.com${LLM_PROXY_PATHS.catalog}`,
          );
          assert.equal(
        ...
        fetchContextWindow: async (projected) => {
          assert.equal(isResolvedGatewayConfig(projected), true);
          if (!isResolvedGatewayConfig(projected))
            throw new Error('expected managed projection');
          assert.equal(projected.llm.registry, resolvedRegistry);
          assert.equal(Object.isFrozen(projected.llm.target), true);
          if (projected.llm.target.model === 'upstream-main') {
            assert.equal(
              projected.llm.target,
              projected.llm.registry.targets.main,
            );
            events.push('managed main context');
          } else {
            assert.equal(projected.llm.target.model, 'upstream-compaction');
            assert.equal(
              projected.llm.target,
              projected.llm.registry.targets.compaction,
            );
            events.push('managed compaction context');
          }
          return projected.llm.target.contextSize;
        },
        loadExtensions: async () => {
          events.push('extensions loaded');
          throw new Error('extensions must not run');
    ...
    assert.deepEqual(events, [
      ...
      'resident active',
      'catalog fetched',
      'managed main context',
      'managed compaction context',
      'database closed',
    ]);
    assert.ok(database);
    assert.equal(database.isOpen, false);
