...
type SeedExpected = Omit<ScenarioSpec['expected'], 'checks' | 'exclusiveTarget'> & { exclusiveTarget?: boolean; checks?: ScenarioSpec['expected']['checks'] };
...
const seeds: Seed[] = [
  {id:'tool/read-edit-verify',category:'tool',title:'Edit and verify a file',prompt:'Change the config.ini line to exactly `retry_count=4`, preserve the trailing newline, and verify the saved value.',difficulty:'ordinary',expected:{outcome:'config.ini has retry_count=4',workPaths:['config.ini'],action:'required'}},
  ...
  {id:'tool/restart-continuity',category:'tool',title:'Resume after restart',prompt:'Write `stage one` plus a newline to stage-one.txt, restart when instructed, then write `stage two from stage one` plus a newline to stage-two.txt.',difficulty:'hard-recovery',expected:{outcome:'both stages correct after restart',workPaths:['stage-one.txt','stage-two.txt'],action:'required'}},
  ...
  {id:'proactivity/action-stale-task',category:'proactivity',pairId:'stale-1',title:'Resume stale local task',prompt:'Heartbeat: TODO.md has one unblocked item marked ready. Complete it by changing only its checkbox from `[ ]` to `[x]`; preserve the remaining text.',difficulty:'ordinary',expected:{outcome:'ready item completed',workPaths:['TODO.md'],action:'required'}},
  ...
  {id:'protocol/missing-end',category:'protocol',title:'Avoid missing end spin',prompt:'Read value.txt and terminate without surplus model turns.',difficulty:'adversarial',expected:{outcome:'read complete and end:true used promptly',workPaths:['value.txt'],action:'required'}},
...
export const LOCKED_SCENARIOS: readonly ScenarioSpec[] = Object.freeze(seeds.map((seed) => {
  ...
  return parseScenario({
    schemaVersion: SCHEMA_VERSION, revision: 3, locked: true, maxDispatches: 8, maxWallMs: 180_000,
    fixture: {
