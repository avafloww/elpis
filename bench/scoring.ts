...
export function passesHardGates(gates: HardGates): boolean {
  return Object.values(gates).every(Boolean);
}
const clamp01 = (n: number): number => Math.max(0, Math.min(1, n));

export function mechanicalCategoryScore(
  record: RunRecord,
  category: Category,
): number {
  ...
  if (category === 'tool')
    return clamp01(
      1 -
        ...
        0.15 * m.failedCalls -
        0.2 * m.blockedCalls -
        0.2 * m.malformedCalls -
        0.15 * m.unchangedRetries -
        0.1 * m.duplicateWork,
    );
  if (category === 'proactivity')
    return clamp01(
      1 -
        universal -
        0.2 * m.postOutcomeDispatches -
        0.07 * m.surplusModelTurns -
        0.1 * m.duplicateWork,
    );
  if (category === 'protocol')
    return clamp01(
      1 -
        universal -
        0.2 * m.postOutcomeDispatches -
        0.25 * m.missingTerminalFlags -
        0.3 * m.failedTerminalFlags -
        0.1 * m.emptyTerminalCalls -
        0.12 * m.surplusModelTurns,
    );
  return clamp01(
    1 - universal - 0.12 * extraSends - 0.12 * m.surplusModelTurns,
  );
}
...
export function aggregateJudgePanel(
  ...
  for (const score of scores)
    (
      byCriterion.get(score.criterion) ??
      (byCriterion.set(score.criterion, []), byCriterion.get(score.criterion)!)
    ).push(score);
  const medians: Record<string, number> = {};
  const unstable: string[] = [];
  for (const [criterion, rows] of byCriterion) {
    if (rows.length !== 3 || new Set(rows.map((r) => r.family)).size < 2)
      throw new Error(
        `criterion ${criterion} requires exactly three judges spanning at least two families`,
      );
...
export function buildSuiteSummary(
  ...
  for (const run of runs) {
    ...
    if (run.judges?.length) {
      ...
      if (judged.length)
        value = passesHardGates(run.record.gates)
          ? value * (median(judged) / 4)
          : 0;
    }
    categoryValues[run.category].push(value);
  }
  const categoryScores = Object.fromEntries(
    Object.entries(categoryValues).map(([k, v]) => [
      k,
      v.length ? v.reduce((a, b) => a + b, 0) / v.length : 0,
    ]),
  );
  const weightedScore = (Object.keys(CATEGORY_WEIGHTS) as Category[]).reduce(
    (n, category) => n + categoryScores[category] * CATEGORY_WEIGHTS[category],
    0,
  );
  return {
    ...
    runIds: runs.map((r) => r.record.runId),
    categoryScores,
    weightedScore,
    hardGatePassRate: runs.length
      ? runs.filter((r) => passesHardGates(r.record.gates)).length / runs.length
      : 0,
    unstableCriteria,
    judgedCriteria,
    inconclusive: judgedCriteria > 0 && unstableCriteria / judgedCriteria > 0.1,
  };
}
...
export function compareSummaries(
  ...
  if (a.inconclusive || b.inconclusive)
    return {
      delta: b.weightedScore - a.weightedScore,
      verdict: 'inconclusive',
      reason: 'more than 10% of judged criteria are unstable',
    };
  ...
  if (Math.abs(delta) <= 0.03)
    return {
      delta,
      verdict: 'inconclusive',
      reason: 'difference is within the 0.03 repeatability bound',
    };
  return {
    delta,
    verdict: delta > 0 ? 'keep' : 'discard',
    reason: 'weighted benchmark score changed beyond the repeatability bound',
  };
}
