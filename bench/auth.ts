...
export async function authLogin(
  ...
  try {
    dir = ensurePrivateDir(path.join(root, 'auth'));
    ...
    if (provider === 'anthropic' || provider === 'anthropic-oauth') {
      ...
      const code = (await rl.question('Paste code#state: ')).trim();
      rl.close();
      if (!code) throw new Error('no authorization code entered');
      const credentials = await exchangeAnthropicCode(code, pkce, state);
      new OAuthStore(db, 'anthropic', refreshAnthropicToken).write(credentials);
      return;
    }
    if (provider === 'codex' || provider === 'codex-oauth') {
      const credentials = await loginOpenAICodexDevice({
        onCode(url, code) {
          stdout.write(`Open ${url} and enter ${code}\n`);
        },
        onProgress(message) {
    ...
    throw new Error(
      'auth login supports anthropic-oauth or codex-oauth; configure API keys directly for openai-compatible providers',
    );
  } finally {
