...
import {
  ...
  startEpisodeContainer,
  withContainerTimeout,
} from './docker.js';
...
import {
  episodeResumeStateSchema,
  type EpisodeResumeState,
  type EpisodeRunControl,
} from './bootstrap.js';
    ...
    llm: {
      ...
      baseUrl:
        ...
        (provider.provider_type === 'anthropic-oauth'
          ...
          : provider.provider_type === 'codex-oauth'
            ? 'https://chatgpt.com/backend-api'
            : ''),
      apiKey: provider.api_key ?? '',
      api: provider.api,
      reasoningEffort: provider.reasoning_effort ?? null,
...
export function oracleCode(scenario: ScenarioSpec, call = 1): string {
  ...
  const write = (file: string, content: string) => {
    ...
    statements.push(
      `if (!fs.existsSync(${JSON.stringify(file)}) || fs.readFileSync(${JSON.stringify(file)}, 'utf8') !== ${JSON.stringify(content)}) fs.writeFileSync(${JSON.stringify(file)}, ${JSON.stringify(content)})`,
    );
  };
  ...
  for (const check of scenario.expected.checks) {
    if (check.kind === 'file-equals') write(check.path, check.content);
      ...
      write(check.path, JSON.stringify(check.value) + '\n');
    else if (check.kind === 'path-exists') {
      if (check.type === 'directory')
        statements.push(
          `fs.mkdirSync(${JSON.stringify(check.path)}, { recursive: true })`,
        );
      else write(check.path, '');
    } else if (check.kind === 'path-absent')
      statements.push(
        `fs.rmSync(${JSON.stringify(check.path)}, { recursive: true, force: true })`,
      );
      ...
      for (const file of check.files)
        write(path.join(check.path, file), 'oracle\n');
    } else if (check.kind === 'send-includes')
      sendValues.push(
        ...(check.match === 'any' ? check.values.slice(0, 1) : check.values),
      );
  }
  ...
  const target = scenario.expected.targetChannel
    ? scenario.fixture.channels[scenario.expected.targetChannel]
    : undefined;
  if (target) {
    const recipient = scenario.expected.targetRecipient
      ? `@${scenario.expected.targetRecipient} `
      : '';
    const text =
      recipient +
      (sendValues.length > 0
        ? sendValues.join(' ')
        : `oracle: ${scenario.expected.outcome}`);
    statements.push(
      `await elpis.channel(${JSON.stringify(target)}).send(${JSON.stringify(text)})`,
    );
  }
  ...
  return {
    ...
    async complete() {
      ...
      const sandbox = sandboxSeed
        ? scenario.fixture.mind.find((item) => item.key === sandboxSeed.mindKey)
            ?.title
        : undefined;
      const wake =
        scenario.expected.decision === 'no-op'
          ? { auto: true }
          : { after: '1h' };
      const message = {
        ...
        tool_calls: [
          {
            ...
            function: {
              ...
              arguments: JSON.stringify({
                code,
                detail: 'Apply the oracle outcome',
                ...(sandbox ? { sandbox } : {}),
                wake,
              }),
            },
      ...
      stampGeneration(message, {
        ...
        apiEndpoint: 'https://oracle.elpisbench.invalid/v1/responses',
      });
      return {
        message,
        stripped: false,
    ...
    async summarize() {
      return 'Oracle summary preserving the completed fixture state and remaining request.';
    },
  };
}
...
function noToolBaselineLLM(scenario: ScenarioSpec): LLM {
  let calls = 0;
  return {
    model: 'elpisbench-no-tool-baseline',
    runTool: {} as LLM['runTool'],
    async complete() {
      calls++;
      if (calls <= scenario.maxDispatches)
        return {
          message: {
            role: 'assistant' as const,
            content: 'I cannot use tools, but I think this is done.',
          },
          stripped: false,
          usage: { prompt_tokens: 1, completion_tokens: 1, total_tokens: 2 },
        };
      ...
      return {
        message: {
          ...
          tool_calls: [
            {
              id: `baseline-stop-${calls}`,
              type: 'function' as const,
              function: {
                name: 'run',
                arguments:
                  '{"code":"","detail":"Stop the bounded baseline","wake":{"after":"1h"}}',
              },
            },
          ],
        },
...
function candidateLLM(serialized: string): LLM {
  ...
  const queue = (Array.isArray(raw) ? raw : [raw]) as Array<{
    content?: string;
    tool_calls?: {
      id: string;
      type: 'function';
  ...
  return {
    ...
    async complete() {
      ...
      const message = {
        ...
        ...(item.tool_calls
          ? {
              tool_calls: item.tool_calls.map((c) => ({
                ...c,
                function: {
                  ...c.function,
                  arguments:
...
export async function runScenario(
  ...
  opts: { oracle?: boolean; noToolBaseline?: boolean; candidate?: string } = {},
): Promise<RunRecord> {
  const provider = config.providers[providerName];
  if (!provider) throw new Error(`unknown provider ${providerName}`);
  const image = imageIdentity(config.image);
  const identity = opts.oracle
    ? { provider_type: 'openai-compatible', model: 'elpisbench-oracle' }
    : opts.noToolBaseline
      ? {
      ...
      : opts.candidate
        ? {
            provider_type: 'openai-compatible',
            model: 'grpo-candidate',
            candidate: contentDigest(opts.candidate),
          }
        : {
            ...provider,
            api_key: provider.api_key
              ? contentDigest(provider.api_key)
              : undefined,
          };
  ...
  const episodeRoot = ensurePrivateDir(
    path.join(config.data_directory ?? privateDataRoot(), 'episodes', digest),
  );
  ...
  prepareEpisodeMounts(
    ...
    resultDir,
    clockFile,
    scenario.fixture.clockAt ? new Date(scenario.fixture.clockAt) : new Date(),
  );
  const runId = digest;
  ...
  const llm = opts.oracle
    ? oracleLLM(scenario)
    : opts.noToolBaseline
      ? noToolBaselineLLM(scenario)
      : opts.candidate
        ? candidateLLM(opts.candidate)
        : providerLLM(provider, config.data_directory ?? privateDataRoot());
  const gateway: CompletionGateway = {
    complete: (messages) =>
      llm.complete(messages as Parameters<LLM['complete']>[0]),
    summarize: (text) => llm.summarize(text),
    async resetSession() {
      llm.resetSession?.();
    },
    async advanceClock(ms) {
      advanceClockFile(clockFile, ms);
    },
  };
  ...
  for (let replacements = 0; ; replacements++) {
    if (replacements > 1)
      throw new Error('episode requested more than one simulated restart');
    const child = startEpisodeContainer({
      image: config.image,
      workDir,
      resultDir,
      clockFile,
      name,
      limits: { timeoutMs: scenario.maxWallMs + 30_000 },
    });
    raw = await withContainerTimeout(
      child,
      name,
      serveGateway(child, gateway, {
        type: 'bootstrap',
        scenario,
        run: runControl,
        ...(resume ? { resume } : {}),
      }),
      scenario.maxWallMs + 30_000,
    );
    if (!(
      typeof raw === 'object' &&
      raw !== null &&
      (raw as { restart?: unknown }).restart === true
    ))
      break;
    resume = episodeResumeStateSchema.parse(
      (raw as { resume?: unknown }).resume,
    );
    llm.resetSession?.();
  }
