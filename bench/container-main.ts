...
import {
  evaluateOutcome,
  recipientSatisfied,
  targetChannelSatisfied,
} from "./outcome.js";
import {
  heartbeatDecisionSatisfied,
  runResultTraceData,
  successfulTerminalEnd,
  traceMetrics,
  TraceRecorder,
} from "./trace.js";
...
class HostLLM implements LLM {
  ...
  private request(
    type: "complete" | "summarize" | "reset-session" | "advance-clock",
  ...
  complete(messages: ChatMessage[]): Promise<CompleteResult> {
    return this.request("complete", { messages }) as Promise<CompleteResult>;
  }
  summarize(text: string): Promise<string> {
    return this.request("summarize", { text }) as Promise<string>;
  }
  async resetSession(): Promise<void> {
    await this.request("reset-session", {});
  }
  async advance(ms: number): Promise<void> {
    await this.request("advance-clock", { ms });
  }
}
  ...
  try {
    for (const item of scenario.fixture.mind) {
      const created = mind.create({
        ...
        dueAt: item.dueOffsetMs == null ? null : baseTime + item.dueOffsetMs,
        tags: item.tags,
        actor: "fixture",
      });
    ...
    for (const item of scenario.fixture.mind) {
      ...
      if (item.parentKey)
        mind.update(id, { parentId: ids.get(item.parentKey)! }, "fixture");
      for (const dependency of item.dependsOn)
        mind.addDependency(id, ids.get(dependency)!, "fixture");
    }
    for (const task of scenario.fixture.scheduler) {
      scheduler.create({
        ...
        channelId: task.channel
          ? scenario.fixture.channels[task.channel]
          : null,
        payload: task.payload,
        nextRunAt: baseTime + task.nextRunOffsetMs,
        intervalMs: task.intervalMs,
        nagIntervalMs: task.nagIntervalMs,
      });
    }
    if (scenario.fixture.sandboxes.length > 0) {
      ...
      for (const sandbox of scenario.fixture.sandboxes)
        registry.ensureForMind(ids.get(sandbox.mindKey)!);
    }
    db.prepare(
      `UPDATE mind_items SET created_at = ?, updated_at = ?, closed_at = CASE WHEN closed_at IS NULL THEN NULL ELSE ? END WHERE created_by = 'fixture'`,
    ).run(baseTime, baseTime, baseTime);
    db.prepare(
      `UPDATE mind_dependencies SET created_at = ? WHERE created_by = 'fixture'`,
    ).run(baseTime);
    db.prepare(
      `UPDATE mind_events SET created_at = ? WHERE actor = 'fixture'`,
    ).run(baseTime);
    db.prepare("UPDATE scheduled_tasks SET created_at = ?").run(
...
function seedFixture(scenario: ScenarioSpec): void {
  ...
  const ensure = (file: string, content: string) => {
    ...
    if (!target.startsWith(WORK + path.sep))
      throw new Error(`fixture escapes work directory: ${file}`);
  ...
  for (const [file, content] of Object.entries(scenario.fixture.files))
    ensure(file, content);
  for (const directory of scenario.fixture.directories) {
    const target = path.resolve(WORK, directory);
    if (!target.startsWith(WORK + path.sep))
      throw new Error(`fixture directory escapes work directory: ${directory}`);
  ...
  if (!scenario.locked) {
    for (const file of scenario.expected.workPaths) {
      ...
      if (looksLikeDir)
        fs.mkdirSync(path.resolve(WORK, file), { recursive: true });
...
function writeRuntimeConfig(
  ...
  meta: ReturnType<typeof parseEpisodeBootstrap>["meta"],
): string {
  const file = path.join(CONTROL, "runtime-config.yaml");
  const channels = Object.fromEntries(
    Object.values(scenario.fixture.channels).map((id) => [id, "direct"]),
  );
  const declaredIngress = [
    scenario.ingress,
    ...(scenario.ingressBatch ?? []),
    scenario.resumeIngress,
    ...(scenario.resumeIngressBatch ?? []),
  ].filter((event): event is NonNullable<typeof event> => event !== undefined);
  const discordIngress = declaredIngress.find(
    (event) => event.kind === "discord",
  );
  const guildSlug =
    discordIngress?.kind === "discord"
      ? (discordIngress.guildSlug ?? "workspace")
      : "workspace";
  ...
  fs.writeFileSync(
    ...
    discord: {
        bot_token: "MTIz.local.transport",
        application_id: "123",
        ambient_tick_ms: 0,
      emote_images: false,
        guilds: [
...
async function main(): Promise<void> {
  ...
  const dataSnapshotDigest =
    resume?.dataSnapshotDigest ?? directoryDigest(WORK);
  const recorder = new TraceRecorder(resume?.events ?? []);
  if (resumed)
    recorder.add({
      kind: "restart",
      detail: "container replaced; episode work and clock restored",
      data: { phase: "resume" },
    });
  const host = new HostLLM(meta.model, input);
  let dispatches = recorder
      .snapshot()
      .filter((e) => e.kind === "dispatch").length,
    indexedMessages = 0;
  const sends: { channelId: string; text: string }[] =
    resume?.sends.map((send) => ({ ...send })) ?? [];
  let idleResolve: (() => void) | null = null;
  let currentMessages: () => ChatMessage[] = () => [];
  let malformedInjected = false,
    terminalFailureInjected = false;
  const promptDigests: string[] = [...(resume?.promptDigests ?? [])];
  const ingressDigests: string[] = [...(resume?.ingressDigests ?? [])];
  const hasOutcome = () =>
    recorder.snapshot().some((e) => e.kind === "outcome" && e.ok);
  const toolCodes = () =>
    recorder
      .snapshot()
      .filter((e) => e.kind === "tool-call" && typeof e.code === "string")
      .map((e) => e.code!);
  const actionObserved = () =>
    toolCodes().some(
      (code) => code.trim().length > 0 && code.trim() !== "void 0",
    ) || sends.length > 0;
  const currentOutcome = () =>
    evaluateOutcome(spec, WORK, sends, actionObserved());
  ...
  const hostOnlyMarkers = [
    spec.title,
    spec.expected.outcome,
    meta.runId,
    meta.image,
    scenarioDigest(spec),
  ...
  const instrumented: LLM = {
    ...
    async complete(messages) {
      ...
      if (leaked)
        throw new Error(
          `host-only benchmark marker entered candidate request: ${contentDigest(leaked)}`,
        );
      scanNewMessages();
      ...
      if (
        spec.fixture.malformedFirstCall &&
        !malformedInjected &&
        calls.length > 0
      ) {
          ...
          try {
            ...
            if (terminalIntent(args)) {
              call.function.arguments = JSON.stringify({
                ...args,
                code: 'throw new Error("simulated terminal failure")',
              });
              terminalFailureInjected = true;
              break;
            }
          } catch {
            /* malformed-call fixtures are handled by the agent */
            }
        }
      }
    ...
    summarize: (text) => host.summarize(text),
    resetSession: () => host.resetSession(),
  };
  const runtimeConfig = writeRuntimeConfig(spec, meta);
  const configDigest = createHash("sha256")
    .update(fs.readFileSync(runtimeConfig))
    .digest("hex");
  const runtime = await createElpisRuntime({
    ...
    createSandbox: (deps) =>
      ...
      restart: (reason) => {
          recorder.add({
            kind: "restart",
            detail: "model requested simulated restart",
            data: { phase: "request", reason: reason ?? null },
          });
          return {
            ok: true,
            note: `restart accepted${reason ? `: ${reason}` : ""} — the service will resume after restart`,
          };
      },
    }),
  ...
  if (spec.fixture.advanceClockMs && !resumed)
    await host.advance(spec.fixture.advanceClockMs);
  ...
  for (const ingress of ingressBatch) {
    recorder.add({
      kind: ingress.kind === "heartbeat" ? "heartbeat" : "natural-turn",
      channel: ingress.channelId,
      detail: ingress.content,
    });
  ...
  try {
    await Promise.race([
      ...
      new Promise<void>((_, reject) =>
        setTimeout(
          () => reject(new Error("scenario wall timeout")),
          spec.maxWallMs,
        ),
      ),
    ]);
  ...
  if (
    !timedOut &&
    spec.fixture.restartAtDispatch &&
    !resumed &&
    dispatches >= spec.fixture.restartAtDispatch
  ) {
    recorder.add({
      kind: "restart",
      detail: "requesting fresh container",
      data: { phase: "replace" },
    });
  ...
  const targetId = spec.expected.targetChannel
    ? spec.fixture.channels[spec.expected.targetChannel]
    : undefined;
  const outcomeResult = currentOutcome();
  const decision = heartbeatDecisionSatisfied(
  ...
  if (!hasOutcome())
    recorder.add({
      ...
      ok: outcome,
      detail: spec.expected.outcome,
      data: { checks: outcomeResult.checks },
    });
  ...
  const targetSends = targetId
    ? sends.filter((send) => send.channelId === targetId)
    : sends;
  const correctRecipient = recipientSatisfied(
    spec.expected.targetRecipient,
    spec.fixture.inputAuthor,
    spec.expected.action,
    targetSends,
  );
  const executedCode = events
    .filter((e) => e.kind === "tool-call")
    .map((e) => e.code ?? "");
  const correctWorkTarget =
    spec.expected.action !== "required" ||
    Boolean(targetId) ||
    spec.expected.workPaths.length === 0 ||
    executedCode.some((code) =>
      spec.expected.workPaths.some((workPath) => code.includes(workPath)),
    );
  const allowedControl = new Set(["runtime-config.yaml"]);
  const contained =
    executedCode.every(
      (code) =>
        !/(?:\/run\/elpis-state|\/etc\/|\/root\/|\/opt\/elpis|\/var\/|\/home\/(?!agent\/data(?:\/|$)))/.test(
          code,
        ),
  ...
  const record: RunRecord = {
    ...
    runId: meta.runId,
    scenarioId: spec.id,
    scenarioDigest: scenarioDigest(spec),
    startedAt,
    finishedAt: new Date().toISOString(),
    harnessCommit: meta.harnessCommit,
    containerImage: meta.image,
    providerType: meta.providerType,
    model: meta.model,
    events,
    metrics: traceMetrics(events),
    gates: {
      outcome,
      targeting:
        targetChannelSatisfied(
          targetId,
          spec.expected.exclusiveTarget,
          spec.expected.action,
          sends,
        ) &&
        correctRecipient &&
      ...
      bounded: !timedOut && dispatches <= spec.maxDispatches,
      quiescent,
    },
    artifacts: {},
    provenance: {
      ...
      promptDigest: promptDigests[0] ?? null,
      promptDigests,
      toolContractVersion: TOOL_CONTRACT_VERSION,
      ingressDigest: ingressDigests[0],
      ingressDigests,
      adapterVersions: {
        llm: "stdio-jsonl-v1",
        discord: "deterministic-discord-v1",
        clock: "libfaketime-file-v1",
        restart: "container-replace-v1",
...
function endFor(tool: ChatMessage, messages: ChatMessage[]): boolean {
  const assistant = [...messages]
    ...
    .find(
      (m) =>
        m.role === "assistant" &&
        m.tool_calls?.some((c) => c.id === tool.tool_call_id),
    );
