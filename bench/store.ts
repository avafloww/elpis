...
export function privateDataRoot(env = process.env): string {
  return path.resolve(
    env.ELPISBENCH_DATA_DIR ??
      path.join(os.homedir(), '.local', 'share', 'elpisbench'),
  );
}
...
export function artifactPath(
  ...
  if (!/^[a-z0-9-]+$/.test(kind) || !/^[a-f0-9]{64}$/.test(digest))
    throw new Error('invalid content-addressed artifact key');
