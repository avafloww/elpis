...
export function serveGateway(
  ...
  bootstrap: EpisodeBootstrap,
): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const rl = readline.createInterface({ input: child.stdout });
    writeJsonLine(child.stdin, bootstrap);
    const respond = (response: GatewayResponse) =>
      writeJsonLine(child.stdin, response);
    rl.on('line', async (line) => {
      ...
      if (request.type === 'episode-result') {
        resolve(request.result);
        return;
      }
      if (request.type === 'episode-restart') {
        resolve({ restart: true, resume: request.resume });
        return;
      }
      if (request.type === 'episode-error') {
      ...
      try {
        ...
        if (request.type === 'complete')
          value = await gateway.complete(request.messages);
        else if (request.type === 'summarize')
          value = await gateway.summarize(request.text);
        else if (request.type === 'reset-session')
          value = await gateway.resetSession();
        else if (request.type === 'advance-clock')
          value = await gateway.advanceClock(request.ms);
    ...
    child.stderr.on('data', (chunk) =>
      process.stderr.write(`[container] ${chunk}`),
    );
    child.once('error', reject);
    child.once('exit', (code) => {
      if (code !== 0)
        reject(new Error(`ElpisBench container exited with code ${code}`));
    });
  });
}
