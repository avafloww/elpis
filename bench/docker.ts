...
export interface DockerLimits { memoryMb?: number; cpus?: number; pids?: number; timeoutMs?: number }
export interface DockerEpisodeOptions {
  image: string; workDir: string; resultDir: string; clockFile: string; name: string; limits?: DockerLimits;
}
...
export function dockerRunArgs(opts: DockerEpisodeOptions): string[] {
  ...
  return [
    'run', '--rm', '--interactive', '--name', opts.name,
    '--read-only', '--network', 'none', '--cap-drop', 'ALL', '--security-opt', 'no-new-privileges:true',
    '--user', `${uid}:${gid}`, '--pids-limit', String(limits.pids ?? 128), '--memory', `${limits.memoryMb ?? 1024}m`, '--cpus', String(limits.cpus ?? 1),
...
export function prepareEpisodeMounts(workDir: string, resultDir: string, clockFile: string, initialClock = new Date()): void {
...
export function advanceClockFile(clockFile: string, ms: number): void {
  if (!Number.isFinite(ms) || ms < 0 || ms > 14 * 24 * 60 * 60 * 1000) throw new Error('clock advance must be between 0 and 14 days');
  const raw = fs.readFileSync(clockFile, 'utf8').trim();
  const parsed = /^@(\d{4}-\d{2}-\d{2}) (\d{2}:\d{2}:\d{2})$/.exec(raw);
  const timestamp = parsed ? Date.parse(`${parsed[1]}T${parsed[2]}Z`) : Number.NaN;
  if (!Number.isFinite(timestamp)) throw new Error('invalid benchmark clock file');
  fs.writeFileSync(clockFile, `${formatFaketime(new Date(timestamp + ms))}\n`, { mode: 0o600 });
}

export function startEpisodeContainer(opts: DockerEpisodeOptions): ChildProcessWithoutNullStreams {
  return spawn('docker', dockerRunArgs(opts), { stdio: ['pipe', 'pipe', 'pipe'], env: process.env });
}
...
export async function withContainerTimeout<T>(child: ChildProcessWithoutNullStreams, name: string, promise: Promise<T>, timeoutMs: number): Promise<T> {
  let timer: NodeJS.Timeout;
  try {
    return await Promise.race([promise, new Promise<T>((_, reject) => { timer = setTimeout(() => reject(new Error(`episode timeout after ${timeoutMs}ms`)), timeoutMs); })]);
  } finally {
    clearTimeout(timer!);
