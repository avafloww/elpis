...
function resolveAttachment(
  ...
  if (target === root || !target.startsWith(root + path.sep))
    throw new Error(
      `ingress attachment escapes work directory: ${attachment.path}`,
    );
  const realTarget = fs.realpathSync(target);
  if (!realTarget.startsWith(root + path.sep))
    throw new Error(
      `ingress attachment escapes work directory through symlink: ${attachment.path}`,
    );
  const stat = fs.statSync(realTarget);
  if (!stat.isFile())
    throw new Error(`ingress attachment is not a file: ${attachment.path}`);
  return {
    url: attachment.url,
    ...
    size: stat.size,
    ...(attachment.inlineText !== undefined
      ? { inlineText: attachment.inlineText }
      : {}),
  };
}
...
function resolveDeclared(
  ...
  const empty = {
    ...
    forwarded: null,
    mentions: [] as string[],
    attachments: [] as InboundMessageAttachment[],
  };
  ...
  if (ingress.kind === 'discord') {
    ...
    if (!channelId)
      throw new Error(
        `production ingress references unknown channel: ${ingress.channel}`,
      );
    const policyChannelId = ingress.policyChannel
      ? spec.fixture.channels[ingress.policyChannel]
      : channelId;
    if (!policyChannelId)
      throw new Error(
        `production ingress references unknown policy channel: ${ingress.policyChannel}`,
      );
    return {
      id,
      channelId,
      channelName: ingress.channelName ?? ingress.channel,
      author: ingress.author,
      authorId: ingress.authorId ?? derivedAuthorId(ingress.author),
      ...
      replyTo: ingress.replyTo
        ? {
            ...ingress.replyTo,
            authorId:
              ingress.replyTo.authorId ??
              derivedAuthorId(ingress.replyTo.author),
          }
        : null,
      forwarded: ingress.forwarded,
      mentions: ingress.mentions,
      attachments: ingress.attachments.map((attachment) =>
        resolveAttachment(workRoot, attachment),
      ),
      guildId: 'workspace-guild',
      guildSlug: ingress.guildSlug ?? 'workspace',
      ...(ingress.bot !== undefined ? { bot: ingress.bot } : {}),
      wakeClass: ingress.wakeClass,
      policyChannelId,
      kind: 'discord',
    };
  }
  if (ingress.kind === 'scheduler') {
    const channelId = ingress.channel
      ? spec.fixture.channels[ingress.channel]
      : INTERNAL_CHANNEL_ID;
    if (!channelId)
      throw new Error(
        `production scheduler ingress references unknown channel: ${ingress.channel}`,
      );
    return {
      id,
      channelId,
...
function declaredBatch(
  spec: ScenarioSpec,
  resumed: boolean,
): CandidateIngressSpec[] {
  if (spec.track !== 'production') return [];
  const batch = resumed
    ? (spec.resumeIngressBatch ??
      (spec.resumeIngress ? [spec.resumeIngress] : undefined))
    : (spec.ingressBatch ?? (spec.ingress ? [spec.ingress] : undefined));
  if (!batch)
    throw new Error(
      resumed
        ? 'production restart requires explicit resume ingress'
        : 'production scenario requires explicit ingress',
    );
  return batch;
}
...
export function resolveCandidateIngressBatch(
  ...
  if (spec.track === 'production') {
    ...
    return declaredBatch(spec, resumed).map((ingress, index) =>
      resolveDeclared(spec, ingress, index, phase, workRoot),
    );
  }
  ...
  if (spec.fixture.heartbeat)
    return [
      {
        id: `micro-heartbeat-${resumed ? 'resume' : 'initial'}`,
        channelId: INTERNAL_CHANNEL_ID,
        channelName: 'heartbeat',
        author: 'agent',
        authorId: 'agent',
  ...
  const channelName =
    spec.fixture.inputChannel ?? Object.keys(spec.fixture.channels)[0];
  const channelId = spec.fixture.channels[channelName];
  if (!channelId)
    throw new Error(`unknown fixture input channel: ${channelName}`);
  const author = spec.fixture.inputAuthor ?? 'human';
  return [
    {
      id: `micro-discord-${resumed ? 'resume' : 'initial'}`,
      channelId,
      channelName,
      author,
      authorId: spec.fixture.inputAuthor
        ? derivedAuthorId(author)
        : 'person-human',
      content,
      createdAt,
      replyTo: null,
...
export function resolveCandidateIngress(
  spec: ScenarioSpec,
  resumed: boolean,
  workRoot = '/home/agent/data',
): ResolvedCandidateIngress {
  ...
  if (batch.length !== 1)
    throw new Error(
      `candidate ingress batch has ${batch.length} events; use resolveCandidateIngressBatch`,
    );
  return batch[0];
}
