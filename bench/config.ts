...
const providerSchema = z.object({
  ...
  api: z.enum(['auto', 'responses', 'chat']).default('auto'),
...
const configSchema = z.object({
  ...
  generator_provider: z.string().optional(),
  ...
  judges: z
    .array(
      z.object({
        ...
        provider: z.string(),
        family: z.string(),
        teacher_pool: z.boolean().default(false),
      }),
    )
    .length(3),
  image: z.string().default('elpisbench:latest'),
  concurrency: z.number().int().min(1).max(16).default(3),
  allow_private_input: z.boolean().default(false),
  data_directory: z.string().optional(),
});
export type BenchConfig = z.infer<typeof configSchema>;
export type ProviderConfig = z.infer<typeof providerSchema>;

export function configPath(arg?: string): string {
  return path.resolve(
    arg ??
      process.env.ELPISBENCH_CONFIG ??
      path.join(privateDataRoot(), 'config.yaml'),
  );
}
export function loadBenchConfig(arg?: string): BenchConfig {
  ...
  try {
    ...
    throw new Error(
      `unable to read ElpisBench config ${file}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  for (const name of [
    ...
    if (!config.providers[name])
      throw new Error(
        `ElpisBench config references unknown provider ${JSON.stringify(name)}`,
      );
  }
  for (const [name, provider] of Object.entries(config.providers)) {
    if (
      ...
      throw new Error(
        `provider ${name}: openai-compatible requires base_url and api_key`,
      );
    if (
      ...
      throw new Error(
        `provider ${name}: codex-oauth is pinned to https://chatgpt.com/backend-api`,
      );
    if (provider.provider_type === 'codex-oauth' && provider.api === 'chat')
      throw new Error(
        `provider ${name}: codex-oauth does not support api: chat`,
      );
  }
  return config;
}
