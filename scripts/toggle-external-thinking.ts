...
export function toggleExternalThinking(configText: string): {
  text: string;
  before: boolean;
  after: boolean;
} {
  const parsed = parseYaml(configText) as {
    llm?: { external_thinking?: unknown };
  } | null;
  ...
  if (matches.length !== 1) {
    throw new Error(
      `expected exactly one external_thinking YAML line, found ${matches.length}`,
    );
  }

  const after = !before;
  const text = configText.replace(pattern, `$1${after}$3`);
  const verified = parseYaml(text) as {
    llm?: { external_thinking?: unknown };
  } | null;
  if (verified?.llm?.external_thinking !== after) {
    throw new Error(
      'updated config did not parse back to the intended external-thinking state',
    );
  }
...
function usage(): never {
  console.error(
    'usage: npm run toggle-external-thinking -- [--config /path/to/config.yaml] [--no-restart]',
  );
  process.exit(2);
}
...
async function main(): Promise<void> {
  ...
  const onDisk = parseYaml(fs.readFileSync(configPath, 'utf8')) as {
    llm?: { external_thinking?: unknown };
  } | null;
  ...
  const result = spawnSync('systemctl', ['--user', 'restart', SERVICE_UNIT], {
    stdio: 'inherit',
  });
  if (result.error) throw result.error;
  if (result.status !== 0)
    throw new Error(
      `failed to restart ${SERVICE_UNIT} (exit ${result.status ?? 'unknown'})`,
    );
