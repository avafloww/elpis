...
At episode start, Elpis freezes every selected package into the private mode-0600 episode trace before any effect. Only each `SKILL.md` body enters the initial motor context; the package manifest stays resident-side. Skill authors must cite every auxiliary file by its exact `skill:<name>/<relative-path>` handle inside that body, so no separate discovery manifest enters Holo context. A selected main body explicitly tells the motor it is already loaded and must not be sourced or reread.
