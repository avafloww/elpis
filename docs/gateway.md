...
The Gateway process composes the HTTP plane with a private broker over the same hardened SQLite store. OpenAI-compatible targets use their immutable canonical HTTPS base URL. Anthropic OAuth targets admit only the versioned `messages` grammar. Codex OAuth admits only the versioned `codex/responses`, `codex/models`, and `models` grammars and always dispatches to fixed `https://chatgpt.com/backend-api` paths; stored grammar strings are version assertions, never URL templates. A Codex target also requires a visible-ASCII account ID that can be sent by the transport, and that identity is part of the immutable target snapshot. Provider credentials have no public readback or callback-lease surface.

Each admitted request receives a dispatch-local OAuth credential snapshot; no reusable secret reader or callback lease escapes the broker. Only the in-flight refresh promise is shared per immutable credential for process-wide single-flight. Refresh may replace token material but any provider-reported immutable identity must match the authorized snapshot; omitted identity is preserved rather than inferred. Refresh persists through the exact SQLite secret revision without holding a transaction across a provider call, and application shutdown drains refresh and CAS work before closing the database. Rejected-response body cleanup is best-effort and observed but never awaited before abort checks, refresh, or retry. A stale authorized revision or lost refresh CAS fails that dispatch rather than adopting replacement secrets; a later admission may read the winner. Changing a target generation or revoking its grant affects later admissions, not a request already admitted to network work.

A Gateway-managed resident keeps provider-specific shaping, streaming parsers, tool handling, usage accounting, and replay rules in the resident. The resident dispatches only the catalog-selected serialized request through its authenticated Gateway client: OpenAI Responses and Chat, Anthropic Messages, and Codex Responses are supported without local provider credentials or OAuth reads. SDK retries and cross-surface fallbacks are disabled so one provider attempt maps to one Gateway admission. Abort signals propagate into that dispatch. Worker and secretary Pods use their token-bound resident completion brokers and never receive the Gateway client or resolved registry authority.
...
Each model head identifies one immutable target generation. Reapplying the same canonical target is a no-op; changing its credential identity, base URL, provider model, allowed routes, wire grammar, or declared capabilities creates a new generation and removes every prior instance grant for that model. Disabling a model also removes its grants, and enabling it again requires another generation. An instance catalog contains only its exact current grants, sorted and validated by the shared Gateway LLM protocol. Model references retain the producer's `localeCompare` ordering (including underscore/dash ordering); the codec invokes a module-captured comparator. Grants never carry forward implicitly.
