...
A run may make at most four queries and submit at most 128 KiB of normalized query input. Only own enumerable string properties participate in query options and schemas; non-enumerable and symbol members are inert and ignored. Direct or prototype proxies, enumerable accessors, and unknown enumerable options are rejected before any provider call, and enumeration stops at fixed candidate/node bounds. Each prompt is limited to 64 KiB, each optional schema to 16 KiB, combined provider input to 80 KiB, and returned text to 64 KiB. OpenAI-compatible and Anthropic calls request at most 4096 output tokens. The Codex subscription transport rejects caller-supplied output-token caps, so that path is bounded instead by a 64 KiB visible-output abort and the same 120-second wall deadline. All paths enforce the visible-output and deadline bounds even if provider cancellation is not cooperative.

Schema mode appends a JSON-only instruction, parses exact JSON, and validates locally with Ajv without coercion, defaults, or property removal. It first clones and deep-freezes the own-enumerable data projection of plain schema objects without executing proxies, accessors, `toJSON`, functions, or inherited behavior. It accepts a bounded JSON Schema draft-07 subset: all reference keywords and regex-bearing `pattern`/`patternProperties` are rejected, and `format` is treated as an annotation rather than enforced. Parsed model JSON is limited to depth 64 and 4096 total values before validation and freezing. Invalid model output throws; it is never repaired silently.
...
Every resident sandbox run captures one opaque outbound origin before executing: the exact active `mentions`-turn authorization, an explicit unscoped marker, or a permanently denied marker when an active mention cannot produce a valid authorization. The scope includes a process-local Agent-turn identity and source channel. `AsyncLocalStorage` retains it across awaits and detached-future continuation, including the automatic typing pause/resume around `elpis.sleep`. A late send, explicit typing effect, or sleep resume therefore cannot discard an expired mention origin, retarget another channel, alter a later turn's sleep depth, or inherit its authorization; direct and social runs remain unscoped only within their originating turn.
