...
Admission requires the estimated input plus output reserve to fit the selected model's context window. An over-budget input records and logs an observable estimated-budget failure before any model request or quality retry. The rejected real fold is one completed failed cycle: the agent alerts on the normal failed-cycle cadence and latches automatic retry backoff immediately, including when an idle manual request has no later loop checkpoint. The failure does not move the selected fold boundary, tighten the serialization cap, discard additional history, or produce a replacement summary, so the original history remains live. A skip because there is no non-trivial prefix to fold is not a failed cycle. Without `summaryInputBudget`, behavior is unchanged.
