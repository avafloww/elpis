...
The bundled `codex-voice-host` helper cannot directly bridge Discord audio: native media stays inside that child process and opens physical microphone and speaker devices; only SDP, mute/suppression controls, and peak telemetry cross its control pipe. The harness now includes an inert, bounded JSONL signaling broker and a harness-owned one-call WebRTC media child. The child uses Werift with native Opus for 24 kHz mono PCM, carries opaque Realtime events over one ordered data channel, persists no raw audio, has no device or credential access, and exits after one call. The broker keeps IPC open until a truthful close receipt and observed child exit. Bounded signal escalation contains a failed graceful shutdown but is still reported as abnormal.

### Inert WebRTC response correlation

A strict adapter now translates the broker's ordered `oai-events` and bare decoded PCM into the existing resident-owned `VoiceSession` event contract. It permits one client-managed `appendSpeech` handoff at a time and binds the first valid `response.created` to the exact locally pending request token; that token is deliberately not sent to Codex. Response and output-item IDs remain immutable tombstones for the call. Generic output items claim their IDs, but only an assistant message containing official `output_audio` content may own bare PCM; item IDs carried by audio-start, transcript, or audio-done events remain references until that typed item object arrives. Unsolicited responses, reused identities, conflicting IDs, malformed correlation events, bounded pre-identity audio overflow, and audio that cannot be attributed after a stopped or cleared boundary close the adapter. Unknown call-level telemetry remains inert.

Provider `response.done` records generation status but does not by itself report successful delivery. Only the matching owned `output_audio_buffer.stopped(response_id)`, together with completed generation, emits the successful terminal event. The existing session then waits for the local player to drain before recording `played`. A matching `cleared` retires interrupted media. PCM already in flight after a cancelled or failed generation is discarded only until that owner reaches its exact stopped or cleared media terminal; PCM after the terminal remains fatal. Because cancellation events and the next `appendSpeech` use different transports, a cancelled or failed response must reach an exact `stopped` or `cleared` media terminal before another handoff can cross; cancellation callbacks cannot reenter between `response.cancel` and the global buffer clear. An active-generation cancellation carries a bounded event ID and ignores only a matching `response_cancel_not_active` race. When completed generation is already known, cancellation skips the futile response cancel and clears only the output buffer. Either path makes cancellation dominate a later successful terminal. If the response-cancel send itself reentrantly delivers the exact stopped or cleared terminal, the adapter skips the otherwise subsequent global clear so no uncovered clear can race with the next handoff. Compatible late done, transcript-terminal, stopped, and cleared events for a retired response are inert and cannot affect a newer response; post-terminal PCM before a newer exact response binds is a protocol failure. Finalized ingress remains ordered, and deleting a consumed input sends `conversation.item.delete` so provider conversation state does not silently retain it.

An interruption before `response.created` cannot identify a concrete provider response through the current `VoiceSession` transport interface. The adapter therefore cannot close at the instant of that interruption. It preserves safety by retaining the unresolved first handoff and refusing/closing on any later speech attempt; a delayed creation can never bind the later request.

No production factory invokes this subscription path yet: it is not wired to `/join`, starts no app-server or media process, and changes no credential or provider behavior. Before enabling the path, verify client-managed handoff ownership and subscription-backed media behavior in a real Discord call. The public API bridge remains useful groundwork, not evidence that subscription voice or human-call acceptance is complete.
