...
Discord Opus is decoded into PCM16 mono at 24 kHz. Semantic VAD identifies utterance boundaries, with automatic responses disabled. Finalized transcriptions enter ordinary ingress in audio commit order with `source="voice"` provenance. Partial transcripts never enter durable history or invoke tools. ASR may make mistakes; it is not an exact record of sound.
...
Each authored speech request carries a unique token echoed in response metadata; only that exact request may bind a response for playback. Delayed or unsolicited responses cannot claim another reply.
