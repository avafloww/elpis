...
No production factory invokes this subscription path yet: it is not wired to `/join`, starts no app-server or media process, and changes no credential or provider behavior. WebRTC delivers output audio on a remote media track while response and item identity arrive separately as control events, so bounded correlation with the existing resident-owned voice session still must land before activation. Before enabling the path, verify that client-managed handoffs prevent autonomous Codex turns, preserve ordered resident ingress and explicit speech receipts, and survive a real Discord call. The public API bridge remains useful groundwork, not evidence that subscription voice or human-call acceptance is complete.
