...
1. Call `mind_discover` while repository/task context is fresh, or `mind_context` for an assigned item. Discovery ranks relevant ready tasks from title, body, tags, and recent comments.
...
5. Call `mind_block` to record a blocker, release the lease, and move the item to `waiting`; call `mind_release` to return unfinished work to `open`.
...
Expired claims are returned to open work with a visible system comment and audit event. Claim, status, comment, and lifecycle event changes are committed atomically.

## Tool groups

- discovery/read: `mind_list`, `mind_get`, `mind_ready`, `mind_graph`, `mind_discover`, `mind_context`;
- task lifecycle: `mind_claim`, `mind_renew`, `mind_log`, `mind_block`, `mind_release`, `mind_finish`;
- correspondence: `mind_ask`, `mind_await`, `mind_message`, `mind_comment`;
- graph/metadata: `mind_create`, `mind_update`, `mind_link`, `mind_unlink`.
