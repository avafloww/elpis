...
export function detectRuntimeProfile(options: ProfileDetectionOptions = {}): RuntimeProfile {
  const sentinelPath = options.sentinelPath ?? RESTRICTED_SENTINEL;
  const exists = options.exists ?? fs.existsSync;
  if (exists(sentinelPath)) return Object.freeze({ restricted: true, source: 'sentinel' });
...
export function resolveBuiltinModules(config: Config, profile: RuntimeProfile = { restricted: false, source: 'normal' }): BuiltinModuleRegistry {
  const policy = config.modules ?? { enabled: null, disabled: [] };
  const selected = (id: BuiltinModuleId): boolean => policy.enabled !== null
    ? policy.enabled.includes(id)
    : !policy.disabled.includes(id);
  const status = (id: BuiltinModuleId, keys: string[], available: boolean, unavailableReason: string): BuiltinModuleStatus => {
    if (!selected(id)) return { id, keys, state: 'disabled', reason: `${id} module is excluded by modules policy` };
    if (!available) return { id, keys, state: 'unavailable', reason: unavailableReason };
  ...
  const statuses: BuiltinModuleStatus[] = [
    status('kagi', ['search', 'extract'], Boolean(config.kagi.apiKey), 'Kagi is selected but not configured (set kagi.api_key)'),
    status('bsky', ['bsky'], config.bluesky !== null, 'Bluesky is selected but not configured (set bluesky.identifier and bluesky.app_password)'),
  ...
  return Object.freeze({
    statuses: frozen,
    state(id: BuiltinModuleId) { return find(id)?.state ?? 'disabled'; },
    isSelected(id: BuiltinModuleId) { return (find(id)?.state ?? 'disabled') !== 'disabled'; },
    isActive(id: BuiltinModuleId) { return find(id)?.state === 'active'; },
    reason(id: BuiltinModuleId) { return find(id)?.reason ?? `unknown built-in module: ${id}`; },
  });
}
