...
function bundled(source: string, kind: string): string[] {
  ...
  if (words.length < 100 || new Set(words).size !== words.length || words.some((word) => !/^[a-z][a-z0-9]*$/.test(word))) {
...
export const BUNDLED_ALIAS_WORDLISTS: Readonly<AliasWordlists> = Object.freeze({
  ...
  `, 'adverbs')),
  adjectives: Object.freeze(bundled(`
...
function parseAuthored(file: string): string[] {
  ...
  const words = lines.map((line) => line.trim()).filter((line) => line && !line.startsWith('#'));
  ...
  if (new Set(words).size !== words.length) throw new Error('contains duplicate words');
  return words;
}

export function loadAliasWordlists(dataDirectory: string, logger: Pick<Logger, 'warn'>): AliasWordlists {
  ...
  for (const kind of Object.keys(FILES) as (keyof AliasWordlists)[]) {
    ...
    try {
      ...
      logger.warn(`sandbox wordlist: preserving invalid ${file}; using bundled ${kind}: ${error instanceof Error ? error.message : String(error)}`);
...
export function* aliasCandidates(wordlists: AliasWordlists, chooseStart: AliasRandomIndex = randomInt): Generator<string> {
  const { adverbs, adjectives, nouns } = wordlists;
  if (!adverbs.length || !adjectives.length || !nouns.length) throw new Error('sandbox aliases: every wordlist must be non-empty');
  const total = adverbs.length * adjectives.length * nouns.length;
  if (!Number.isSafeInteger(total)) throw new Error('sandbox aliases: wordlist product exceeds safe allocation space');
  const start = chooseStart(total);
  if (!Number.isInteger(start) || start < 0 || start >= total) throw new Error('sandbox aliases: random index is out of range');
