...
const require_ = Object.assign(function requireFresh(id: string): unknown {
  if (id.startsWith('.') || id.startsWith('/')) {
    ...
    try { resolved = baseRequire.resolve(id); } catch { /* real error below */ }
    if (resolved) {
...
export const RESERVED_GLOBALS: Readonly<Record<string, true>> = {
  elpis: true, fs: true, _: true,
  console: true, process: true, require: true, Buffer: true, URL: true,
  URLSearchParams: true, TextEncoder: true, TextDecoder: true, fetch: true,
  btoa: true, atob: true, crypto: true,
  Object: true, Array: true, JSON: true, Math: true, Date: true, Promise: true,
  RegExp: true, Map: true, Set: true, Error: true, Symbol: true, Number: true,
  String: true, Boolean: true, structuredClone: true, parseInt: true,
  parseFloat: true, isNaN: true, isFinite: true, setTimeout: true,
  clearTimeout: true, setInterval: true, clearInterval: true, queueMicrotask: true,
  globalThis: true, HARNESS_ROOT: true, DATA_DIR: true,
};
...
export function coerceNextRunAt(v: unknown): number {
  ...
  if (typeof v !== 'number' || !Number.isFinite(v)) {
    throw new Error(`elpis.schedule: nextRunAt must be finite epoch-ms, an ISO-8601 string, or a Date (got ${JSON.stringify(v)})`);
  }
  return v;
}
...
export function buildGlobals(deps: SandboxDeps): Record<string, unknown> {
 ...
 // then/catch/finally pass through so `await` and Promise.all work normally.
  type ShResult = { stdout: string; stderr: string; code: number | null; signal: string | null };
  async function shImpl(cmd: string, opts: { cwd?: string; timeout?: number; maxBuffer?: number } = {}): Promise<ShResult> {
 ...
 // group signal fails (pid gone / no group).
    const killGroup = (sig: NodeJS.Signals) => {
      if (child.pid) {
        try { process.kill(-child.pid, sig); return; } catch { /* group gone */ }
      }
 ...
 // reused pid is never killed by mistake.
    ...
    const unregister = () => { if (scope && child.pid) scope.childPids.delete(child.pid); };
    let stdout = '';
    let stderr = '';
 ...
 // sees empty output and can't tell timeout from silence.
        resolve({ stdout, stderr: stderr + `\n[elpis.sh TIMED OUT after ${timeoutMs}ms — output above is partial]`, code: null, signal: 'TIMEOUT' });
      }
    }, timeoutMs);
    ...
    child.on('error', (err) => {
      ...
      if (!done) { done = true; clearTimeout(timer); unregister(); reject(err); }
    });
 ...
 // after a timeout-triggered SIGTERM but before the 5s SIGKILL fires.
      if (killTimer) { clearTimeout(killTimer); killTimer = undefined; }
      if (!done) { done = true; clearTimeout(timer); unregister(); resolve({ stdout, stderr, code, signal: signal ?? null }); }
    });
    return promise;
  }
 ...
 * teachable error. then/catch/finally pass through. */
  function guardShPromise(p: Promise<ShResult>, name: string): Promise<ShResult> {
    return new Proxy(p, {
 ...
 // incompatible receiver". Bind them; pass everything else through.
        ...
        if (SH_RESULT_KEYS.has(prop)) {
          throw new TypeError(`${name}() is now async — write (await ${name}(...)).${prop} (you forgot to await)`);
        }
        return Reflect.get(target, prop, recv);
      },
    }) as Promise<ShResult>;
  ...
  const sh = ((cmd: string, opts?: { cwd?: string; timeout?: number; maxBuffer?: number }) =>
    guardShPromise(shImpl(cmd, opts), 'elpis.sh')) as ((cmd: string, opts?: { cwd?: string; timeout?: number; maxBuffer?: number }) => Promise<ShResult>) & { q: (s: unknown) => string };
 ...
 // elpis.grep("channel(", { fixed: true, ignoreCase: true, max: 50 })
  e.grep = async (
    pattern: string,
    opts: { path?: string; glob?: string; ignoreCase?: boolean; fixed?: boolean; max?: number } = {},
  ) => {
    if (typeof pattern !== 'string' || pattern === '') {
      throw new Error('grep(pattern, opts?): pattern must be a non-empty string');
    }
 ...
 // alternation patterns silently report "no matches" (-08-03 #20).
    ...
    const max = Number.isFinite(opts.max) ? Math.max(1, Math.floor(opts.max as number)) : 200;
    const cmd = `grep ${flags.join(' ')} -e ${sh.q(pattern)} ${sh.q(where)} 2>/dev/null | head -n ${max}`;
    const r = await shImpl(cmd, { cwd: deps.config.paths.harnessRoot, timeout: 30_000 });
    const hits = r.stdout.trimEnd();
    if (!hits) return `grep: no matches for ${JSON.stringify(pattern)} in ${where}`;
    return hits;
  };
 ...
 // current run's scope so a bg detach can kill the tree, matching shImpl.
  const requireSsh = (): SshRegistry => {
    if (!deps.ssh) throw new Error('ssh not wired — the harness did not construct an ssh registry');
    return deps.ssh;
  };
  e.ssh = (host: string, opts?: { user?: string }): SshHandle => {
    if (typeof host !== 'string' || host === '') {
      throw new Error('elpis.ssh(host): host must be a non-empty string — e.g. elpis.ssh("ai.example.com").exec("uptime")');
    }
 ...
 // multiple sites or files; each call is its own atomic read-modify-write.
  ...
  const renderEditDiff = (p: string, hunks: editor.EditHunk[]): string => {
    ...
    for (const h of hunks) {
      rows.push(`${p} @@ -${h.from},${h.removed.length} +${h.from},${h.inserted.length} @@`);
      const emit = (sign: '-' | '+', ls: string[]) => {
        const tag = (i: number) => `${sign}${String(h.from + i).padStart(4)}: ${ls[i]}`;
        if (ls.length <= DIFF_EDGE * 2 + 1) {
          ...
          for (let i = ls.length - DIFF_EDGE; i < ls.length; i++) rows.push(tag(i));
        }
      };
      emit('-', h.removed);
      emit('+', h.inserted);
  ...
  e.edit = (p: string, oldString: string, newString: string, opts?: { replaceAll?: boolean }) => {
    const src = fs.readFileSync(p, 'utf8');
    const r = editor.replace(src, oldString, newString, { all: opts?.replaceAll }); // throws BEFORE any write
    fs.writeFileSync(p, r.source);
    return { ok: true as const, path: p, replacements: r.count, diff: renderEditDiff(p, r.hunks) };
  };
 ...
 // Heredocs stay raw; fill is the separate, explicit way to inject a value.
  e.fill = (template: string, vars: Record<string, unknown>) => fill(template, vars);
 ...
 // hand-written split/map/pad/join/console.log incantation every time.
  e.read = (p: string, opts: { from?: number; to?: number; numbers?: boolean } = {}) => {
    const out = formatRead(p, fs.readFileSync(p, 'utf8'), opts, deps.config.sandbox.previewMaxBytes);
 ...
 // dead read; appendDatedBullet does its own single read+write.
  ...
  }) as ((thread: string, text: string) => { ok: true; thread: string; file: string }) &
    { close: (thread: string, conclusion?: string) => { ok: true; thread: string; archivedTo?: string } };
  ponderFn.close = (thread: string, conclusion?: string) => {
    const slug = slugifyName(thread);
    const file = path.join(ponderDir, `${slug}.md`);
    fs.mkdirSync(ponderResolvedDir, { recursive: true });
    let body = '';
    try { body = fs.readFileSync(file, 'utf8'); } catch { return { ok: true, thread }; }
    if (conclusion) {
 ...
 // append -2, -3… when a same-named thread was resolved before.
    ...
    try { fs.unlinkSync(file); } catch { /* already moved */ }
 ...
 // detach path registers pending promises into.
  if (deps.bg) {
    const bgReg = deps.bg;
    e.bg = {
      start: (cmd: string, opts?: BgStartOpts) => bgReg.start(cmd, { ...opts, originChannelId: deps.inbound?.channelId ?? opts?.originChannelId }),
      list: () => bgReg.list(),
      get: (id: string) => bgReg.get(id),
      tail: (id: string, lines?: number) => bgReg.tail(id, lines),
      rearm: (id: string, when?: unknown) => bgReg.rearm(id, when === undefined ? undefined : coerceNextRunAt(when)),
 ...
 // a custom endpoint's levels are honored here and not just in the registry.
  ...
  const assertFleetEffort = (o?: { effort?: string | null }) => {
    if (o?.effort != null && !fleetEfforts.includes(o.effort)) {
      const supported = fleetEfforts.length ? fleetEfforts.join('|') : '(none — this endpoint takes no effort parameter)';
      throw new Error(`fleet: effort must be ${supported} — got ${JSON.stringify(o.effort)}`);
    }
  };
 ...
 // means there is no fleet to reach for — do the work yourself.
      throw new Error(deps.config.fleet?.enabled === false
 ...
 // a rejection instead of a same-tick synchronous throw for these verbs.
    ...
    send: async (ref: string, text: string, opts?: Parameters<FleetHandle['send']>[2]) => requireFleet().send(ref, text, opts),
 ...
 // both from DATA_DIRECTORY; there is no separate path knob).
    ...
    for (const sub of ['people', 'ponder', path.join('ponder', 'resolved'), 'notes']) {
    ...
    for (const file of roots) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        matches.push({ file: path.relative(dataDir, file), line: i + 1, text: lines[i].trim().slice(0, 300) });
      }
      if (truncated) break;
    }
 ...
 // puts it straight into the tool result).
  e.preview = (v: unknown, opts: { max?: number; depth?: number; strings?: number } = {}) =>
 ...
 // only in dist/ with no commit behind it. `{ allowDirty: true }` overrides.
    if (!opts?.allowDirty) {
      const status = await shImpl('git status --porcelain', { cwd: root, timeout: 30_000 });
      if (status.code === 0 && status.stdout.trim() !== '') {
        return {
          ok: false,
          note: 'deploy aborted: uncommitted changes in the harness tree — commit + push first ' +
      ...
      const upstream = await shImpl('git rev-parse --abbrev-ref @{u}', { cwd: root, timeout: 30_000 });
      if (upstream.code === 0) {
        const unpushed = await shImpl('git log @{u}.. --oneline', { cwd: root, timeout: 30_000 });
        if (unpushed.code === 0 && unpushed.stdout.trim() !== '') {
          return {
            ok: false,
            note: 'deploy aborted: local commit(s) not pushed to origin — push first (elpis.git.push()) so a ' +
    ...
    const res = await shImpl('npm run build', { cwd: deps.config.paths.harnessRoot, timeout: 180_000 });
    if (res.code !== 0) {
      return {
        ok: false,
 ...
 // (elpis.deploy handles its own cwd).
  ...
  async function gitRun(args: string, opts: GitOpts = {}): Promise<ShResult> {
    return shImpl(`git ${args}`, { cwd: opts.cwd ?? deps.config.paths.dataDirectory, timeout: opts.timeout ?? 60_000 });
  }
  const gitApi = {
    ...
    add: async (paths?: string | string[], opts?: GitOpts) => {
      const spec = paths === undefined ? '.' : (Array.isArray(paths) ? paths.join(' ') : String(paths));
 ...
 // is how two fixes were silently lost — a git op that fails must fail loud.
      if (r.code !== 0) throw new Error(`elpis.git.add failed (exit ${r.code}): ${(r.stderr || r.stdout).trim().slice(0, 500)}`);
      return { ok: true, ...r };
    },
    commit: async (message: string, opts?: GitOpts & { add?: boolean }) => {
      if (typeof message !== 'string' || message === '') {
        throw new Error('elpis.git.commit(message): message must be a non-empty string');
      }
      const flags = opts?.add ? '-a' : '';
      const r = await gitRun(`commit ${flags} -m ${JSON.stringify(message)}`, opts);
 ...
 // exact silent no-op that lost the fetchContextWindow + send fixes twice.
      if (r.code !== 0) throw new Error(`elpis.git.commit failed (exit ${r.code}): ${(r.stderr || r.stdout).trim().slice(0, 500)}`);
      const sha = (await gitRun('rev-parse --short HEAD', opts)).stdout.trim();
      return { ok: true, sha, stdout: r.stdout, stderr: r.stderr, code: r.code, signal: r.signal };
    },
    push: async (opts?: GitOpts) => {
      const r = await gitRun('push', opts);
      if (r.code !== 0) throw new Error(`elpis.git.push failed (exit ${r.code}): ${(r.stderr || r.stdout).trim().slice(0, 500)}`);
      return { ok: true, ...r };
    },
 ...
 // throws (from add/commit/push), so a broken ship can't look like success.
    commitAndPush: async (message: string, opts?: GitOpts & { add?: boolean }) => {
 ...
 // channels come from the directory.
  e.channel = (idOrRef?: string) => {
    if (idOrRef === undefined || idOrRef === null || idOrRef === '') {
      const known = deps.listChannelsWithNames ? deps.listChannelsWithNames().map((c) => c.name) : [];
      throw new Error(
    ...
    if (channelId === INTERNAL_CHANNEL_ID) {
      throw new Error(`elpis.channel('internal') is not a real room — it is a provenance label. Pass a real channel ref or id.`);
    }
 ...
 // otherwise the echo would read '999999 (999999)'.
    ...
    const resolvedLabel = deps.channelLabel ? deps.channelLabel(channelId) : channelId;
    const label = resolvedLabel !== channelId ? `${resolvedLabel} (${channelId})` : channelId;
    return {
      id: channelId,
      name: resolvedName,
      send: async (content: unknown, sendOpts?: { allowEscapes?: boolean; files?: { path: string; name?: string }[] }) => {
        const hasFiles = Array.isArray(sendOpts?.files) && sendOpts!.files.some((f) => typeof (f as { path?: unknown }).path === 'string');
 ...
 // narrow for downstream uses). Attachment-only send: caption optional.
        ...
        else throw new Error('elpis.channel().send(content) requires a string (or pass { files } for an attachment-only send)');
        if (!deps.send) {
          throw new Error('elpis.channel().send() is not wired in this harness');
        }
 // Escape guard: literal `\n`/`\t`/`\uXXXX` render as backslash-n.
        if (!sendOpts?.allowEscapes && /\\(?:n|t|r|u[0-9a-fA-F]{4})/.test(text)) {
          throw new Error(
...
if (files && files.length > 0) sendRecord.files = files.map((f) => f.name || String(f.path).split('/').pop()).filter((n): n is string => typeof n === 'string');
 ...
 // becomes visible in the very next tool result.
        return { ok: true, channelId, note: `message delivered to ${label}. anything you return in this turn's content block will be ignored` };
      },
      typing: () => {
        if (!deps.typing) {
          throw new Error('elpis.channel().typing() is not wired in this harness');
        }
 ...
 // operator-only and unreachable from inside the sandbox.
      mute: (reason?: string) => {
        if (!deps.moderate) {
          throw new Error('elpis.channel().mute() is not wired in this harness');
        }
        const r = deps.moderate(channelId, typeof reason === 'string' ? reason : undefined);
        return { ok: r.ok, channelId, note: r.note };
      },
    };
  };
 // channel.list — enumerate known channels as { id, name } objects.
  const channelFn = e.channel as ((id?: string) => unknown) & { list: () => { id: string; name: string | null }[] };
  channelFn.list = () => {
    if (deps.listChannelsWithNames) return deps.listChannelsWithNames();
    return deps.listChannels ? deps.listChannels().map((id) => ({ id, name: null })) : [];
  };
 ...
 // For the watch-together pipeline (episode keyframes etc).
  e.watch = (paths: string[], note: string) => {
    if (!deps.watch) throw new Error('elpis.watch is not wired in this harness');
    if (!Array.isArray(paths) || paths.length === 0) throw new Error('elpis.watch(paths, note): paths must be a non-empty array of local image paths');
    const res = deps.watch(paths, note ?? '');
    return { ...res, note: `queued ${res.count} frame(s) — they arrive as your next turn. describe/react in that turn; the frames are gone after it.` };
  };
  ...
  e.mind = {
    add: (opts: { title: string; body?: string; kind?: 'task' | 'project' | 'idea' | 'question' | 'reminder'; status?: 'inbox' | 'open' | 'in_progress' | 'waiting' | 'done' | 'cancelled'; priority?: number; parentId?: number | string | null; dueAt?: unknown; tags?: string[]; dependsOn?: Array<number | string>; remindAt?: unknown; channelId?: string | null; actor?: string }) => {
      ...
      return requireMind().create({
        ...
        dueAt: opts.dueAt === undefined || opts.dueAt === null ? null : coerceNextRunAt(opts.dueAt),
        remindAt: opts.remindAt === undefined || opts.remindAt === null ? null : coerceNextRunAt(opts.remindAt),
    ...
    list: (filter?: { statuses?: Array<'inbox' | 'open' | 'in_progress' | 'waiting' | 'done' | 'cancelled'>; kinds?: Array<'task' | 'project' | 'idea' | 'question' | 'reminder'>; tag?: string; query?: string; parentId?: number | string | null; ready?: boolean; blocked?: boolean; overdue?: boolean; includeArchived?: boolean; sort?: 'created_asc' | 'created_desc' | 'updated_asc' | 'updated_desc' | 'last_comment_asc' | 'last_comment_desc'; limit?: number; offset?: number }) => requireMind().list(filter ? { ...filter, parentId: filter.parentId === undefined || filter.parentId === null ? filter.parentId : parseMindId(filter.parentId) } : undefined),
    ready: (limit?: number) => requireMind().ready(limit),
    stats: () => requireMind().stats(),
    graph: (ref: number | string, depth?: number) => requireMind().graph(parseMindId(ref), depth),
    update: (ref: number | string, patch: { title?: string; body?: string; kind?: 'task' | 'project' | 'idea' | 'question' | 'reminder'; status?: 'inbox' | 'open' | 'in_progress' | 'waiting' | 'done' | 'cancelled'; priority?: number; parentId?: number | string | null; dueAt?: unknown; tags?: string[] }, actor?: string) => {
      const normalized: Parameters<NonNullable<SandboxDeps['mind']>['update']>[1] = {};
      ...
      if (patch.parentId !== undefined) normalized.parentId = patch.parentId === null ? null : parseMindId(patch.parentId);
      if (patch.dueAt !== undefined) normalized.dueAt = patch.dueAt === null ? null : coerceNextRunAt(patch.dueAt);
      return requireMind().update(parseMindId(ref), normalized, actor ?? mindActor());
    },
    status: (ref: number | string, status: 'inbox' | 'open' | 'in_progress' | 'waiting' | 'done' | 'cancelled', actor?: string) => requireMind().setStatus(parseMindId(ref), status, actor ?? mindActor()),
    done: (ref: number | string, comment?: string, actor?: string) => {
      const id = parseMindId(ref); const who = actor ?? mindActor();
    ...
    unlinks: (ref: number | string, from: number | string, actor?: string) => requireMind().removeDependency(parseMindId(ref), parseMindId(from), actor ?? mindActor()),
    tag: (ref: number | string, tag: string, actor?: string) => requireMind().addTag(parseMindId(ref), tag, actor ?? mindActor()),
    untag: (ref: number | string, tag: string, actor?: string) => requireMind().removeTag(parseMindId(ref), tag, actor ?? mindActor()),
 ...
 // the requested millisecond count (or 0 if omitted/invalid).
  e.sleep = async (ms?: number) => {
    const delay = typeof ms === 'number' && Number.isFinite(ms) ? Math.max(0, Math.floor(ms)) : 0;
 ...
 // undefined when no scheduler was wired.
  ...
  const scheduleGlobal = (opts: { name: string; kind?: string; channelId?: string | null; payload: string; nextRunAt: unknown; intervalMs?: number | null; nagIntervalMs?: number | null; parentId?: number | null }) => {
    if (!opts || typeof opts.name !== 'string' || opts.name.trim() === '') throw new Error('elpis.schedule: { name } must be a non-empty string');
    if (typeof opts.payload !== 'string') throw new Error('elpis.schedule: { payload } must be a string');
    return requireScheduler().create({ ...opts, nextRunAt: coerceNextRunAt(opts.nextRunAt) });
  };
  ...
  scheduleGlobal.update = (id: number, patch: { payload?: string; nextRunAt?: unknown; intervalMs?: number | null; nagIntervalMs?: number | null; snoozeUntil?: unknown }) => {
    const p: { payload?: string; nextRunAt?: number; intervalMs?: number | null; nagIntervalMs?: number | null; snoozeUntil?: number | null } = {};
    if (patch.payload !== undefined) p.payload = patch.payload;
    if (patch.intervalMs !== undefined) p.intervalMs = patch.intervalMs;
    if (patch.nagIntervalMs !== undefined) p.nagIntervalMs = patch.nagIntervalMs;
    if (patch.nextRunAt !== undefined) p.nextRunAt = coerceNextRunAt(patch.nextRunAt);
 ...
 // should indicate — don't add sleepPause/sleepResume calls here.
  e.timeout = async <T>(promise: Promise<T>, ms?: number): Promise<T> => {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      timer = setTimeout(() => reject(new Error(`timeout after ${delay}ms`)), delay);
    });
    try {
