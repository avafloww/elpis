...
export interface WakeAdviceTurnContext {
  ...
  ranCode: boolean;
  continuedMindId: MindId | null;
}

export interface WakeAdvisorState extends WakeAdviceTurnContext {
  inProgress: { id: MindId; title: string }[];
  ready: { id: MindId; title: string }[];
  waiting: { id: MindId; title: string }[];
  runningBg: number;
...
function capHistory(value: string, max: number): string {
  ...
  return (
    value.slice(0, head) + marker + value.slice(value.length - (kept - head))
  );
}
...
function boundedMessage(message: ChatMessage): ChatMessage {
  ...
  if (message.role === "assistant") {
    ...
    if (message.tool_calls)
      bounded.tool_calls = message.tool_calls.map((call) => ({
        id: call.id,
        type: call.type,
        function: {
          name: call.function.name,
          arguments: boundedArguments(call.function.arguments),
        },
      }));
  }
...
function reasoningChars(message: ChatMessage): number {
  return (message.reasoning_items ?? []).reduce(
    (sum, item) =>
      sum +
  ...
  for (let index = 0; index < messages.length;) {
    ...
    if (completeCalls.length > 0) {
      ...
      units.push([
        ...
        ...outputs.filter(
          (output) =>
            output.tool_call_id && completeIds.has(output.tool_call_id),
        ),
      ]);
...
function fitNewestCompleteCalls(
  unit: ChatMessage[],
  maxChars: number,
): ChatMessage[] {
  ...
  for (let start = assistant.tool_calls.length - 1; start >= 0; start--) {
    ...
    const candidate = [
      ...
      ...unit
        .slice(1)
        .filter(
          (output) => output.tool_call_id && ids.has(output.tool_call_id),
        ),
    ];
...
function hardBoundHistory(messages: ChatMessage[]): ChatMessage[] {
  ...
  for (let index = units.length - 1; index >= 0; index--) {
    const unit =
      selected.length === 0
        ? fitNewestCompleteCalls(units[index], remaining)
        : units[index];
...
export function buildWakeAdvisorHistory(
  ...
  const scoped = messages
    ...
    .map(boundedMessage);
  const history = hardBoundHistory(
    recentCycleHistory(balancedHistoryUnits(scoped)),
  );
...
function mindItems(
  ...
  try {
    ...
    return (items ?? [])
      .slice(0, 4)
      .map(itemSummary)
      .filter((item): item is { id: MindId; title: string } => item !== null);
  } catch {
    return [];
  }
}
...
export function snapshotWakeAdvisorState(
  ...
  try {
    const tasks = (deps.scheduler?.list() ?? []) as {
      name?: unknown;
      nextRunAt?: unknown;
      doneAt?: unknown;
      snoozeUntil?: unknown;
...
export async function adviseWake(
  ...
  try {
    ...
    const parsed = JSON.parse(completion.content.trim()) as Record<
      string,
      unknown
    >;
