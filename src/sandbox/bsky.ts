...
async function ensureSession(cfg: BskyConfig): Promise<BskySession> {
  if (session && sessionConfig && sessionConfig.identifier === cfg.identifier)
    return session;
  ...
  const add = (start: number, end: number, feature: Record<string, string>) => {
    facets.push({
      index: {
        ...
        byteEnd: utf8Length(text.slice(0, end)),
      },
      features: [feature],
    });
  };
...
export async function bskyPost(
  cfg: BskyConfig,
  text: string,
): Promise<{ uri: string; cid: string }> {
...
export async function bskyReply(
  cfg: BskyConfig,
  text: string,
...
export async function bskyLike(
  cfg: BskyConfig,
  uri: string,
  cid: string,
): Promise<{ uri: string; cid: string }> {
...
export async function bskyFollow(
  cfg: BskyConfig,
  did: string,
): Promise<{ uri: string; cid: string }> {
  const s = await ensureSession(cfg);
...
export async function bskyTimeline(
  cfg: BskyConfig,
  limit = 20,
): Promise<
  Array<{
...
export async function bskyFeed(
  cfg: BskyConfig,
  limit = 10,
): Promise<
  Array<{ text: string; likes: number; reposts: number; uri: string }>
> {
  const s = await ensureSession(cfg);
  const data = await xrpc(
    cfg,
    ...
    undefined,
    { actor: s.handle, limit: String(limit) },
    s.accessJwt,
  );
  return (data.feed ?? []).map((p: any) => ({
    text: p.post?.record?.text ?? '',
...
export async function bskyNotifications(
  cfg: BskyConfig,
  limit = 10,
): Promise<{
  unread: number;
  items: Array<{ reason: string; author: string; text?: string }>;
}> {
  const s = await ensureSession(cfg);
  const count = await xrpc(
    cfg,
    'GET',
