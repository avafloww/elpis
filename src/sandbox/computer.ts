...
export type ComputerRunner = (
  command: string,
  opts?: ComputerRunOptions,
) => Promise<ComputerProcessResult>;
...
export function displayShellCommand(
  command: string,
  display: string,
  xauthority: string,
): string {
  if (typeof command !== 'string' || !command.trim())
    throw new Error(
      'elpis.computer: display command must be a non-empty string',
    );
...
export function parseWindows(text: string): DesktopWindow[] {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    const match = line.match(
      /^(\S+)\s+(-?\d+)\s+(\d+)\s+(-?\d+)\s+(-?\d+)\s+(\d+)\s+(\d+)\s+(\S+)\s+(\S+)\s*(.*)$/,
    );
    if (!match) continue;
    windows.push({
      id: match[1],
...
function finiteInt(
  ...
  if (typeof value !== 'number' || !Number.isFinite(value))
    throw new Error(`elpis.computer: ${label} must be a finite number`);
...
function inputKeys(value: unknown, label = 'keys', unique = true): string[] {
  ...
  if (raw.length === 0)
    throw new Error(`elpis.computer.${label}: provide at least one key`);
  const keys = raw.map((key) => {
    if (
      ...
      throw new Error(
        `elpis.computer.${label}: every key must be a non-empty xdotool key name`,
      );
    }
    return key;
  });
...
export function heldKeysScript(
  keysValue: string | string[],
  ...
  if (durationMs > MAX_HOLD_MS)
    throw new Error(`elpis.computer: durationMs must be <= ${MAX_HOLD_MS}`);
  const release = keys
    .slice()
    .reverse()
    .map((key) => `xdotool keyup ${shellQuote(key)} || true`)
    .join('; ');
  const press = keys
...
export function pointerClickScript(
  xValue: number,
  yValue: number,
  buttonValue: number,
  countValue: number,
  ...
  if (count > 100)
    throw new Error('elpis.computer: click count must be <= 100');
...
export function pointerDragScript(
  fromXValue: number,
  fromYValue: number,
  toXValue: number,
  toYValue: number,
...
function windowId(value: unknown): string {
  ...
  if (!/^0x[0-9a-f]+$/i.test(id))
    throw new Error(
      'elpis.computer: window id must look like 0x0123abcd (use windows())',
    );
  return id;
}

function appName(value: unknown): string {
  const name = String(value || 'app')
    .toLowerCase()
    .replace(/[^a-z0-9_-]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 32);
...
export function coordinateGridFilter(
  width: number,
  height: number,
  spacing = 100,
): string {
  ...
  const filters = [
    `drawgrid=width=${gap}:height=${gap}:thickness=1:color=${color}@0.58`,
  ];
  for (let x = 0; x < w; x += gap) {
    filters.push(
      `drawtext=text='${x}':x=${x + 3}:y=4:fontsize=13:fontcolor=${color}:box=1:boxcolor=black@0.62:boxborderw=2`,
    );
  }
  for (let y = gap; y < h; y += gap) {
    filters.push(
      `drawtext=text='${y}':x=4:y=${y + 3}:fontsize=13:fontcolor=${color}:box=1:boxcolor=black@0.62:boxborderw=2`,
    );
  }
  return filters.join(',');
}
...
export function createComputerTools(
  ...
  const xauthority =
    options.xauthority ?? path.join(options.computerDir, 'Xauthority');
  ...
  const exec = async (
    label: string,
    command: string,
    opts: ComputerRunOptions & { allowNonzero?: boolean } = {},
  ) => {
  ...
  const screenshot = async (
    opts: ComputerRunOptions & { filename?: string } = {},
  ) => {
    fs.mkdirSync(screenshotsDir, { recursive: true });
    const file = opts.filename
      ? path.resolve(opts.filename)
      : path.join(screenshotsDir, `desktop-${Date.now()}.png`);
    await exec('screenshot', `scrot --overwrite ${shellQuote(file)}`, opts);
    if (!fs.existsSync(file))
      throw new Error(
        `elpis.computer.screenshot failed: scrot did not create ${file}`,
      );
  ...
  const hold = async (
    ...
    if (opts.timeout !== undefined && opts.timeout < durationMs + 1000) {
      throw new Error(
        'elpis.computer.hold: timeout must allow durationMs plus 1000ms cleanup margin',
      );
    }
    await exec('hold', script, {
      ...opts,
      timeout: opts.timeout ?? Math.max(60_000, durationMs + 5000),
    });
  ...
  const look = async (
    ...
    if (!options.watch)
      throw new Error(
        'elpis.computer.look(): multimodal watch delivery is not wired',
      );
    ...
    const spacing =
      opts.grid === undefined || opts.grid === true
        ? 100
        : finiteInt(opts.grid, 'grid spacing', 25);
    const dimensions = await exec(
      'look.grid',
    ...
    if (!match)
      throw new Error(
        `elpis.computer.look failed: could not parse display dimensions from ${JSON.stringify(dimensions.stdout.trim())}`,
      );
    const ext = path.extname(shot.file) || '.png';
    const gridFile = `${shot.file.slice(0, -ext.length)}-grid-${spacing}${ext}`;
    const filter = coordinateGridFilter(
      Number(match[1]),
      Number(match[2]),
      spacing,
    );
    await exec(
      'look.grid',
      `ffmpeg -y -loglevel error -i ${shellQuote(shot.file)} -vf ${shellQuote(filter)} ${shellQuote(gridFile)}`,
      opts,
    );
    if (!fs.existsSync(gridFile))
      throw new Error(
        `elpis.computer.look failed: ffmpeg did not create ${gridFile}`,
      );
  ...
  return {
    start: async (opts: ComputerRunOptions = {}) => {
      await exec(
        'start',
        `sudo -n systemctl start ${shellQuote(xorgServiceName)}`,
        opts,
      );
      await exec(
        'start',
      ...
      return {
        ok: true,
        xorgService: xorgServiceName,
        desktopService: serviceName,
      };
    },
    stop: async (opts: ComputerRunOptions = {}) => {
      ...
      await exec(
        'stop',
        `sudo -n systemctl stop ${shellQuote(xorgServiceName)}`,
        opts,
      );
      return {
        ok: true,
    ...
    restart: async (opts: ComputerRunOptions = {}) => {
      ...
      await exec(
        'restart',
        `sudo -n systemctl restart ${shellQuote(xorgServiceName)}`,
        opts,
      );
      await exec(
        'restart',
    ...
    status: async (opts: ComputerRunOptions = {}) => {
      const xorg = await exec(
        'status',
        `sudo -n systemctl is-active ${shellQuote(xorgServiceName)}`,
        { ...opts, allowNonzero: true },
      );
      ...
      return {
        ok:
          xorg.stdout.trim() === 'active' &&
          service.stdout.trim() === 'active' &&
          screen.code === 0,
        xorg: xorg.stdout.trim() || 'unknown',
        desktop: service.stdout.trim() || 'unknown',
        display,
        dimensions: screen.stdout.trim() || null,
        error:
          screen.code === 0
            ? null
            : (screen.stderr || screen.stdout).trim().slice(0, 1000),
      };
    },
    launch: async (
      command: string,
      opts: ComputerRunOptions & { name?: string; cwd?: string } = {},
    ) => {
      if (typeof command !== 'string' || !command.trim())
        throw new Error(
          'elpis.computer.launch(command): command must be a non-empty string',
        );
    ...
    windows: async (opts: ComputerRunOptions = {}) => {
      ...
      if (ids.length === 1) {
        const klass = await exec(
          'windows',
          `xprop -id ${shellQuote(ids[0])} WM_CLASS`,
          { ...opts, allowNonzero: true },
        );
        if (/tint2/i.test(klass.stdout)) return [];
      }
      const result = await exec('windows', 'wmctrl -lpGx', opts);
      return parseWindows(result.stdout).filter(
        (window) => !/tint2/i.test(window.class),
      );
    },
    focus: async (id: string, opts: ComputerRunOptions = {}) => {
      const target = windowId(id);
      await exec('focus', `wmctrl -ia ${shellQuote(target)}`, opts);
      return { ok: true, window: target };
    },
    closeWindow: async (id: string, opts: ComputerRunOptions = {}) => {
      const target = windowId(id);
      await exec('closeWindow', `wmctrl -ic ${shellQuote(target)}`, opts);
      return { ok: true, window: target };
    },
    move: async (x: number, y: number, opts: ComputerRunOptions = {}) => {
      const px = finiteInt(x, 'x', 0),
        py = finiteInt(y, 'y', 0);
    ...
    click: async (
      ...
      if (!button)
        throw new Error(
          'elpis.computer.click: button must be left, middle, or right',
        );
    ...
    drag: async (
      ...
      const x1 = finiteInt(fromX, 'fromX', 0),
        y1 = finiteInt(fromY, 'fromY', 0);
      const x2 = finiteInt(toX, 'toX', 0),
        y2 = finiteInt(toY, 'toY', 0);
      await exec('drag', pointerDragScript(x1, y1, x2, y2), opts);
      return { ok: true, from: [x1, y1], to: [x2, y2] };
    },
    type: async (
      text: string,
      opts: ComputerRunOptions & { delay?: number } = {},
    ) => {
      if (typeof text !== 'string')
        throw new Error('elpis.computer.type(text): text must be a string');
      const delay = finiteInt(opts.delay ?? 10, 'delay', 0);
      await exec(
        'type',
        `xdotool type --clearmodifiers --delay ${delay} -- ${shellQuote(text)}`,
        opts,
      );
    ...
    key: async (
      ...
      await exec(
        'key',
        `xdotool key --clearmodifiers ${keys.map(shellQuote).join(' ')}`,
        opts,
      );
      return { ok: true, keys };
    },
    ...
    release,
    sequence: async (
      stepsValue: ComputerSequenceStep[],
      opts: ComputerRunOptions = {},
    ) => {
      if (!Array.isArray(stepsValue) || stepsValue.length === 0)
        throw new Error(
          'elpis.computer.sequence(steps): provide at least one step',
        );
      if (stepsValue.length > MAX_SEQUENCE_STEPS)
        throw new Error(
          `elpis.computer.sequence: at most ${MAX_SEQUENCE_STEPS} steps`,
        );
      const steps = stepsValue.map((step, index) => {
        if (typeof step !== 'object' || step === null)
          throw new Error(
            `elpis.computer.sequence: step ${index} must be an object`,
          );
        const keys = inputKeys(step.keys, `sequence step ${index} keys`);
        const durationMs = finiteInt(
          step.durationMs,
          `sequence step ${index} durationMs`,
          1,
        );
        if (durationMs > MAX_HOLD_MS)
          throw new Error(
            `elpis.computer.sequence: step ${index} durationMs must be <= ${MAX_HOLD_MS}`,
          );
        const waitMs = finiteInt(
          step.waitMs ?? 0,
          `sequence step ${index} waitMs`,
          0,
        );
        return { keys, durationMs, waitMs };
      });
      const totalMs = steps.reduce(
        (sum, step) => sum + step.durationMs + step.waitMs,
        0,
      );
      if (totalMs > MAX_SEQUENCE_MS)
        throw new Error(
          `elpis.computer.sequence: total duration must be <= ${MAX_SEQUENCE_MS}`,
        );
      for (const step of steps) {
        await hold(step.keys, step.durationMs, opts);
        if (step.waitMs > 0)
          await new Promise<void>((resolve) =>
            setTimeout(resolve, step.waitMs),
          );
      }
    ...
    scroll: async (clicks: number, opts: ComputerRunOptions = {}) => {
      ...
      await exec(
        'scroll',
        `xdotool click --repeat ${Math.abs(amount)} ${button}`,
        opts,
      );
    ...
    clipboard: {
      ...
      set: async (text: string, opts: ComputerRunOptions = {}) => {
        if (typeof text !== 'string')
          throw new Error(
            'elpis.computer.clipboard.set(text): text must be a string',
          );
        const file = path.join(
          options.computerDir,
          `.clipboard-${randomUUID()}`,
        );
        fs.writeFileSync(file, text, { mode: 0o600 });
        try {
          await exec(
            'clipboard.set',
            `xclip -selection clipboard -in < ${shellQuote(file)}`,
            opts,
          );
        } finally {
    ...
    raw: async (command: string, opts: ComputerRunOptions = {}) => {
      if (typeof command !== 'string' || !command.trim())
        throw new Error(
          'elpis.computer.raw(command): command must be a non-empty string',
        );
