...
export interface BgRegistry {
  ...
  start(cmd: string, opts?: BgStartOpts): { id: string; pid: number; logFile: string };
 ...
 * cancel/reap can kill sh/sudo children the detached run is still driving. */
  registerFuture(src: string, promise: Promise<unknown>, opts?: { ttlMs?: number; childPids?: Set<number>; originChannelId?: string }): string;
...
export function createBgRegistry(dataDirectory: string, opts?: BgRegistryOpts): BgRegistry {
 ...
 // persisted but died while the harness was down remains pending.
        if (j.kind === 'job' && j.pid) {
          ...
          if (!j.running) {
            ...
            j.finishedAt ??= now;
            if (!wasRunning && j.settleNotifiedAt === undefined) j.settleNotifiedAt = now;
          }
          j.nudgeAt ??= j.startedAt + (jobNudgeMs);
        } else if (j.kind === 'future') {
 // futures don't survive a restart (the promise died with the process).
          if (j.running) { j.running = false; j.rejected = true; j.value = '[process restarted — future lost]'; }
        }
        jobs.set(j.id, j);
      }
 ...
 // start/exit/settle/cancel.
    const arr = Array.from(jobs.values()).slice(-50).map((j) => {
      if (j.value === undefined) return j;
      return { ...j, value: previewValue(j.value, 200) };
    });
    try { fs.writeFileSync(registryFile, JSON.stringify(arr, null, 2)); } catch { /* best-effort */ }
  }
  ...
  function start(cmd: string, opts?: BgStartOpts): { id: string; pid: number; logFile: string } {
    const id = nextId('j');
    ...
    const job: BgJob = {
      id, kind: 'job', cmd, pid: child.pid ?? undefined, logFile,
      startedAt, running: true, originChannelId: opts?.originChannelId,
 ...
 // If we read from the middle of the file, the first "line" is partial — drop it.
  ...
  function notifyJobStillRunning(job: BgJob): void {
    ...
    saveRegistry();
    try { opts?.onJobStillRunning?.({ ...job }, boundedTail(job.id)); } catch { /* notice is best-effort */ }
    scheduleJobNudge(job);
  }

  function notifyJobSettled(job: BgJob): void {
    if (!jobDeliveryActive || job.running || job.settleNotifiedAt != null) return;
    job.settleNotifiedAt = Date.now();
    saveRegistry();
    try { opts?.onJobSettled?.({ ...job }, boundedTail(job.id)); } catch { /* notice is best-effort */ }
  }
 ...
 // also makes a manual rearm safe if an older timer callback was already due.
    ...
    const timer = setTimeout(() => {
      ...
      if ((job.nudgeAt ?? 0) > Date.now()) { scheduleJobNudge(job); return; }
      if (job.pid && !isPidAlive(job.pid)) finishJob(job, null, job.signal ?? null);
  ...
  function finishJob(job: BgJob, code: number | null, signal: string | null): void {
  ...
  function rearm(id: string, at = Date.now() + jobNudgeMs): BgJob {
    ...
    if (!Number.isFinite(at)) throw new Error('rearm time must be a finite epoch timestamp');
    job.nudgeAt = at;
    saveRegistry();
    if (jobDeliveryActive) scheduleJobNudge(job);
    return { ...job };
  ...
  function activate(): void {
    ...
    for (const job of jobs.values()) {
      ...
      if (job.running && job.pid && !isPidAlive(job.pid)) finishJob(job, null, job.signal ?? null);
 ...
 // ignored — no record overwrite, no spurious [bg settled] notice.
  ...
  function registerFuture(src: string, promise: Promise<unknown>, opts?: { ttlMs?: number; childPids?: Set<number>; originChannelId?: string }): string {
    const id = nextId('f');
    const ttl = opts?.ttlMs ?? 30 * 60 * 1000;
    const job: BgJob = {
      id, kind: 'future', src, startedAt: Date.now(), running: true,
      ttlAt: Date.now() + ttl,
  ...
  function settleFuture(id: string, value: unknown, rejected: boolean): boolean {
    const j = jobs.get(id);
 ...
 // leaking every detached promise for the process lifetime).
  ...
  function reapAbandoned(): string[] {
    ...
    for (const [id, j] of jobs) {
      if (j.kind === 'future' && j.running && !j.cancelled && j.ttlAt && j.ttlAt <= now) {
        const f = futures.get(id);
        if (f) for (const pid of f.childPids) killTree(pid);
        futures.delete(id);
        j.running = false;
