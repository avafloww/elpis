...
function freezeValue(value: unknown, location: string, seen: WeakSet<object>): unknown {
  ...
  if (typeof value !== 'object') throw new Error(`${location} contains unsupported ${typeof value}`);
  if (seen.has(value)) throw new Error(`${location} contains a circular reference`);
  seen.add(value);
  if (Array.isArray(value)) {
    const copy = value.map((entry, index) => freezeValue(entry, `${location}[${index}]`, seen));
    seen.delete(value);
    return Object.freeze(copy);
  }
  if (!plainObject(value)) throw new Error(`${location} must contain only plain objects, arrays, primitives, and functions`);
  const copy = Object.create(null) as Record<string, unknown>;
  for (const key of Object.keys(value).sort(compareText)) {
    if (FORBIDDEN_KEYS.has(key)) throw new Error(`${location} contains forbidden key ${key}`);
    const descriptor = Object.getOwnPropertyDescriptor(value, key);
    if (!descriptor || !('value' in descriptor)) throw new Error(`${location}.${key} must be a data property`);
...
function readDefinition(module: Record<string, unknown>, fileName: string): ExtensionDefinition {
...
function composePrompt(loaded: { summary: ExtensionSummary; prompt: string }[]): string {
  ...
  const blocks = loaded.map(({ summary, prompt }) => {
    ...
    lines.push(`Exported members: ${summary.members.length ? summary.members.map((name) => `\`${name}\``).join(', ') : '(none)'}.`);
    if (prompt) lines.push('', prompt);
    return lines.join('\n');
  });
  const result = blocks.join('\n\n');
  if (result.length > MAX_TOTAL_PROMPT_CHARS) throw new Error(`combined extension prompt exceeds ${MAX_TOTAL_PROMPT_CHARS} characters`);
  return result;
}
...
export async function loadExtensions(options: LoadExtensionsOptions): Promise<ExtensionRegistry> {
  const log = (level: 'info' | 'warn' | 'error', ...args: unknown[]): void => {
    try { options.log?.(level, ...args); } catch { /* diagnostics must not become a boot dependency */ }
  };
  const failures: ExtensionFailure[] = [];
  const fail = (file: string, namespace: string | null, stage: ExtensionFailureStage, error: unknown): void => {
    const failure = Object.freeze({ file, namespace, stage, error: message(error) });
  ...
  const importModule = options.importModule ?? (async (filePath: string) => tsImport(pathToFileURL(filePath).href, {
    parentURL: import.meta.url,
    tsconfig: false,
  }) as Promise<Record<string, unknown>>);
  const candidates: { file: string; namespace: string }[] = [];
  for (const entry of entries.filter((item) => item.isFile() && EXTENSION_FILE.test(item.name)).sort((a, b) => compareText(a.name, b.name))) {
    try {
      candidates.push({ file: entry.name, namespace: normalizeExtensionNamespace(entry.name) });
    } catch (error) {
      fail(entry.name, null, 'namespace', error);
    }
  }
  candidates.sort((a, b) => compareText(a.namespace, b.namespace) || compareText(a.file, b.file));
  const collided = new Set<string>();
  for (let start = 0; start < candidates.length;) {
    let end = start + 1;
    while (end < candidates.length && candidates[end].namespace === candidates[start].namespace) end += 1;
    if (end - start > 1) {
      const files = candidates.slice(start, end).map((candidate) => candidate.file).join(', ');
      for (const candidate of candidates.slice(start, end)) {
        fail(candidate.file, candidate.namespace, 'namespace', `namespace ${candidate.namespace} is also claimed by ${files}`);
      }
  ...
  const apis = Object.create(null) as Record<string, Readonly<Record<string, unknown>>>;
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      stage = 'api';
      if (!plainObject(rawApi)) throw new Error(`${candidate.file} extension.activate must return a plain object`);
      const api = freezeValue(rawApi, `elpis.ext.${candidate.namespace}`, new WeakSet()) as Readonly<Record<string, unknown>>;
      ...
      log('info', `extension loaded: ${candidate.file} -> elpis.ext.${candidate.namespace}`);
    } catch (error) {
