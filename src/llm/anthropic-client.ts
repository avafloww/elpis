...
const ANTHROPIC_BETAS = ['oauth-2025-04-20', 'claude-code-20250219', 'interleaved-thinking-2025-05-14'];
...
export function createBillingHeader(firstUserText: string): string {
  ...
  const suffix = createHash('sha256').update(`${FINGERPRINT_SALT}${k}${CLAUDE_CODE_VERSION}`).digest('hex').slice(0, 3);
...
interface WireMessage { role: 'user' | 'assistant'; content: ContentBlock[] }
...
function messageToBlocks(m: ChatMessage): { role: 'user' | 'assistant'; blocks: ContentBlock[] } | null {
  if (m.role === 'tool') {
    if (!m.tool_call_id) return null;
    const isError = (m.content ?? '').split('\n').some((l) => l.startsWith('[run FAILED'));
    return { role: 'user', blocks: [{ type: 'tool_result', tool_use_id: m.tool_call_id, content: m.content ?? '', is_error: isError }] };
  }
  if (m.role === 'assistant') {
 ...
 // through untouched.
    for (const tb of m.thinking_blocks ?? []) blocks.push(tb as unknown as ContentBlock);
...
export function translate(messages: ChatMessage[]): { system: TextBlock[]; wire: WireMessage[] } {
 ...
 // Tiered body with cache breakpoints after the last stable + last boundary block.
  ...
  const lastOfTier = (tier: SystemTier): number => {
    let idx = -1;
    segments.forEach((s, i) => { if (s.tier === tier) idx = i; });
    return idx;
  };
  ...
  segments.forEach((seg, i) => {
    ...
    if (i === stableBreak || i === boundaryBreak) block.cache_control = { type: 'ephemeral' };
    system.push(block);
  });
 ...
 // history caching). Only text/tool_result blocks accept cache_control.
  ...
  if (lastMsg) {
    for (let i = lastMsg.content.length - 1; i >= 0; i--) {
      ...
      if (b.type === 'text' || b.type === 'tool_result') { (b as TextBlock | ToolResultBlock & { cache_control?: CacheControl }).cache_control = { type: 'ephemeral' }; break; }
    }
  }
  return { system, wire };
}
...
function toLLMUsage(u: AnthropicUsage): LLMUsage {
 ...
 // `cached_tokens` = "served from prefix cache" contract.
    cached_tokens: u.cache_read_input_tokens === undefined ? undefined : cacheRead,
  };
}
...
async function postAnthropic(
 ...
 // check and the request — force a refresh so the retry uses a fresh token.
    ...
    const err = Object.assign(new Error(`anthropic ${res.status}: ${text}`), { status: res.status });
...
async function anthropicComplete(
      ...
      try {
        ...
        for await (const evt of parseSSE(res, controller.signal)) {
          if (evt.type === 'message_start') {
            const u = (evt.message as { usage?: AnthropicUsage } | undefined)?.usage;
            if (u) usage = { ...usage, ...u };
          } else if (evt.type === 'content_block_start') {
            const cb = evt.content_block as { type?: string; id?: string; name?: string; data?: string; thinking?: string; signature?: string } | undefined;
            const idx = evt.index as number;
            if (cb?.type === 'tool_use') toolBlocks[idx] = { id: cb.id, name: cb.name, json: '' };
            else if (cb?.type === 'thinking') thinkingAcc[idx] = { type: 'thinking', thinking: cb.thinking ?? '', signature: cb.signature ?? '' };
            else if (cb?.type === 'redacted_thinking') thinkingAcc[idx] = { type: 'redacted_thinking', data: cb.data ?? '' };
          } else if (evt.type === 'content_block_delta') {
            const idx = evt.index as number;
            const d = evt.delta as { type?: string; text?: string; thinking?: string; signature?: string; partial_json?: string } | undefined;
            if (d?.type === 'text_delta' && d.text) {
              content += d.text;
              try { hub?.streamDelta('content', d.text); } catch { /* observer only */ }
            } else if (d?.type === 'thinking_delta' && d.thinking) {
              reasoning += d.thinking;
              const tb = thinkingAcc[idx];
              if (tb && tb.type === 'thinking') tb.thinking += d.thinking;
              try { hub?.streamDelta('reasoning', d.thinking); } catch { /* observer only */ }
        ...
        throw e instanceof RetriableError || e instanceof NonRetriableError ? e : classifyError(e);
      }
 ...
 // Assemble tool calls; arguments must be the JSON string the run tool expects.
      const tool_calls = Object.keys(toolBlocks)
        ...
        .map((i) => toolBlocks[i])
        .filter((t) => t.id)
        .map((t) => ({ id: t.id as string, type: 'function' as const, function: { name: t.name ?? 'run', arguments: t.json || '{}' } }));

      const sanitized = sanitizeAssistantMessage({ content, reasoning_content: reasoning || undefined, tool_calls: tool_calls.length > 0 ? tool_calls : undefined });
 // Attach the captured thinking blocks (in stream order) for verbatim replay.
      const thinkingBlocks = Object.keys(thinkingAcc).map(Number).sort((a, b) => a - b).map((i) => thinkingAcc[i]);
      if (thinkingBlocks.length > 0) sanitized.message.thinking_blocks = thinkingBlocks;
      return { message: sanitized.message, stripped: sanitized.stripped, usage: toLLMUsage(usage), promptChars: charsSent, ...(requestId ? { requestId } : {}) };
  } finally {
    try { hub?.streamEnd(); } catch { /* observer only */ }
  }
}
...
async function anthropicSummarize(config: Config, store: OAuthStore, text: string): Promise<string> {
  ...
  if (data.stop_reason === 'max_tokens') {
    throw new Error('summarize truncated by max_tokens (anthropic stop_reason=max_tokens)');
  }
  return (data.content ?? []).filter((b) => b.type === 'text').map((b) => b.text ?? '').join('');
}

/** Parse an Anthropic SSE Response body into `data:` JSON events. */
async function* parseSSE(res: Response, signal: AbortSignal): AsyncGenerator<Record<string, unknown>> {
  const body = res.body;
  if (!body) return;
  ...
  for await (const chunk of body as unknown as AsyncIterable<Uint8Array>) {
    ...
    while ((nl = buffer.indexOf('\n')) >= 0) {
      ...
      if (line.startsWith('data:')) {
        ...
        try { yield JSON.parse(payload) as Record<string, unknown>; } catch { /* skip malformed */ }
      }
    }
  }
}
...
export function createAnthropicOAuthLLM(
  ...
  config.logger.info(`llm: using the Anthropic Messages surface (Claude subscription OAuth) | model=${config.llm.model}`);
  const identity = store.read();
  if (identity?.email) config.logger.info(`llm: anthropic account ${identity.email}${identity.orgName ? ` (${identity.orgName})` : ''}`);
  return {
    model: config.llm.model,
    runTool: RUN_TOOL,
