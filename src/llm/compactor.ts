...
export function walkKeepBoundary(
 ...
 */
export function enforcePairIntegrity(
  messages: ChatMessage[],
  boundary: number,
): number {
  let b = boundary;
  if (b > 0 && b < messages.length) {
    ...
    if (
      ...
      before.tool_calls &&
      before.tool_calls.length > 0
    ) {
      const ids = new Set(before.tool_calls.map((tc) => tc.id));
      while (
        b < messages.length &&
        messages[b].role === 'tool' &&
        ids.has(messages[b].tool_call_id!)
      ) {
        b++;
      }
    }
  ...
  return {
    ...
    start(messages: ChatMessage[]): void {
      ...
      if (
        head &&
        head.role === 'system' &&
        head.content.startsWith(SUMMARY_PREFIX)
      ) {
    ...
    applyCompaction(messages: ChatMessage[]): ChatMessage[] {
      ...
      const next = [
        summaryMessage(summary, boundaryIndex),
        ...kept,
        compactionNotice(boundaryIndex),
      ];
      resultSummary = null;
      boundaryIndex = 0;
