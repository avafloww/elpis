...
function parseTimestamp(
  ...
  if (!Number.isFinite(parsed))
    throw new Error(`${field} must be epoch milliseconds, ISO-8601, or null`);
  return parsed;
}
...
function compactTextData(data: unknown): unknown {
  ...
  if (detailItem)
    return { item_id: detailItem.id, fields: Object.keys(record) };
  ...
  if (Array.isArray(record.nodes))
    return {
      ...
      edges: Array.isArray(record.edges) ? record.edges.length : undefined,
    };
...
function toolResult(
  ...
  const textSummary: Record<string, unknown> = {
    ok: true,
    data: compactTextData(data),
  };
...
function compactGraph(graph: MindGraph): Record<string, unknown> {
  return {
    rootId: graph.rootId,
    nodes: graph.nodes.map(compactItem),
    edges: graph.edges,
  };
}
  ...
  return new Promise((resolve) => {
    ...
    const check = () => {
      const reply =
        mind
          .get(itemId)
          ?.comments.find((comment) => comment.replyToId === replyToId) ?? null;
...
function createSessionServer(deps: McpEndpointDeps): McpServer {
  ...
  const actor = (): string =>
    `mcp:${sanitizeClientName(server.server.getClientVersion()?.name)}`;
  ...
  const registerTool = <Input extends z.ZodRawShape>(
    ...
    const callback = (async (args, extra) => {
      try {
        return await handler(args as z.output<z.ZodObject<Input>>, extra);
      } catch (error) {
        deps.logger.warn(
          `mcp tool ${name}: ${error instanceof Error ? error.message : String(error)}`,
        );
        return toolError(error);
      }
    }) as ToolCallback<Input>;
    return server.registerTool<typeof TOOL_OUTPUT_SCHEMA, Input>(
      name,
    ...
    async (input) => {
      ...
      const base: MindListFilter = {
        ...
        ...(input.parent_id !== undefined
          ? {
              parentId:
                input.parent_id === null ? null : parseMindId(input.parent_id),
            }
          : {}),
      ...
      return toolResult(
        { items: items.map(compactItem), returned: items.length },
        {
          page: {
            next_cursor:
              nextOffset < totalCount ? encodeListCursor(nextOffset) : null,
  ...
  registerTool(
    "mind_get",
    {
      ...
      inputSchema: {
        id: ID,
        include: z
          .array(z.enum(DETAIL_PARTS))
          .max(DETAIL_PARTS.length)
          .optional(),
    ...
    async ({ id, include, comment_limit, event_limit }) => {
      ...
      return toolResult(
        projectDetail(
          item,
          include as DetailPart[] | undefined,
          comment_limit,
          event_limit,
        ),
      );
  ...
  registerTool(
    ...
    async ({ limit }) => {
      const items = deps.mind.ready(limit);
      return toolResult({
  ...
  registerTool(
    ...
    async ({ id, depth, relations }) =>
      toolResult(
        compactGraph(
          deps.mind.graph(
            parseMindId(id),
            depth,
            relations as MindGraphRelation[] | undefined,
          ),
        ),
      ),
  );
        ...
        include_related === false
          ...
          : deps.mind
              .discover(`${item.title}\n${item.body}\n${item.tags.join(" ")}`, {
                ...
                parentId:
                  related_parent_id === undefined
                    ? undefined
                    : related_parent_id === null
                      ? null
                      : parseMindId(related_parent_id),
                limit: (related_limit ?? 6) + 1,
              })
              .filter((match) => match.item.id !== itemId)
              .slice(0, related_limit ?? 6);
      return toolResult({
        ...
        graph: compactGraph(
          deps.mind.graph(
            itemId,
            depth ?? 2,
            graph_relations as MindGraphRelation[] | undefined,
          ),
        ),
  ...
  registerTool(
    ...
    async ({ id, ttl_minutes }) => {
      const item = deps.mind.renewClaim(
        parseMindId(id),
        principal,
  ...
  registerTool(
    ...
    async ({ id, note, ttl_minutes }) => {
      const item = deps.mind.resumeClaim(parseMindId(id), {
        owner: actor(),
        principal,
        note,
        ttlMs: ttl_minutes === undefined ? undefined : ttl_minutes * 60_000,
  ...
  registerTool(
    ...
    async ({ id, result, verification, omissions }) => {
      const item = deps.mind.finishClaim(
        parseMindId(id),
        principal,
        actor(),
        result,
  ...
  registerTool(
    ...
    async ({ ids, note }) => {
      ...
      const items = itemIds.map((id) => {
        if (!createdItemIds.has(id))
          throw new Error(
            `mind: item #${id} was not created by this MCP session`,
          );
        const item = deps.mind.get(id);
        if (!item) throw new Error(`mind: no item #${id}`);
        if (item.claim)
          throw new Error(
            `mind: item #${id} has an active claim; release it before session cleanup`,
          );
        return item;
      });
      const archived = items.map((item) => {
        if (item.archivedAt != null) return item;
        deps.mind.addComment(
          item.id,
          `Session-scoped archive: ${note}`,
          actor(),
        );
        return deps.mind.archive(item.id, actor());
      });
      const changed = archived.filter(
        (item, index) =>
          items[index].archivedAt == null && item.archivedAt != null,
      ).length;
      return toolResult(
        {
          items: archived.map(compactItem),
  ...
  registerTool(
    ...
    async ({ id, depends_on }) => {
      const itemId = parseMindId(id);
      const dependencyId = parseMindId(depends_on);
      const before =
        deps.mind
          .get(itemId)
  ...
  registerTool(
    ...
    async ({ id, body, wait_seconds }, extra) => {
      ...
      let woke = true;
      try {
        deps.wake({ taskId, commentId: comment.id, actor: from, body });
      } catch (error) {
        woke = false;
        deps.logger.warn(
          `mcp: comment c#${comment.id} persisted but wake failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      const reply = woke
        ? await waitForReply(
  ...
  registerTool(
    ...
    async ({ id, comment_id, wait_seconds }, extra) => {
      const taskId = parseMindId(id);
      const source = deps.mind
        .get(taskId)
        ?.comments.find((comment) => comment.id === comment_id);
      if (!source)
        throw new Error(`mind: no comment #${comment_id} on item #${taskId}`);
      ...
      try {
        if (method === "POST") {
          ...
          transport.onerror = (error) =>
            deps.logger.warn(`mcp transport: ${error.message}`);
        ...
        if (!res.headersSent)
          rpcError(
            res,
            /exceeds/.test(String(error)) ? 413 : 400,
