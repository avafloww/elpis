...
const HM_OPTIONS = { hour: '2-digit', minute: '2-digit', hourCycle: 'h23' } as const;
const STAMP_OPTIONS = {
  weekday: 'short',
  month: 'short',
  day: 'numeric',
...
export function localHm(ms: number, timeZone?: string): string {
  const fmt = new Intl.DateTimeFormat('en-US', { ...HM_OPTIONS, timeZone } as any);
  return `[${fmt.format(ms)}]`;
}

export function localStamp(ms: number, timeZone?: string): string {
  const fmt = new Intl.DateTimeFormat('en-US', { ...STAMP_OPTIONS, timeZone } as any);
