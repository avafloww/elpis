...
export function protectDisplayHeredocs(input: string): ProtectedDisplayCode {
  ...
  for (let n = 0; n < HEREDOC_MAX; n++) {
    ...
    const terminator = new RegExp(`^[ \\t]*${tag}(?![A-Za-z0-9_])([^\\r\\n]*)\\r?$`, 'm').exec(rest);
    if (!terminator) return { code: input, heredocs: [], error: `heredoc <<<${tag} has no terminator` };
    const trailer = terminator[1] ?? '';
    const carriage = terminator[0].endsWith('\r') ? 1 : 0;
    const terminatorTagLength = terminator[0].length - trailer.length - carriage;
    ...
    code = code.slice(0, match.index) + JSON.stringify(token) + trailer + code.slice(blockEnd);
  }
  return { code: input, heredocs: [], error: `too many heredocs in one run (max ${HEREDOC_MAX})` };
}

export function restoreDisplayHeredocs(input: string, heredocs: DisplayHeredoc[]): string {
  let code = input;
