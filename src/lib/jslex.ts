...
export function blankLiterals(code: string): string {
 ...
 // template depth together.
  ...
  for (let i = 0; i < code.length; i++) {
    ...
    switch (state) {
      case 'code':
        if (ch === '/' && next === '/') { keep(' '); blank(next); i++; state = 'line-comment'; break; }
        if (ch === '/' && next === '*') { keep(' '); blank(next); i++; state = 'block-comment'; break; }
        if (ch === "'") { keep(ch); state = 'single'; break; }
        if (ch === '"') { keep(ch); state = 'double'; break; }
        if (ch === '`') { keep(ch); state = 'template'; templateDepth = 1; braceDepth = 0; break; }
        keep(ch);
        break;
      case 'single':
      case 'double': {
        const quote = state === 'single' ? "'" : '"';
        if (ch === '\\') { blank(ch); if (next !== undefined) { blank(next); i++; } break; }
        if (ch === quote) { keep(ch); state = 'code'; break; }
 ...
 // interpolation, we stay in template state for the outer one.
          templateDepth--;
          if (templateDepth === 0) { keep(ch); state = 'code'; } else { blank(ch); }
          break;
        }
        if (ch === '$' && next === '{') { blank(ch); blank(next); i++; braceDepth++; break; }
        if (braceDepth > 0) {
 // Inside an interpolation: blank it, but track nested braces and
 // nested template opens so we resume the right context.
          if (ch === '{') { blank(ch); braceDepth++; break; }
          if (ch === '}') { blank(ch); braceDepth--; break; }
          if (ch === '`') { blank(ch); templateDepth++; break; }
        }
        blank(ch);
        break;
      case 'line-comment':
        ...
        break;
      case 'block-comment':
        if (ch === '*' && next === '/') { blank(ch); blank(next); i++; state = 'code'; break; }
        blank(ch);
        break;
    }
  }
