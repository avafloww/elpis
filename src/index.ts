...
export function isUnannouncedRestart(
...
export function formatSandboxLateProcessErrorNotice(
...
export async function createElpisRuntime(
  ...
  // message arrives.
  const channels = createChannelDirectory(
    db,
  ...
  const emotes = config.discord.emoteImages
    ? createEmoteRegistry({
        log: config.logger,
        keyframes: config.discord.emoteKeyframes,
      })
    : undefined;
  ...
  if (
    memoryLimits.threshold !== config.memory.consolidationThresholdTokens ||
    memoryLimits.target !== config.memory.consolidationTargetTokens
  ) {
    config.logger.warn(
      `memory consolidation limits clamped to model window: threshold=${memoryLimits.threshold}, target=${memoryLimits.target} tokens`,
    );
  }
  ...
  const agent = new Agent({
    ...
    emotes,
    send: async () => {
      /* replaced by discord wiring on start */
    },
  ...
  try {
    ...
    config.logger.warn(
      `harness-changelog delivery failed: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
  ...
  const shutdown = (sig: string) => {
    ...
    try {
      ...
      config.logger.warn(
        `shutdown transcript flush failed: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
    process.exit(0);
  };
