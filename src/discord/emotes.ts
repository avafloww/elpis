...
export function emoteCdnUrl(ref: EmoteOrStickerRef): string {
  ...
  switch (ref.format) {
    case 'gif':
      return `https://media.discordapp.net/stickers/${ref.id}.gif`;
    case 'lottie':
      return `https://cdn.discordapp.com/stickers/${ref.id}.json`;
    default:
      return `https://cdn.discordapp.com/stickers/${ref.id}.png`;
  }
}
...
export function attachmentName(
  ref: EmoteOrStickerRef,
  opts?: { frame?: number; of?: number; ext?: string },
): string {
...
export function ffmpegExtractArgs(
  inputPath: string,
  outPattern: string,
  format: 'apng' | null,
): string[] {
...
export function createEmoteRegistry(opts: {
  ...
  const { log } = opts;
  const keyframes = Math.min(
    MAX_KEYFRAMES,
    Math.max(1, Math.floor(opts.keyframes)),
  );
  ...
  async function readManifest(dir: string): Promise<ManifestEntry[] | null> {
    try {
      const m = JSON.parse(
        await fs.promises.readFile(path.join(dir, 'attach.json'), 'utf8'),
      ) as Manifest;
      if (!Array.isArray(m.entries) || m.entries.length === 0) return null;
      for (const e of m.entries)
        await fs.promises.access(path.join(dir, e.file));
      return m.entries;
    } catch {
      return null;
    }
  ...
  async function writeManifest(
    ...
    await fs.promises.writeFile(
      path.join(dir, 'attach.json'),
      JSON.stringify({ entries } satisfies Manifest),
    );
  }
  ...
  async function download(url: string, dest: string): Promise<number> {
    ...
    if (buf.byteLength > DOWNLOAD_MAX_BYTES)
      throw new Error(`${buf.byteLength} bytes exceeds cap`);
  ...
  function runFfmpeg(
    ...
    return new Promise((resolve) => {
      ...
      child.on('error', (e) => {
        clearTimeout(timer);
        resolve({ code: 127, stderr: e.message });
      });
      child.on('close', (code) => {
        clearTimeout(timer);
        resolve({ code, stderr });
  ...
  async function extractKeyframes(
    ...
    const apng =
      ref.kind === 'sticker' && ref.format === 'apng'
        ? ('apng' as const)
        : null;
    const { code, stderr } = await runFfmpeg(
      ffmpegExtractArgs(origPath, path.join(framesDir, 'f-%03d.png'), apng),
    );
    const produced = (await fs.promises.readdir(framesDir))
      .filter((f) => f.startsWith('f-'))
      .sort();
    if (code !== 0 || produced.length === 0) {
      log.warn(
        `emotes: ffmpeg extract failed for ${refKey(ref)} (code ${code}): ${stderr.slice(0, 200)}`,
      );
      await fs.promises.rm(framesDir, { recursive: true, force: true });
      return null;
    }
    ...
    for (let i = 0; i < picked.length; i++) {
      ...
      await fs.promises.copyFile(
        path.join(framesDir, produced[picked[i]]),
        dest,
      );
  ...
  async function ensureFiles(
    ...
    if (ref.kind === 'sticker' && ref.format === 'lottie') {
      const entries: ManifestEntry[] = [
        {
          name: attachmentName(ref, { ext: 'json' }),
          file: '',
    ...
    try {
      ...
      log.warn(
        `emotes: download failed for ${refKey(ref)} (${ref.name}): ${e instanceof Error ? e.message : String(e)}`,
      );
      return null;
    }
  ...
  return {
    async collect(
      input: EmoteCollectInput,
    ): Promise<InboundMessageAttachment[]> {
      const refs: EmoteOrStickerRef[] = [
        ...
        ...(input.stickers ?? []).flatMap((s): StickerRef[] => {
          ...
          return format
            ? [{ kind: 'sticker', id: s.id, name: s.name, format }]
            : [];
        }),
      ];
      ...
      if (capped.length < fresh.length) {
        log.warn(
          `emotes: ${fresh.length - capped.length} first-use emote(s) beyond the per-message cap of ${MAX_NEW_PER_MESSAGE} deferred to next use`,
        );
      }
        ...
        capped.map(async (ref) => {
          ...
          if (!p) {
            ...
            p = ensureFiles(ref).then(
              ...
              (e) => {
                log.warn(
                  `emotes: ensure failed for ${key} (${ref.name}): ${e instanceof Error ? e.message : String(e)}`,
                );
                return null;
              },
            );
          ...
          if (entries === 'timeout') {
            log.info(
              `emotes: ${key} (${ref.name}) not ready within ${deadlineMs}ms — cache priming continues, attaches on next use`,
            );
            return [];
          }
          ...
          seen.add(key);
          log.info(
            `emotes: attached first-use ${key} (${ref.name}) x${entries.length}`,
          );
