...
export async function downloadAttachment(
  ...
  try {
    ...
    if (attachment.size > ATTACHMENT_MAX_BYTES) {
      log.warn(
        `downloadAttachment: skipping ${attachment.name}: ${attachment.size} bytes exceeds limit`,
      );
      return null;
    }
    ...
    log.warn(
      `downloadAttachment: failed ${attachment.name}: ${e instanceof Error ? e.message : String(e)}`,
    );
    return null;
  }
}
...
export function isInlinableAttachmentType(contentType: string | null): boolean {
  ...
  return (
    /^text\//i.test(contentType) || /^application\/json\b/i.test(contentType)
  );
}
...
async function buildInboundAttachments(
  ...
  const localPaths = await Promise.all(
    attachments.map((a, index) =>
      downloadAttachment(
        { url: a.url, name: a.name, size: a.size, id: a.id },
        message.id,
        index,
        log,
      ),
  ...
  for (let i = 0; i < attachments.length; i++) {
    ...
    if (localPath && contentType && /^image\//i.test(contentType)) {
      ...
      if (resolved !== contentType) {
        log.info(
          `buildInboundAttachments: ${a.name}: declared ${contentType} but bytes are ${resolved} — using sniffed type`,
        );
    ...
    if (
      localPath &&
      isInlinableAttachmentType(a.contentType) &&
      ...
      try {
        ...
        log.warn(
          `buildInboundAttachments: inline read failed for ${a.name}: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    }
    out.push({
...
export function clearConfirmCustomId(userId: string): string {
  return `clear-confirm:${userId}`;
}
export function clearCancelCustomId(userId: string): string {
  return `clear-cancel:${userId}`;
}
/** Custom-ids for the /clear-thinking confirmation buttons, namespaced by user
 * id (same pattern as /clear). Pure; tested. */
export function clearThinkingConfirmCustomId(userId: string): string {
  return `clear-thinking-confirm:${userId}`;
}
export function clearThinkingCancelCustomId(userId: string): string {
  return `clear-thinking-cancel:${userId}`;
}
  ...
  return {
    ...
    mentionsMe:
      botUserId === undefined
        ...
        : mentionedUserIds.includes(botUserId),
    replyToMe:
      botUserId === undefined
        ? replyToAuthorId !== null
        : replyToAuthorId === botUserId,
  };
}
 ...
 * Pure; exported for tests. */
export function applyOutboundMentions(
  text: string,
...
export function outboundMentionDirectory(
 ...
 * more. */
function nameMap<T extends { id: string }>(
  coll: unknown,
  name: (x: T) => string,
): Map<string, string> {
  const items = coll as
...
export function isAuthorizedOperator(config: Config, userId: string): boolean {
  return (
    config.operator.discordId !== null && config.operator.discordId === userId
  );
}
...
export function formatUsageBars(
...
async function safeReply(
  ...
  try {
    ...
    log.warn(
      `failed to reply to /${name}: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
}
...
export function mindAddAmbientNotice(
  item: MindItem,
  source: {
    channelId: string;
...
export function createDiscord(
  ...
  client.on(Events.MessageCreate, async (message: Message) => {
    ...
    const policyChannelName = channel.isThread()
      ? channelDisplayName(channel.parent)
    ...
    if (!policy || policy.tier === "drop") {
      ...
      if (!droppedLogged.has(key)) {
        ...
        log.info(
          `dropping #${policyChannelName} (${policyChannelId}) in ${guild.slug} — receive mode is drop`,
        );
      }
      return;
    }
    ...
    if (
      policy.guild.pluralKit &&
      (isPluralKitCommand(message.content) ||
        message.author.id === PLURALKIT_BOT_ID)
    )
      return;
    ...
    if (policy.guild.pluralKit) {
      try {
        pkInfo = await pluralKit.resolve(
          message.id,
          message.author.bot !== true && message.webhookId === null,
        );
      } catch (e) {
        log.warn(
          `PluralKit lookup failed for ${message.id}: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    }
    if (pkInfo?.original === message.id) {
      log.debug(
        `suppressing PluralKit original ${message.id} (proxy ${pkInfo.id})`,
      );
      return;
    }
    ...
    const identity = pkInfo
      ? pluralKitIdentity(pkInfo, fallbackAuthor)
      : { author: fallbackAuthor, authorId: message.author.id };
    const authorIsBot = pkInfo ? false : message.author.bot === true;
    log.debug(
      `inbound message #${message.channelId} <${identity.author}>: ${message.content.slice(0, 120)}`,
    );
      ...
      try {
        ...
        if (policy.guild.pluralKit) {
          try {
            ...
            log.warn(
              `PluralKit reply lookup failed for ${ref.id}: ${e instanceof Error ? e.message : String(e)}`,
            );
          }
        }
        return {
    ...
    if (cls === "drop") {
      ...
      if (!droppedLogged.has(key)) {
        ...
        if (wrongGuild) {
          log.info(
            `dropping #${policyChannelName} (${policyChannelId}) — claimed by guild ${message.guildId} but configured under ${policy.guild.slug}`,
          );
        } else {
          log.info(
            `dropping #${policyChannelName} (${policyChannelId}) in ${policy.guild.slug} — deafened`,
          );
        }
      }
      return;
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (
      !interaction.guildId ||
      !config.discord.guilds.some((g) => g.id === interaction.guildId)
    )
      return;
    ...
    if (!isAuthorizedOperator(config, interaction.user.id)) {
      await safeReply(log, name, () =>
        interaction.reply({
          content: operatorGateReason(config, name),
    ...
    if (name === "mind") {
      ...
      try {
        ...
        if (sub === "list") {
          ...
          const guild = config.discord.guilds.find(
        ...
        await safeReply(log, name, () =>
          interaction.reply({
            content: `Mind: ${error instanceof Error ? error.message : String(error)}`,
            flags: MessageFlags.Ephemeral,
          }),
        );
      }
    ...
    if (name === "clear" || name === "new") {
      ...
      try {
        ...
        log.warn(
          `failed to run /${name}: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
      return;
    }
    ...
    if (name === "exec") {
      ...
      await safeReply(log, name, () =>
        interaction.reply({ content: out, flags: MessageFlags.Ephemeral }),
      );
      return;
    }
    ...
    if (name === "restart") {
      ...
      try {
        ...
        log.warn(
          `/restart: transcript flush failed: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
      ...
      setTimeout(() => {
        try {
          ...
          log.error(
            `/restart: restart hook threw: ${e instanceof Error ? e.message : String(e)}`,
          );
        }
      }, 500);
      return;
...
async function registerSlashCommands(config: Config): Promise<void> {
  ...
  for (const g of optedIn) {
    try {
      await rest.put(
        Routes.applicationGuildCommands(config.discord.applicationId, g.id),
        { body: commands },
      );
      config.logger.info(
      ...
      config.logger.warn(
        `slash command registration failed for guild ${g.id}: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
}
