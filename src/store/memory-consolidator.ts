...
export interface MemoryConsolidationResult {
  path: string;
  status: 'disabled' | 'below-threshold' | 'unchanged' | 'consolidated' | 'failed';
  beforeTokens: number;
  afterTokens: number;
  error?: string;
}

export function effectiveMemoryLimits(threshold: number, target: number, context: number, reserve: number): { threshold: number; target: number } {
  ...
  const effectiveTarget = Math.min(target, Math.max(512, Math.floor(effectiveThreshold * 0.75)));
  return { threshold: effectiveThreshold, target: Math.min(effectiveTarget, effectiveThreshold - 1) };
}
...
function splitFrontmatter(text: string): { prefix: string; body: string } {
  const match = /^(---\r?\n[\s\S]*?\r?\n---\r?\n)/.exec(text);
  return match ? { prefix: match[1], body: text.slice(match[1].length) } : { prefix: '', body: text };
}
...
function privatePrompt(kind: 'memory' | 'person', file: string, target: number, part: string, soul: string): string {
  const specific = kind === 'person'
...
function atomicReplace(file: string, original: string, replacement: string, dataDirectory: string): void {
  ...
  const mode = (() => { try { return fs.statSync(file).mode & 0o777; } catch { return 0o600; } })();
  const temp = path.join(path.dirname(file), `.${path.basename(file)}.consolidating-${process.pid}-${crypto.randomUUID()}`);
  try {
  ...
  const old = fs.readdirSync(backups)
    ...
    .sort()
    .reverse();
  for (const name of old.slice(5)) fs.rmSync(path.join(backups, name), { force: true });
}
...
export class MemoryConsolidator {
  ...
  private allowed(file: string): boolean {
    ...
    const people = path.resolve(this.options.dataDirectory, 'people') + path.sep;
    return resolved.startsWith(people) && path.dirname(resolved) === people.slice(0, -1) && resolved.endsWith('.md');
  }

  private async rewrite(file: string, original: string): Promise<string> {
    const { prefix, body } = splitFrontmatter(original);
    const kind = path.dirname(file) === path.join(this.options.dataDirectory, 'people') ? 'person' as const : 'memory' as const;
    const soul = (() => { try { return fs.readFileSync(this.options.soulPath, 'utf8'); } catch { return ''; } })();
    const ratio = Math.max(2, Math.min(6, 10_000 / Math.max(1, this.options.estimateTokens(10_000))));
    const maxInputTokens = Math.max(512, Math.min(this.options.thresholdTokens, Math.floor(this.options.maxContextTokens * 0.55)));
    const maxChars = Math.max(2_048, Math.floor(maxInputTokens * ratio));
    let current = body;

    for (let round = 0; round < 4; round++) {
      const parts = chunksAt(current, maxChars);
      const perPartTarget = Math.max(256, Math.floor(this.options.targetTokens / parts.length));
      const summaries: string[] = [];
      for (let i = 0; i < parts.length; i++) {
        const system = privatePrompt(kind, file, perPartTarget, `chunk ${i + 1}/${parts.length}, pass ${round + 1}`, soul);
        const messages = [{ role: 'system' as const, content: system }, { role: 'user' as const, content: parts[i] }];
        const raw = this.options.llm.completeStandalone
          ? (await this.options.llm.completeStandalone(messages, { cacheKey: `memory-consolidation-${crypto.randomUUID()}` })).content
          : await this.options.llm.summarize(parts[i], system);
        const cleaned = cleanModelOutput(raw);
        if (!cleaned) throw new Error(`empty consolidation output for chunk ${i + 1}/${parts.length}`);
        summaries.push(cleaned);
      }
      const next = summaries.join('\n\n').trim() + '\n';
      if (next.length >= current.length) throw new Error(`consolidation did not shrink pass ${round + 1} (${current.length}→${next.length} chars)`);
      current = next;
      const candidate = prefix + current;
      if (this.options.estimateTokens(candidate.length) <= this.options.thresholdTokens) return candidate;
    }
    throw new Error(`consolidation remained above ${this.options.thresholdTokens} tokens after 4 passes`);
  }

  async checkNow(file: string, force = false): Promise<MemoryConsolidationResult> {
    const resolved = path.resolve(file);
    if (!this.allowed(resolved)) throw new Error(`memory consolidation path is outside MEMORY.md/people: ${resolved}`);
    if (this.options.thresholdTokens === 0) return { path: resolved, status: 'disabled', beforeTokens: 0, afterTokens: 0 };
    let original: string;
    try { original = fs.readFileSync(resolved, 'utf8'); }
    catch { return { path: resolved, status: 'unchanged', beforeTokens: 0, afterTokens: 0 }; }
    const beforeTokens = this.options.estimateTokens(original.length);
    if (beforeTokens <= this.options.thresholdTokens) return { path: resolved, status: 'below-threshold', beforeTokens, afterTokens: beforeTokens };
    const originalDigest = digest(original);
    if (!force && this.failedDigests.get(resolved) === originalDigest) {
      return { path: resolved, status: 'unchanged', beforeTokens, afterTokens: beforeTokens };
    }

    this.options.logger.info(`[memory] consolidating ${path.relative(this.options.dataDirectory, resolved)} (~${beforeTokens} tokens; threshold ${this.options.thresholdTokens})`);
    try {
      const replacement = await this.rewrite(resolved, original);
      if (digest(fs.readFileSync(resolved, 'utf8')) !== originalDigest) throw new Error('file changed while consolidation was running; preserving newer write');
      const afterTokens = this.options.estimateTokens(replacement.length);
      if (afterTokens >= beforeTokens || afterTokens > this.options.thresholdTokens) {
        throw new Error(`output failed size gate (${beforeTokens}→${afterTokens} tokens)`);
      }
      atomicReplace(resolved, original, replacement, this.options.dataDirectory);
      this.failedDigests.delete(resolved);
      this.options.logger.info(`[memory] consolidated ${path.relative(this.options.dataDirectory, resolved)}: ~${beforeTokens}→${afterTokens} tokens`);
      return { path: resolved, status: 'consolidated', beforeTokens, afterTokens };
    } catch (error) {
      this.failedDigests.set(resolved, originalDigest);
      const message = error instanceof Error ? error.message : String(error);
      this.options.logger.warn(`[memory] consolidation failed for ${path.relative(this.options.dataDirectory, resolved)}; original preserved: ${message}`);
      return { path: resolved, status: 'failed', beforeTokens, afterTokens: beforeTokens, error: message };
    }
  }
  ...
  async ensureBootSafe(): Promise<MemoryConsolidationResult[]> {
    ...
    for (const name of fs.readdirSync(peopleDir).sort()) if (name.endsWith('.md')) files.push(path.join(peopleDir, name));
  ...
  safeMemoryView(): string {
    let raw = '';
    try { raw = fs.readFileSync(this.options.memoryPath, 'utf8'); } catch { return ''; }
    if (this.options.thresholdTokens === 0 || this.options.estimateTokens(raw.length) <= this.options.thresholdTokens) return raw;
    const ratio = Math.max(2, Math.min(6, 10_000 / Math.max(1, this.options.estimateTokens(10_000))));
    ...
    const configuredBudget = Math.floor(this.options.thresholdTokens * ratio * 0.8);
    const budget = Math.min(configuredBudget, Math.max(header.length + 200, Math.floor(raw.length * 0.75)));
  ...
  private enqueue(file: string): void {
    this.queue = this.queue.then(async () => { await this.checkNow(file); }).catch(error => {
      this.options.logger.warn(`[memory] consolidation queue error: ${error instanceof Error ? error.message : String(error)}`);
    });
  }
  ...
  startWatching(): void {
    ...
    const watch = (dir: string, accept: (name: string) => boolean): void => {
      const watcher = fs.watch(dir, { persistent: false }, (_event, name) => {
        if (typeof name === 'string' && accept(name)) this.request(path.join(dir, name));
      });
      watcher.on('error', error => this.options.logger.warn(`[memory] watcher error for ${dir}: ${error.message}`));
      this.watchers.push(watcher);
    };
    watch(path.dirname(this.options.memoryPath), name => path.resolve(path.dirname(this.options.memoryPath), name) === path.resolve(this.options.memoryPath));
