...
function runCodeMigration(
  ...
  const wrapIterator = <T>(iterator: Iterator<T>): Iterator<T> =>
    new Proxy(iterator, {
      get(target, property) {
        assertActive();
    ...
    new Proxy(statement, {
      get(target, property) {
        ...
        return (...args: unknown[]) => {
          ...
          return property === 'iterate'
            ? wrapIterator(result as Iterator<unknown>)
            : result;
        };
      },
    });
...
function normalizeMigration(
  ...
  if (hasSql === hasUp)
    throw new Error(
      `migration ${value.name} must define exactly one of sql or up`,
    );
  const allowed = new Set(
    hasSql ? ['name', 'sql'] : ['name', 'checksum', 'up'],
  );
  const unknown = Object.keys(value).find((key) => !allowed.has(key));
  if (unknown)
    throw new Error(
      `migration ${value.name} contains unknown field ${unknown}`,
    );
  if (hasSql) {
    const sql = (value as SqlMigration).sql;
    if (typeof sql !== 'string' || !sql.trim())
      throw new Error(`migration ${value.name}.sql must be a non-empty string`);
    return {
      name: value.name,
      checksum: checksum(sql),
      up: (database) => database.exec(sql),
    };
  }
  const code = value as CodeMigration;
  if (typeof code.up !== 'function')
    throw new Error(`migration ${value.name}.up must be a function`);
  if (code.up.constructor.name === 'AsyncFunction')
    throw new Error(`migration ${value.name}.up must be synchronous`);
  if (
    typeof code.checksum !== 'string' ||
    !CHECKSUM_PATTERN.test(code.checksum)
  ) {
    throw new Error(
      `migration ${value.name}.checksum must be a lowercase SHA-256 hex string`,
    );
  }
  return {
    name: value.name,
    checksum: code.checksum,
    up: (database) => runCodeMigration(database, code.up),
  };
}
...
export function runComponentMigrations(
  ...
  if (!COMPONENT_PATTERN.test(component))
    throw new Error(`migration component must match ${COMPONENT_PATTERN}`);
  if (!Array.isArray(migrations))
    throw new Error(`migrations for ${component} must be an array`);
  const normalized = migrations.map(normalizeMigration);
  for (let index = 1; index < normalized.length; index += 1) {
    if (normalized[index - 1].name >= normalized[index].name) {
      throw new Error(
        `migrations for ${component} must be strictly sorted by unique name`,
      );
    }
  }
  ...
  if (applied.length > normalized.length) {
    throw new Error(
      `migration history for ${component} contains undeclared entries`,
    );
  }
  for (let index = 0; index < applied.length; index += 1) {
    ...
    if (!declared || receipt.name !== declared.name) {
      throw new Error(
        `migration history for ${component} is not an exact declared prefix at ${receipt.name}`,
      );
    }
    if (receipt.checksum !== declared.checksum) {
      throw new Error(
        `migration checksum drift for ${component}/${receipt.name}`,
      );
    }
  }
  ...
  for (const migration of normalized.slice(applied.length)) {
    ...
    try {
      ...
      throw new Error(
        `migration ${component}/${migration.name} failed: ${message(error)}`,
        { cause: error },
      );
    }
  }
